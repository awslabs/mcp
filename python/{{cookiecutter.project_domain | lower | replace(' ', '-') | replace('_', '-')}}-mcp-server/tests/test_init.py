...
"""Tests for the awslabs.{{cookiecutter.project_domain | lower | replace(' ', '-') | replace('_', '-')}}-mcp-server package."""
...
class TestInit:
    ...
    def test_version(self):
        ...
        assert re.match(version_pattern, awslabs.{{cookiecutter.project_domain | lower | replace(' ', '-') | replace('_', '-') | replace('-', '_')}}_mcp_server.__version__), (
            f"Version '{awslabs.{{cookiecutter.project_domain | lower | replace(' ', '-') | replace('_', '-') | replace('-', '_')}}_mcp_server.__version__}' does not follow semantic versioning"
        )
    ...
    def test_module_reload(self):
        ...
        original_version = awslabs.{{cookiecutter.project_domain | lower | replace(' ', '-') | replace('_', '-') | replace('-', '_')}}_mcp_server.__version__

        # Reload the module
        importlib.reload(awslabs.{{cookiecutter.project_domain | lower | replace(' ', '-') | replace('_', '-') | replace('-', '_')}}_mcp_server)

        # Check that the version is still the same
        assert awslabs.{{cookiecutter.project_domain | lower | replace(' ', '-') | replace('_', '-') | replace('-', '_')}}_mcp_server.__version__ == original_version
