...
async def process_query(query: str, kb_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(
            "Initializing MCP client with awslabs.bedrock-kb-retrieval-mcp-server"
        )
        ...
        async with mcp_client as client:
            ...
            logger.info(
                f"Retrieved {len(original_tools)} tools from MCP server: {tool_names}"
            )
            ...
            if not original_tools:
                ...
                return {
                    "messages": [
                        {
                            "content": "No tools available from the knowledge base server."
                        }
                    ]
                }
            ...
            if hasattr(response, "tool_calls") and response.tool_calls:
                ...
                for tool_call in response.tool_calls:
                    ...
                    new_messages.append(
                        response
                    )  # Add the original AI response with tool_calls
                    new_messages.append(
                        ToolMessage(
