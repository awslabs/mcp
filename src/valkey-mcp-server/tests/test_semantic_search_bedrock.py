...
class TestSemanticSearchBedrock:
    ...
    async def test_bedrock_semantic_search(self):
        ...
        provider = BedrockEmbeddings(region_name="us-east-1", model_id="amazon.titan-embed-text-v1")
        ...
        with patch('awslabs.valkey_mcp_server.tools.semantic_search._embeddings_provider', provider):
            result = await add_documents(
                collection="bedrock_test",
                documents=documents,
                text_fields=["name", "content"]
            )
