...
async def list_move_blocking(
    ...
    """Move element from one list to another, blocking if source is empty.
    ...
    Args:
        ...
        wherefrom: Where to pop from ("LEFT" or "RIGHT")
        whereto: Where to push to ("LEFT" or "RIGHT")
        timeout: Maximum seconds to block (optional)
    ...
    try:
        ...
        result = r.blmove(source, destination, wherefrom, whereto, timeout)
        if result is None:
            return f"Timeout waiting for value in source list '{source}'"
        return f"Successfully moved value '{result}' from {wherefrom} of '{source}' to {whereto} of '{destination}'"
    except ValkeyError as e:
        return f'Error moving value between lists: {str(e)}'
...
@mcp.tool()
async def list_pop_blocking(key: str, timeout: float, side: str = 'LEFT') -> str:
    """Pop value from list, blocking if empty.
    ...
    try:
        ...
        if side not in ['LEFT', 'RIGHT']:
            return "Error: side must be either 'LEFT' or 'RIGHT'"

        if side == 'LEFT':
            result = r.blpop(key, timeout)
        else:
            result = r.brpop(key, timeout)

        if result is None:
            return f"Timeout waiting for value in list '{key}'"
        return f"Successfully popped value '{result[1]}' from {side} of '{key}'"
    except ValkeyError as e:
        return f"Error popping from list '{key}': {str(e)}"


@mcp.tool()
async def list_pop_push_blocking(
    source: str, destination: str, timeout: Optional[float] = None
) -> str:
    """Pop from source list and push to destination list, blocking if source empty.
    ...
    try:
        r = ValkeyConnectionManager.get_connection()
        result = r.brpoplpush(source, destination, timeout)
        if result is None:
            return f"Timeout waiting for value in source list '{source}'"
        return f"Successfully moved value '{result}' from '{source}' to '{destination}'"
    except ValkeyError as e:
        return f'Error moving value between lists: {str(e)}'
