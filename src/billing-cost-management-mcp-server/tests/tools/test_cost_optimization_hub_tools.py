    ...
    if operation == "get_recommendation_summaries":
        ...
        if not kwargs.get("group_by"):
            return format_response(
                "error",
                {},
                "group_by is required for list_recommendations operation"
            )
...
def test_cost_optimization_hub_server_initialization():
    ...
    assert "Tools for working with AWS Cost Optimization Hub API" in cost_optimization_hub_server.instructions