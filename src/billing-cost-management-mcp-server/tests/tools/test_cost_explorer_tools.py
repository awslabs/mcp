...
async def cost_explorer(ctx, operation, **kwargs):
    ...
    from awslabs.billing_cost_management_mcp_server.utilities.aws_service_base import handle_aws_error, format_response
    from awslabs.billing_cost_management_mcp_server.utilities.aws_service_base import create_aws_client
...
def mock_ce_client():
    ...
    mock_client.get_cost_categories.return_value = {
        "CostCategoryNames": ["Department", "Team"]
    }
...
async def test_cost_explorer_get_cost_and_usage(mock_context, mock_ce_client):
    ...
    assert "TimePeriod" in result["data"]["ResultsByTime"][0], "ResultsByTime entry should contain TimePeriod"
    assert "Total" in result["data"]["ResultsByTime"][0], "ResultsByTime entry should contain Total"
    assert "UnblendedCost" in result["data"]["ResultsByTime"][0]["Total"], "Total should contain UnblendedCost"
    assert result["data"]["ResultsByTime"][0]["Total"]["UnblendedCost"]["Amount"] == "100.0", "Unexpected cost amount"
    assert result["data"]["ResultsByTime"][0]["Total"]["UnblendedCost"]["Unit"] == "USD", "Unexpected cost unit"
...
async def test_cost_explorer_get_dimension_values(mock_context, mock_ce_client):
    ...
    assert len(result["data"]["DimensionValues"]) == 2, "DimensionValues should have exactly two entries"
    
    # Verify first dimension value
    assert "Value" in result["data"]["DimensionValues"][0], "First dimension should have a Value field"
    assert result["data"]["DimensionValues"][0]["Value"] == "AWS Lambda", "First dimension value should be 'AWS Lambda'"
    assert "Attributes" in result["data"]["DimensionValues"][0], "First dimension should have an Attributes field"
    
    # Verify second dimension value
    assert "Value" in result["data"]["DimensionValues"][1], "Second dimension should have a Value field"
    assert result["data"]["DimensionValues"][1]["Value"] == "Amazon S3", "Second dimension value should be 'Amazon S3'"
    assert "Attributes" in result["data"]["DimensionValues"][1], "Second dimension should have an Attributes field"
...
async def test_cost_explorer_get_cost_forecast(mock_context, mock_ce_client):
    ...
    assert isinstance(result["data"]["ForecastResultsByTime"], list), "ForecastResultsByTime should be a list"
    assert len(result["data"]["ForecastResultsByTime"]) >= 1, "ForecastResultsByTime should have at least one entry"
...
async def test_cost_explorer_missing_required_parameter(mock_context, mock_ce_client):
    ...
    assert "metric is required" in result["message"], "Error message should indicate missing metric parameter"
    assert "data" not in result or not result["data"], "Error response should not contain meaningful data"
...
async def test_cost_explorer_unknown_operation(mock_context, mock_ce_client):
    ...
    assert "message" in result, "Error response should contain a message field"
    assert isinstance(result["message"], str), "Error message should be a string"
    assert "Unknown operation" in result["message"], "Error message should indicate unknown operation"
    assert "unknownOperation" in result["message"], "Error message should mention the specific unknown operation"
    assert "Supported operations" in result["message"], "Error message should mention supported operations"
    assert "data" not in result or not result["data"], "Error response should not contain meaningful data"
...
async def test_cost_explorer_handle_exception(mock_context, mock_ce_client):
    ...
    assert "data" not in result or not result["data"], "Error response should not contain meaningful data"
    assert "Test error" in str(mock_context.error.call_args), "Original exception message should be logged"


def test_cost_explorer_server_initialization():
    """Test that the cost_explorer_server is properly initialized."""
    # Verify the server name
    assert cost_explorer_server.name == "cost-explorer-tools", "Server name should be 'cost-explorer-tools'"
    
    # Verify the server instructions
    assert isinstance(cost_explorer_server.instructions, str), "Server instructions should be a string"
    ...
    assert ce_tool.name == "cost_explorer", "The imported cost_explorer tool should have the right name"
    ...
    assert hasattr(cost_explorer_server, "instructions"), "Server should have 'instructions' property"