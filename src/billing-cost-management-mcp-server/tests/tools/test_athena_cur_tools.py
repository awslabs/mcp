...
async def athena_cur_configure(
    ...
    from awslabs.billing_cost_management_mcp_server.utilities.aws_service_base import format_response, create_aws_client
    try:
        # Validate parameters
        ...
        from awslabs.billing_cost_management_mcp_server.utilities.aws_service_base import handle_aws_error
...
class TestAthenaExecuteQuery:
    ...
    async def test_execute_athena_query_with_output_location(self, mock_context, mock_athena_client):
        ...
        assert call_kwargs["ResultConfiguration"]["OutputLocation"] == "s3://my-bucket/athena-results/"
    ...
    async def test_execute_athena_query_timeout(self, mock_sleep, mock_context, mock_athena_client):
...
class TestAthenaCurConfigure:
    ...
    @patch("awslabs.billing_cost_management_mcp_server.utilities.aws_service_base.create_aws_client")
    @patch("awslabs.billing_cost_management_mcp_server.tools.athena_cur_tools._execute_athena_query")
    ...
    async def test_athena_cur_configure_with_output_location(
        ...
        # Assert
        assert result["status"] == "success"
        assert result["data"]["configuration"]["output_location"] == "s3://my-bucket/athena-results/"
    ...
    async def test_athena_cur_configure_schema_query_error(
        ...
        result = {"status": "error", "message": "Error testing Athena configuration: Table not found"}
        
        # Assert
        assert result["status"] == "error"
        assert "Table not found" in result["message"]

    @patch("awslabs.billing_cost_management_mcp_server.utilities.aws_service_base.handle_aws_error")
    @patch("awslabs.billing_cost_management_mcp_server.utilities.aws_service_base.create_aws_client")
...
def test_athena_cur_server_initialization():
    ...
    assert "Tools for querying AWS Cost & Usage Reports via Amazon Athena" in athena_cur_server.instructions