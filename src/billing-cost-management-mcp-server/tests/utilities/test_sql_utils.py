...
class TestShouldConvertToSql:
    ...
    def test_should_convert_with_force_enabled(self, mock_getenv):
        ...
        mock_getenv.side_effect = lambda key, default=None: 'true' if key == 'MCP_FORCE_SQL' else default
        ...
        with patch.dict('sys.modules'):
            ...
            from awslabs.billing_cost_management_mcp_server.utilities.sql_utils import should_convert_to_sql, FORCE_SQL_CONVERSION
...
class TestGetSessionDbPath:
    ...
    def test_get_session_db_path(self, mock_register, mock_makedirs, mock_abspath, mock_dirname, mock_uuid):
...
class TestCreateTable:
    ...
    def test_create_table_standard(self):
        ...
        assert sql_statement == "CREATE TABLE test_table (id INTEGER PRIMARY KEY, name TEXT, value REAL)"
    ...
    def test_create_table_empty_schema(self):
        ...
        mock_cursor.execute.assert_called_once()
        sql_statement = mock_cursor.execute.call_args[0][0]
        assert sql_statement == f"CREATE TABLE {table_name} ()"  # Empty column list is allowed in the implementation
...
class TestExecuteQuery:
    ...
    def test_execute_query_with_parameters(self):
        ...
        columns, rows = execute_query(mock_cursor, query)  # But this function doesn't handle params
...
class TestConvertApiResponseToTable:
    ...
    async def test_convert_api_response_error(self, mock_json_dumps, mock_context):
        ...
        # Sample API response
        response = {"ResultsByTime": [{"TimePeriod": {"Start": "2023-01-01", "End": "2023-01-31"}}]}
