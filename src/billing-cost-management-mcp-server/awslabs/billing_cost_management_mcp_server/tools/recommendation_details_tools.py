...
async def get_recommendation_details(ctx: Context, recommendation_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        return await handle_aws_error(ctx, e, "get_recommendation_details", "Cost Optimization Hub")
...
async def process_recommendation(ctx: Context, recommendation: Dict[str, Any]) -> Dict[str, Any]:
    ...
    template_content = get_template_for_recommendation(recommendation, result["additional_details"])
    if template_content:
...
async def get_compute_optimizer_data(ctx: Context, recommendation: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        if resource_type == RESOURCE_TYPE_ECS_SERVICE:
            response = compute_optimizer.get_ecs_service_recommendations(serviceArns=[resource_arn])
            return response
