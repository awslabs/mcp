...
@patch('awslabs.aws_network_mcp_server.tools.cloud_wan.get_cloudwan_attachment_details.get_aws_client')
async def test_vpc_attachment_success(mock_get_client):
    ...
    assert result['vpc_specific']['subnet_arns'] == ['arn:aws:ec2:us-east-1:123456789012:subnet/subnet-123']
...
async def test_direct_connect_gateway_attachment_success(mock_get_client):
    ...
    assert result['direct_connect_specific']['direct_connect_gateway_arn'] == 'arn:aws:directconnect:us-east-1:123456789012:dx-gateway/dx-gw-123'
...
async def test_site_to_site_vpn_attachment_success(mock_get_client):
    ...
    assert result['vpn_specific']['vpn_connection_arn'] == 'arn:aws:ec2:us-east-1:123456789012:vpn-connection/vpn-123'
...
async def test_transit_gateway_route_table_attachment_success(mock_get_client):
    ...
    assert result['transit_gateway_specific']['transit_gateway_route_table_arn'] == 'arn:aws:ec2:us-east-1:123456789012:transit-gateway-route-table/tgw-rtb-123'
...
async def test_attachment_not_found(mock_get_client):
    ...
    assert 'Attachment invalid-attachment not found or unsupported attachment type' in str(exc_info.value)
