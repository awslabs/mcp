...
class TestQueryKnowledgeBasesTool:
    ...
    async def test_query_knowledge_bases_tool(self, mock_query_knowledge_base):
        ...
        mock_query_knowledge_base.return_value = json.dumps({
            ...
            "location": {
                "s3Location": {
                    "uri": "s3://test-bucket/test-document.txt"
                }
            },
            "score": 0.95
        })
...
class TestMain:
    ...
    def test_main_default(self, mock_mcp, mock_argument_parser):
        ...
        mock_argument_parser.assert_called_once_with(description="A Model Context Protocol (MCP) server")
        mock_parser.add_argument.assert_any_call("--sse", action="store_true", help="Use SSE transport")
        mock_parser.add_argument.assert_any_call("--port", type=int, default=8888, help="Port to run the server on")
...
class TestServerIntegration:
    ...
    @patch("awslabs.bedrock_kb_retrieval_mcp_server.server.discover_knowledge_bases")
    @patch("awslabs.bedrock_kb_retrieval_mcp_server.server.query_knowledge_base")
    async def test_server_integration(self, mock_query_knowledge_base, mock_discover_knowledge_bases):
