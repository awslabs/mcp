...
class TestQueryKnowledgeBase:
    ...
    async def test_query_knowledge_base_default(self, mock_bedrock_agent_runtime_client):
        ...
        assert documents[0]["location"]["s3Location"]["uri"] == "s3://test-bucket/test-document.txt"
        ...
        assert documents[1]["location"]["s3Location"]["uri"] == "s3://test-bucket/another-document.txt"
    ...
    @pytest.mark.asyncio
    async def test_query_knowledge_base_with_unsupported_region(self, mock_bedrock_agent_runtime_client):
    ...
    async def test_query_knowledge_base_with_image_content(self, mock_bedrock_agent_runtime_client):
        ...
        mock_bedrock_agent_runtime_client.retrieve.return_value = {
            "retrievalResults": [
                ...
                {
                    "content": {
                        "text": "This is a text document content.",
                        "type": "TEXT"
                    },
                    "location": {
                        "s3Location": {
                            "uri": "s3://test-bucket/document.txt"
                        }
                    },
                    "score": 0.85
                }
