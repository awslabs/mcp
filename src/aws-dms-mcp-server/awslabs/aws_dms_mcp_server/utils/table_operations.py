...
class TableOperations:
    ...
    def get_table_statistics(
        ...
        completions = [s.get("completion_percent", 0) for s in formatted_stats if s.get("completion_percent") is not None]
        if completions:
    ...
    def reload_tables(
        ...
        if not tables or len(tables) == 0:
            raise DMSInvalidParameterException(
                message="Tables list cannot be empty",
        ...
        for idx, table in enumerate(tables):
            if "SchemaName" not in table:
                raise DMSInvalidParameterException(
                    message=f"Table {idx} missing 'SchemaName'",
                    details={"table_index": idx}
                )
            if "TableName" not in table:
                raise DMSInvalidParameterException(
                    message=f"Table {idx} missing 'TableName'",
                    details={"table_index": idx}
                )
