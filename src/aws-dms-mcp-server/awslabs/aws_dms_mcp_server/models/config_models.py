...
class ReplicationInstanceConfig(BaseModel):
    ...
    def validate_instance_class(cls, v: str) -> str:
        ...
        valid_classes = [
            "dms.t2.micro", "dms.t2.small", "dms.t2.medium",
            "dms.t3.micro", "dms.t3.small", "dms.t3.medium", "dms.t3.large",
            "dms.c5.large", "dms.c5.xlarge", "dms.c5.2xlarge",
            "dms.r5.large", "dms.r5.xlarge", "dms.r5.2xlarge"
        ]
        if v not in valid_classes:
            raise ValueError(
                f"Invalid instance class: {v}. Must be one of {valid_classes}"
            )
        return v
...
class EndpointConfig(BaseModel):
    ...
    engine_name: DatabaseEngine = Field(
        ...,
