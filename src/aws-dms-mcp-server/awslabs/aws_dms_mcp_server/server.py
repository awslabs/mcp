...
def create_server(server_config: Optional[DMSServerConfig] = None) -> FastMCP:
    ...
    global task_manager, table_ops, connection_tester, assessment_manager, certificate_manager, subnet_group_manager, event_manager, maintenance_manager, serverless_replication_manager, serverless_manager, metadata_model_manager, fleet_advisor_manager, recommendation_manager
...
def modify_replication_instance(
    ...
    if config.read_only_mode:
        return ResponseFormatter.format_error(DMSMCPException("modify_replication_instance not available in read-only mode"))
    
    try:
        params = {"ReplicationInstanceArn": replication_instance_arn, "ApplyImmediately": apply_immediately, "AllowMajorVersionUpgrade": allow_major_version_upgrade}
        if allocated_storage is not None: params["AllocatedStorage"] = allocated_storage
        if replication_instance_class: params["ReplicationInstanceClass"] = replication_instance_class
        if vpc_security_group_ids: params["VpcSecurityGroupIds"] = vpc_security_group_ids
        if preferred_maintenance_window: params["PreferredMaintenanceWindow"] = preferred_maintenance_window
        if multi_az is not None: params["MultiAZ"] = multi_az
        if engine_version: params["EngineVersion"] = engine_version
        if auto_minor_version_upgrade is not None: params["AutoMinorVersionUpgrade"] = auto_minor_version_upgrade
        if replication_instance_identifier: params["ReplicationInstanceIdentifier"] = replication_instance_identifier
...
def delete_replication_instance(
    ...
    if config.read_only_mode:
        return ResponseFormatter.format_error(DMSMCPException("delete_replication_instance not available in read-only mode"))
...
def reboot_replication_instance(
    ...
    if config.read_only_mode:
        return ResponseFormatter.format_error(DMSMCPException("reboot_replication_instance not available in read-only mode"))
    
    try:
        result = instance_manager.reboot_instance(instance_arn=replication_instance_arn, force_failover=force_failover)
        return result
    except DMSMCPException as e:
...
def describe_replication_instance_task_logs(
    ...
    Raises:
        DMSResourceNotFoundException: Instance not found
    """
    logger.info("describe_replication_instance_task_logs called", instance_arn=replication_instance_arn)
    
    try:
        result = instance_manager.get_task_logs(instance_arn=replication_instance_arn, max_results=max_results, marker=marker)
        return result
    except DMSMCPException as e:
...
def move_replication_task(
    ...
    if config.read_only_mode:
        return ResponseFormatter.format_error(DMSMCPException("move_replication_task not available in read-only mode"))
    
    try:
        result = task_manager.move_task(task_arn=replication_task_arn, target_instance_arn=target_replication_instance_arn)
        return result
    except DMSMCPException as e:
...
def modify_endpoint(
    ...
    if config.read_only_mode:
        return ResponseFormatter.format_error(DMSMCPException("modify_endpoint not available in read-only mode"))
    
    try:
        params = {"EndpointArn": endpoint_arn}
        if endpoint_identifier: params["EndpointIdentifier"] = endpoint_identifier
        if endpoint_type: params["EndpointType"] = endpoint_type
        if engine_name: params["EngineName"] = engine_name
        if username: params["Username"] = username
        if password: params["Password"] = password
        if server_name: params["ServerName"] = server_name
        if port is not None: params["Port"] = port
        if database_name: params["DatabaseName"] = database_name
        if extra_connection_attributes: params["ExtraConnectionAttributes"] = extra_connection_attributes
        if certificate_arn: params["CertificateArn"] = certificate_arn
        if ssl_mode: params["SslMode"] = ssl_mode
        if secrets_manager_secret_id: params["SecretsManagerSecretId"] = secrets_manager_secret_id
...
def describe_endpoint_settings(
    ...
    try:
        result = endpoint_manager.get_endpoint_settings(engine_name=engine_name, max_results=max_results, marker=marker)
        return result
    except DMSMCPException as e:
...
def describe_endpoint_types(
    ...
    try:
        result = endpoint_manager.list_endpoint_types(filters=filters, max_results=max_results, marker=marker)
        return result
    except DMSMCPException as e:
...
def describe_engine_versions(
    ...
    try:
        result = endpoint_manager.list_engine_versions(engine_name=engine_name, max_results=max_results, marker=marker)
        return result
    except DMSMCPException as e:
...
def refresh_schemas(
    ...
    if config.read_only_mode:
        return ResponseFormatter.format_error(DMSMCPException("refresh_schemas not available in read-only mode"))
    
    try:
        result = endpoint_manager.refresh_schemas(endpoint_arn=endpoint_arn, instance_arn=replication_instance_arn)
        return result
    except DMSMCPException as e:
...
def describe_schemas(
    ...
    try:
        result = endpoint_manager.list_schemas(endpoint_arn=endpoint_arn, max_results=max_results, marker=marker)
        return result
    except DMSMCPException as e:
...
def modify_replication_task(
    ...
    if config.read_only_mode:
        return ResponseFormatter.format_error(DMSMCPException("modify_replication_task not available in read-only mode"))
    
    try:
        params = {"ReplicationTaskArn": replication_task_arn}
        if replication_task_identifier: params["ReplicationTaskIdentifier"] = replication_task_identifier
        if migration_type: params["MigrationType"] = migration_type
        if table_mappings: params["TableMappings"] = table_mappings
        if replication_task_settings: params["ReplicationTaskSettings"] = replication_task_settings
        if cdc_start_position: params["CdcStartPosition"] = cdc_start_position
        if cdc_start_time: params["CdcStartTime"] = cdc_start_time
        if cdc_stop_position: params["CdcStopPosition"] = cdc_stop_position
        if task_data: params["TaskData"] = task_data
...
def delete_replication_task(
    ...
    if config.read_only_mode:
        return ResponseFormatter.format_error(DMSMCPException("delete_replication_task not available in read-only mode"))
...
def reload_tables(
    ...
    if config.read_only_mode:
        return ResponseFormatter.format_error(DMSMCPException("reload_tables not available in read-only mode"))
    
    try:
        tables = [{"SchemaName": t.get("schema_name") or t.get("SchemaName"), "TableName": t.get("table_name") or t.get("TableName")} for t in tables_to_reload]
        result = table_ops.reload_serverless_tables(config_arn=replication_config_arn, tables=tables, reload_option=reload_option)
        return result
    except DMSMCPException as e:
...
def delete_endpoint(
    ...
    if config.read_only_mode:
        return ResponseFormatter.format_error(DMSMCPException("delete_endpoint not available in read-only mode"))
...
def delete_connection(
    ...
    Raises:
        ...
        DMSReadOnlyModeException: Read-only mode enabled
    """
    logger.info("delete_connection called", endpoint=endpoint_arn, instance=replication_instance_arn)
    
    if config.read_only_mode:
        return ResponseFormatter.format_error(DMSMCPException("delete_connection not available in read-only mode"))
...
def start_replication_task_assessment(
    ...
    if config.read_only_mode:
        return ResponseFormatter.format_error(DMSMCPException("start_replication_task_assessment not available in read-only mode"))
...
def start_replication_task_assessment_run(
    ...
    if config.read_only_mode:
        return ResponseFormatter.format_error(DMSMCPException("start_replication_task_assessment_run not available in read-only mode"))
...
def cancel_replication_task_assessment_run(
    ...
    Raises:
        ...
        DMSReadOnlyModeException: Read-only mode enabled
    """
    logger.info("cancel_replication_task_assessment_run called", run_arn=replication_task_assessment_run_arn)
    
    if config.read_only_mode:
        return ResponseFormatter.format_error(DMSMCPException("cancel_replication_task_assessment_run not available in read-only mode"))
    
    try:
        result = assessment_manager.cancel_assessment_run(assessment_run_arn=replication_task_assessment_run_arn)
        return result
    except DMSMCPException as e:
...
def delete_replication_task_assessment_run(
    ...
    Raises:
        ...
        DMSReadOnlyModeException: Read-only mode enabled
    """
    logger.info("delete_replication_task_assessment_run called", run_arn=replication_task_assessment_run_arn)
    
    if config.read_only_mode:
        return ResponseFormatter.format_error(DMSMCPException("delete_replication_task_assessment_run not available in read-only mode"))
    
    try:
        result = assessment_manager.delete_assessment_run(assessment_run_arn=replication_task_assessment_run_arn)
        return result
    except DMSMCPException as e:
...
def describe_replication_task_assessment_results(
    ...
    Raises:
        DMSResourceNotFoundException: Task not found
    """
    logger.info("describe_replication_task_assessment_results called", task_arn=replication_task_arn)
    ...
    try:
        ...
        logger.error("Unexpected error in describe_replication_task_assessment_results", error=str(e))
        return ResponseFormatter.format_error(e)
...
def describe_replication_task_individual_assessments(
    ...
    try:
        ...
        logger.error("Unexpected error in describe_replication_task_individual_assessments", error=str(e))
        return ResponseFormatter.format_error(e)
...
def describe_applicable_individual_assessments(
    ...
    try:
        ...
        logger.error("Unexpected error in describe_applicable_individual_assessments", error=str(e))
        return ResponseFormatter.format_error(e)
...
def import_certificate(
    ...
    if config.read_only_mode:
        return ResponseFormatter.format_error(DMSMCPException("import_certificate not available in read-only mode"))
...
def create_replication_subnet_group(
    ...
    Raises:
        ...
        DMSReadOnlyModeException: Read-only mode enabled
    """
    logger.info("create_replication_subnet_group called", identifier=replication_subnet_group_identifier)
    
    if config.read_only_mode:
        return ResponseFormatter.format_error(DMSMCPException("create_replication_subnet_group not available in read-only mode"))
...
def modify_replication_subnet_group(
    ...
    Raises:
        ...
        DMSReadOnlyModeException: Read-only mode enabled
    """
    logger.info("modify_replication_subnet_group called", identifier=replication_subnet_group_identifier)
    
    if config.read_only_mode:
        return ResponseFormatter.format_error(DMSMCPException("modify_replication_subnet_group not available in read-only mode"))
...
def delete_replication_subnet_group(
    ...
    Raises:
        ...
        DMSReadOnlyModeException: Read-only mode enabled
    """
    logger.info("delete_replication_subnet_group called", identifier=replication_subnet_group_identifier)
    
    if config.read_only_mode:
        return ResponseFormatter.format_error(DMSMCPException("delete_replication_subnet_group not available in read-only mode"))
    
    try:
        result = subnet_group_manager.delete_subnet_group(identifier=replication_subnet_group_identifier)
        return result
    except DMSMCPException as e:
...
def create_event_subscription(
    ...
    if config.read_only_mode:
        return ResponseFormatter.format_error(DMSMCPException("create_event_subscription not available in read-only mode"))
...
def modify_event_subscription(
    ...
    if config.read_only_mode:
        return ResponseFormatter.format_error(DMSMCPException("modify_event_subscription not available in read-only mode"))
...
def delete_event_subscription(
    ...
    if config.read_only_mode:
        return ResponseFormatter.format_error(DMSMCPException("delete_event_subscription not available in read-only mode"))
...
def describe_event_categories(
    ...
    try:
        result = event_manager.list_event_categories(
            source_type=source_type,
            filters=filters
        )
        return result
...
def apply_pending_maintenance_action(
    ...
    if config.read_only_mode:
        return ResponseFormatter.format_error(DMSMCPException("apply_pending_maintenance_action not available in read-only mode"))
...
def create_replication_config(
    ...
    if config.read_only_mode:
        return ResponseFormatter.format_error(DMSMCPException("create_replication_config not available in read-only mode"))
...
def modify_replication_config(
    ...
    if config.read_only_mode:
        return ResponseFormatter.format_error(DMSMCPException("modify_replication_config not available in read-only mode"))
...
def delete_replication_config(
    ...
    if config.read_only_mode:
        return ResponseFormatter.format_error(DMSMCPException("delete_replication_config not available in read-only mode"))
    
    try:
        result = serverless_replication_manager.delete_replication_config(arn=replication_config_arn)
        return result
    except DMSMCPException as e:
...
def start_replication(
    ...
    if config.read_only_mode:
        return ResponseFormatter.format_error(DMSMCPException("start_replication not available in read-only mode"))
...
def stop_replication(
    ...
    if config.read_only_mode:
        return ResponseFormatter.format_error(DMSMCPException("stop_replication not available in read-only mode"))
...
def create_migration_project(
    ...
    if config.read_only_mode:
        return ResponseFormatter.format_error(DMSMCPException("create_migration_project not available in read-only mode"))
...
def modify_migration_project(
    ...
    if config.read_only_mode:
        return ResponseFormatter.format_error(DMSMCPException("modify_migration_project not available in read-only mode"))
...
def delete_migration_project(
    ...
    if config.read_only_mode:
        return ResponseFormatter.format_error(DMSMCPException("delete_migration_project not available in read-only mode"))
...
def create_data_provider(
    ...
    if config.read_only_mode:
        return ResponseFormatter.format_error(DMSMCPException("create_data_provider not available in read-only mode"))
...
def modify_data_provider(
    ...
    if config.read_only_mode:
        return ResponseFormatter.format_error(DMSMCPException("modify_data_provider not available in read-only mode"))
...
def delete_data_provider(
    ...
    if config.read_only_mode:
        return ResponseFormatter.format_error(DMSMCPException("delete_data_provider not available in read-only mode"))
...
def create_instance_profile(
    ...
    if config.read_only_mode:
        return ResponseFormatter.format_error(DMSMCPException("create_instance_profile not available in read-only mode"))
...
def modify_instance_profile(
    ...
    if config.read_only_mode:
        return ResponseFormatter.format_error(DMSMCPException("modify_instance_profile not available in read-only mode"))
...
@mcp.tool()
def modify_conversion_configuration(migration_project_arn: str, conversion_configuration: Dict[str, Any]) -> Dict[str, Any]:
    """Modify conversion configuration."""
    if config.read_only_mode:
        return ResponseFormatter.format_error(DMSMCPException("modify_conversion_configuration not available in read-only mode"))
    try:
        return metadata_model_manager.modify_conversion_configuration(arn=migration_project_arn, configuration=conversion_configuration)
    except Exception as e:
        logger.error("Failed to modify conversion configuration", error=str(e))
        return ResponseFormatter.format_error(e)

@mcp.tool()
def describe_extension_pack_associations(migration_project_arn: str, filters: Optional[List[Dict[str, Any]]] = None, max_results: int = 100, marker: Optional[str] = None) -> Dict[str, Any]:
    """List extension pack associations."""
    try:
        return metadata_model_manager.describe_extension_pack_associations(arn=migration_project_arn, filters=filters, max_results=max_results, marker=marker)
    except Exception as e:
...
def start_extension_pack_association(migration_project_arn: str) -> Dict[str, Any]:
    ...
    if config.read_only_mode:
        return ResponseFormatter.format_error(DMSMCPException("start_extension_pack_association not available in read-only mode"))
    try:
...
@mcp.tool()
def describe_metadata_model_assessments(migration_project_arn: str, filters: Optional[List[Dict[str, Any]]] = None, max_results: int = 100, marker: Optional[str] = None) -> Dict[str, Any]:
    """List metadata model assessments."""
    try:
        return metadata_model_manager.describe_metadata_model_assessments(arn=migration_project_arn, filters=filters, max_results=max_results, marker=marker)
    except Exception as e:
        logger.error("Failed to describe metadata model assessments", error=str(e))
        return ResponseFormatter.format_error(e)

@mcp.tool()
def start_metadata_model_assessment(migration_project_arn: str, selection_rules: str) -> Dict[str, Any]:
    """Start metadata model assessment."""
    if config.read_only_mode:
        return ResponseFormatter.format_error(DMSMCPException("start_metadata_model_assessment not available in read-only mode"))
    try:
        return metadata_model_manager.start_metadata_model_assessment(arn=migration_project_arn, selection_rules=selection_rules)
    except Exception as e:
        logger.error("Failed to start metadata model assessment", error=str(e))
        return ResponseFormatter.format_error(e)

@mcp.tool()
def describe_metadata_model_conversions(migration_project_arn: str, filters: Optional[List[Dict[str, Any]]] = None, max_results: int = 100, marker: Optional[str] = None) -> Dict[str, Any]:
    """List metadata model conversions."""
    try:
        return metadata_model_manager.describe_metadata_model_conversions(arn=migration_project_arn, filters=filters, max_results=max_results, marker=marker)
    except Exception as e:
        logger.error("Failed to describe metadata model conversions", error=str(e))
        return ResponseFormatter.format_error(e)

@mcp.tool()
def start_metadata_model_conversion(migration_project_arn: str, selection_rules: str) -> Dict[str, Any]:
    """Start metadata model conversion."""
    if config.read_only_mode:
        return ResponseFormatter.format_error(DMSMCPException("start_metadata_model_conversion not available in read-only mode"))
    try:
        return metadata_model_manager.start_metadata_model_conversion(arn=migration_project_arn, selection_rules=selection_rules)
    except Exception as e:
        logger.error("Failed to start metadata model conversion", error=str(e))
        return ResponseFormatter.format_error(e)

@mcp.tool()
def describe_metadata_model_exports_as_script(migration_project_arn: str, filters: Optional[List[Dict[str, Any]]] = None, max_results: int = 100, marker: Optional[str] = None) -> Dict[str, Any]:
    """List metadata model script exports."""
    try:
        return metadata_model_manager.describe_metadata_model_exports_as_script(arn=migration_project_arn, filters=filters, max_results=max_results, marker=marker)
    except Exception as e:
        logger.error("Failed to describe metadata model exports as script", error=str(e))
        return ResponseFormatter.format_error(e)

@mcp.tool()
def start_metadata_model_export_as_script(migration_project_arn: str, selection_rules: str, origin: str, file_name: Optional[str] = None) -> Dict[str, Any]:
    """Start metadata model export as script."""
    if config.read_only_mode:
        return ResponseFormatter.format_error(DMSMCPException("start_metadata_model_export_as_script not available in read-only mode"))
    try:
        return metadata_model_manager.start_metadata_model_export_as_script(arn=migration_project_arn, selection_rules=selection_rules, origin=origin, file_name=file_name)
    except Exception as e:
        logger.error("Failed to start metadata model export as script", error=str(e))
        return ResponseFormatter.format_error(e)

@mcp.tool()
def describe_metadata_model_exports_to_target(migration_project_arn: str, filters: Optional[List[Dict[str, Any]]] = None, max_results: int = 100, marker: Optional[str] = None) -> Dict[str, Any]:
    """List metadata model target exports."""
    try:
        return metadata_model_manager.describe_metadata_model_exports_to_target(arn=migration_project_arn, filters=filters, max_results=max_results, marker=marker)
    except Exception as e:
        logger.error("Failed to describe metadata model exports to target", error=str(e))
        return ResponseFormatter.format_error(e)

@mcp.tool()
def start_metadata_model_export_to_target(migration_project_arn: str, selection_rules: str, overwrite_extension_pack: Optional[bool] = None) -> Dict[str, Any]:
    """Start metadata model export to target."""
    if config.read_only_mode:
        return ResponseFormatter.format_error(DMSMCPException("start_metadata_model_export_to_target not available in read-only mode"))
    try:
        return metadata_model_manager.start_metadata_model_export_to_target(arn=migration_project_arn, selection_rules=selection_rules, overwrite_extension_pack=overwrite_extension_pack)
    except Exception as e:
        logger.error("Failed to start metadata model export to target", error=str(e))
        return ResponseFormatter.format_error(e)

@mcp.tool()
def describe_metadata_model_imports(migration_project_arn: str, filters: Optional[List[Dict[str, Any]]] = None, max_results: int = 100, marker: Optional[str] = None) -> Dict[str, Any]:
    """List metadata model imports."""
    try:
        return metadata_model_manager.describe_metadata_model_imports(arn=migration_project_arn, filters=filters, max_results=max_results, marker=marker)
    except Exception as e:
        logger.error("Failed to describe metadata model imports", error=str(e))
        return ResponseFormatter.format_error(e)

@mcp.tool()
def start_metadata_model_import(migration_project_arn: str, selection_rules: str, origin: str) -> Dict[str, Any]:
    """Start metadata model import."""
    if config.read_only_mode:
        return ResponseFormatter.format_error(DMSMCPException("start_metadata_model_import not available in read-only mode"))
    try:
        return metadata_model_manager.start_metadata_model_import(arn=migration_project_arn, selection_rules=selection_rules, origin=origin)
    except Exception as e:
        logger.error("Failed to start metadata model import", error=str(e))
        return ResponseFormatter.format_error(e)

@mcp.tool()
def export_metadata_model_assessment(migration_project_arn: str, selection_rules: str, file_name: Optional[str] = None, assessment_report_types: Optional[List[str]] = None) -> Dict[str, Any]:
    """Export metadata model assessment."""
    if config.read_only_mode:
        return ResponseFormatter.format_error(DMSMCPException("export_metadata_model_assessment not available in read-only mode"))
    try:
        return metadata_model_manager.export_metadata_model_assessment(arn=migration_project_arn, selection_rules=selection_rules, file_name=file_name, assessment_report_types=assessment_report_types)
    except Exception as e:
...
@mcp.tool()
def create_fleet_advisor_collector(collector_name: str, description: str, service_access_role_arn: str, s3_bucket_name: str) -> Dict[str, Any]:
    """Create Fleet Advisor collector for database discovery."""
    if config.read_only_mode:
        return ResponseFormatter.format_error(DMSMCPException("create_fleet_advisor_collector not available in read-only mode"))
    try:
        return fleet_advisor_manager.create_collector(name=collector_name, description=description, service_access_role_arn=service_access_role_arn, s3_bucket_name=s3_bucket_name)
    except Exception as e:
...
def delete_fleet_advisor_collector(collector_referenced_id: str) -> Dict[str, Any]:
    ...
    if config.read_only_mode:
        return ResponseFormatter.format_error(DMSMCPException("delete_fleet_advisor_collector not available in read-only mode"))
    try:
...
@mcp.tool()
def describe_fleet_advisor_collectors(filters: Optional[List[Dict[str, Any]]] = None, max_results: int = 100, marker: Optional[str] = None) -> Dict[str, Any]:
    """List Fleet Advisor collectors."""
    try:
        return fleet_advisor_manager.list_collectors(filters=filters, max_results=max_results, marker=marker)
    except Exception as e:
...
def delete_fleet_advisor_databases(database_ids: List[str]) -> Dict[str, Any]:
    ...
    if config.read_only_mode:
        return ResponseFormatter.format_error(DMSMCPException("delete_fleet_advisor_databases not available in read-only mode"))
    try:
...
@mcp.tool()
def describe_fleet_advisor_databases(filters: Optional[List[Dict[str, Any]]] = None, max_results: int = 100, marker: Optional[str] = None) -> Dict[str, Any]:
    """List Fleet Advisor databases."""
    try:
        return fleet_advisor_manager.list_databases(filters=filters, max_results=max_results, marker=marker)
    except Exception as e:
        logger.error("Failed to describe Fleet Advisor databases", error=str(e))
        return ResponseFormatter.format_error(e)

@mcp.tool()
def describe_fleet_advisor_lsa_analysis(max_results: int = 100, marker: Optional[str] = None) -> Dict[str, Any]:
...
def run_fleet_advisor_lsa_analysis() -> Dict[str, Any]:
    ...
    if config.read_only_mode:
        return ResponseFormatter.format_error(DMSMCPException("run_fleet_advisor_lsa_analysis not available in read-only mode"))
    try:
...
@mcp.tool()
def describe_fleet_advisor_schema_object_summary(filters: Optional[List[Dict[str, Any]]] = None, max_results: int = 100, marker: Optional[str] = None) -> Dict[str, Any]:
    """Describe Fleet Advisor schema object summary."""
    try:
        return fleet_advisor_manager.describe_schema_object_summary(filters=filters, max_results=max_results, marker=marker)
    except Exception as e:
        logger.error("Failed to describe Fleet Advisor schema object summary", error=str(e))
        return ResponseFormatter.format_error(e)

@mcp.tool()
def describe_fleet_advisor_schemas(filters: Optional[List[Dict[str, Any]]] = None, max_results: int = 100, marker: Optional[str] = None) -> Dict[str, Any]:
    """List Fleet Advisor schemas."""
    try:
        return fleet_advisor_manager.list_schemas(filters=filters, max_results=max_results, marker=marker)
    except Exception as e:
...
@mcp.tool()
def describe_recommendations(filters: Optional[List[Dict[str, Any]]] = None, max_results: int = 100, marker: Optional[str] = None) -> Dict[str, Any]:
    """List migration recommendations."""
    try:
        return recommendation_manager.list_recommendations(filters=filters, max_results=max_results, marker=marker)
    except Exception as e:
        logger.error("Failed to describe recommendations", error=str(e))
        return ResponseFormatter.format_error(e)

@mcp.tool()
def describe_recommendation_limitations(filters: Optional[List[Dict[str, Any]]] = None, max_results: int = 100, marker: Optional[str] = None) -> Dict[str, Any]:
    """List recommendation limitations."""
    try:
        return recommendation_manager.list_recommendation_limitations(filters=filters, max_results=max_results, marker=marker)
    except Exception as e:
...
def start_recommendations(database_id: str, settings: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if config.read_only_mode:
        return ResponseFormatter.format_error(DMSMCPException("start_recommendations not available in read-only mode"))
    try:
        return recommendation_manager.start_recommendations(database_id=database_id, settings=settings)
    except Exception as e:
...
def batch_start_recommendations(data: Optional[List[Dict[str, Any]]] = None) -> Dict[str, Any]:
    ...
    if config.read_only_mode:
        return ResponseFormatter.format_error(DMSMCPException("batch_start_recommendations not available in read-only mode"))
    try:
...
def delete_instance_profile(
    ...
    if config.read_only_mode:
        return ResponseFormatter.format_error(DMSMCPException("delete_instance_profile not available in read-only mode"))
...
def create_data_migration(
    ...
    if config.read_only_mode:
        return ResponseFormatter.format_error(DMSMCPException("create_data_migration not available in read-only mode"))
...
def modify_data_migration(
    ...
    if config.read_only_mode:
        return ResponseFormatter.format_error(DMSMCPException("modify_data_migration not available in read-only mode"))
...
def delete_data_migration(
    ...
    if config.read_only_mode:
        return ResponseFormatter.format_error(DMSMCPException("delete_data_migration not available in read-only mode"))
...
def start_data_migration(
    ...
    if config.read_only_mode:
        return ResponseFormatter.format_error(DMSMCPException("start_data_migration not available in read-only mode"))
    
    try:
        result = serverless_manager.start_data_migration(arn=data_migration_arn, start_type=start_type)
        return result
    except DMSMCPException as e:
...
def stop_data_migration(
    ...
    if config.read_only_mode:
        return ResponseFormatter.format_error(DMSMCPException("stop_data_migration not available in read-only mode"))
...
def add_tags_to_resource(
    ...
    if config.read_only_mode:
        return ResponseFormatter.format_error(DMSMCPException("add_tags_to_resource not available in read-only mode"))
    
    try:
        result = maintenance_manager.add_tags(
...
def remove_tags_from_resource(
    ...
    if config.read_only_mode:
        return ResponseFormatter.format_error(DMSMCPException("remove_tags_from_resource not available in read-only mode"))
    
    try:
        result = maintenance_manager.remove_tags(
            resource_arn=resource_arn,
            tag_keys=tag_keys
        )
        return result
...
def update_subscriptions_to_event_bridge(
    ...
    if config.read_only_mode:
        return ResponseFormatter.format_error(DMSMCPException("update_subscriptions_to_event_bridge not available in read-only mode"))
...
def delete_certificate(
    ...
    if config.read_only_mode:
        return ResponseFormatter.format_error(DMSMCPException("delete_certificate not available in read-only mode"))
