...
import pytest
import json
from cfnmcpprototype.validator import validate_template
from cfnmcpprototype.compliance_checker import check_compliance
    ...
    def test_oversized_template_validation(self):
        ...
        for i in range(10000):
            large_template["Resources"][f"Bucket{i}"] = {
                "Type": "AWS::S3::Bucket",
                "Properties": {
                    "BucketName": f"test-bucket-{i}" + "x" * 100
                }
            }
...
"""
    ...
    def test_non_string_template(self):
        ...
        assert isinstance(result, dict)
        assert result.get("valid") is False or result.get("validation_results", {}).get("is_valid") is False
...
class TestInvalidParameters:
    ...
    def test_invalid_rules_file_path(self):
        ...
        assert isinstance(result, dict)
        assert "error" in result or result.get("compliance_results", {}).get("overall_status") == "ERROR"
