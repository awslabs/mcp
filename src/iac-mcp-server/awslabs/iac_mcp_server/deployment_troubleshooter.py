...
class DeploymentTroubleshooter:
    ...
    def filter_cloudtrail_events(self, cloudtrail_events: List[Dict], root_cause_event: Dict) -> Dict[str, Any]:
        ...
        physical_resource_id = root_cause_event.get('PhysicalResourceId')
        resource_type = root_cause_event.get('ResourceType')
        
        for event in cloudtrail_events:
            cloudtrail_event_data = json.loads(event.get('CloudTrailEvent', '{}'))
            
            # Filter for CloudFormation-initiated events with errors
            has_error = cloudtrail_event_data.get('errorCode') or cloudtrail_event_data.get('errorMessage')
            is_cfn_event = cloudtrail_event_data.get('sourceIPAddress') == CLOUDTRAIL_SOURCE_IP_FOR_CLOUDFORMATION
    ...
    def filter_cloudformation_stack_events(self, events: List[Dict]) -> Dict[str, Any]:
        ...
        for event in events:
            ...
            if 'FAILED' in status:
                if 'cancelled' in reason.lower() or 'resource creation cancelled' in reason.lower():
    ...
    def troubleshoot_stack_deployment(
        ...
        try:
            ...
            try:
                ...
                cfn_events = self.cfn_client.describe_stack_events(
            ...
            if include_cloudtrail:
                ...
                filtered_ct = cloudtrail_result.get('filtered_events', [])
                ct_url = cloudtrail_result.get('cloudtrail_url', 'N/A')
                
                # Check if events are missing due to indexing delay
                time_since_failure = (datetime.now(timezone.utc) - failure_timestamp).total_seconds()
                indexing_delay_threshold = 900  # 15 minutes
