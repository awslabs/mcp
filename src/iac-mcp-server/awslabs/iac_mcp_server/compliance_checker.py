...
def _extract_resource_info(node: dict, template_resources: dict) -> tuple[str, str]:
    ...
    for path, value in all_paths:
        if '/Resources/' in path:
            ...
            resource_type = value if path.endswith('/Type') else template_resources.get(resource_name, "Unknown")
...
def check_compliance(
    ...
    try:
        guard_result = guardpycfn.validate_with_guard(template_content, _RULES_CONTENT_CACHE, verbose=True)
            ...
            if 'RuleCheck' in container:
                ...
                if rule_check.get('status') == 'FAIL':
                    ...
                    if not found_specific:
                        ...
                        violations.append(create_violation(rule_name, resource_name, resource_type, f"Rule {rule_name} failed validation"))
        ...
        def add_violations_from_child(node, rule_name):
            ...
            for check_type in ['Unary', 'Comparison']:
                ...
                if check_data.get('status') == 'FAIL':
                    message = check_data.get('message', 'Rule violation detected' if check_type == 'Unary' else 'Rule comparison failed')
                    violations.append(create_violation(rule_name, resource_name, resource_type, message))
                    return True
            return False
        ...
        def get_resource_for_rule(rule_name, template_resources):
            ...
            if 'S3' in rule_name:
                ...
                sg_resources = [(name, rtype) for name, rtype in template_resources.items() if 'SecurityGroup' in rtype]
                if sg_resources:
                    return sg_resources[0]
            elif 'IAM' in rule_name:
                iam_resources = [(name, rtype) for name, rtype in template_resources.items() if 'IAM' in rtype]
                if iam_resources:
        ...
        def create_violation(rule_id, resource, resource_type, message):
            return {
                ...
                "message": message,
                "remediation": _REMEDIATION_CACHE.get(rule_id, "Review the resource configuration and ensure it meets the policy requirements")
            }
        
        process_node(guard_result.get('result', {}))
        
        error_count = len(violations)  # All violations are ERROR severity in current implementation
        overall_status = "COMPLIANT" if error_count == 0 else "VIOLATIONS_FOUND"
        
        message = ("Template is compliant with all rules." if overall_status == "COMPLIANT" 
