...
"""AWS helper for the ROSA MCP Server."""

import boto3
import os
from awslabs.rosa_mcp_server import __version__
...
class AwsHelper:
    ...
    def create_boto3_client(cls, service_name: str, region_name: Optional[str] = None) -> Any:
        ...
        The client is configured with a custom user agent suffix 'awslabs/mcp/rosa-mcp-server/{version}'
        to identify API calls made by the ROSA MCP Server. Clients are cached to improve performance
        and reduce resource usage.

        Args:
            service_name: The AWS service name (e.g., 'ec2', 's3', 'iam', 'sts')
        ...
        try:
            ...
            config = Config(user_agent_extra=f'awslabs/mcp/rosa-mcp-server/{__version__}')
