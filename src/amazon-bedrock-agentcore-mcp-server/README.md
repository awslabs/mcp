...
```bash
# Disable browser tools only
AGENTCORE_DISABLE_TOOLS=browser uvx awslabs.amazon-bedrock-agentcore-mcp-server@latest

# Enable only specific primitives
AGENTCORE_ENABLE_TOOLS=docs,runtime uvx awslabs.amazon-bedrock-agentcore-mcp-server@latest
```
