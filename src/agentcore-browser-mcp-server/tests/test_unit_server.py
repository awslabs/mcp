...
PATCH_BASE = 'awslabs.agentcore_browser_mcp_server.server'
...
class TestServerLifespan:
    ...
    async def test_lifespan_calls_cleanup_on_exit(self):
        ...
        with patch(f'{PATCH_BASE}.connection_manager') as mock_cm:
            ...
            from awslabs.agentcore_browser_mcp_server.server import server_lifespan
...
class TestToolRegistration:
    ...
    def test_mcp_instance_created(self):
        ...
        from awslabs.agentcore_browser_mcp_server.server import mcp

        assert mcp.name == 'awslabs.agentcore-browser-mcp-server'

    def test_mcp_has_lifespan(self):
        """FastMCP instance has lifespan set."""
        from awslabs.agentcore_browser_mcp_server.server import mcp, server_lifespan
...
class TestMain:
    ...
    def test_main_calls_mcp_run(self):
        ...
        with patch(f'{PATCH_BASE}.mcp') as mock_mcp:
            from awslabs.agentcore_browser_mcp_server.server import main
...
class TestCleanupStaleSessions:
    ...
    async def test_cleanup_prunes_stale_session(self):
        ...
        with (
            ...
            from awslabs.agentcore_browser_mcp_server.server import _cleanup_stale_sessions
...
class TestToolRegistrationError:
    ...
    def test_tool_registration_error_propagates(self):
        ...
        with patch(
            'awslabs.agentcore_browser_mcp_server.tools.session.BrowserSessionTools.__init__',
            side_effect=RuntimeError('Init failed'),
        ):
            import awslabs.agentcore_browser_mcp_server.server as srv_mod
