...
```bash
uvx awslabs.agentcore-browser-mcp-server@latest
```

Or from a local source checkout (for development):

```bash
uv run --directory /path/to/agentcore-browser-mcp-server \
...
```json
{
  "mcpServers": {
    "agentcore-browser-mcp-server": {
      "command": "uvx",
      "args": ["awslabs.agentcore-browser-mcp-server@latest"],
      "env": {
  ...
  "mcpServers": {
    "agentcore-browser-mcp-server": {
      ...
      "args": [
        ...
        "run",
        "--from",
        "awslabs.agentcore-browser-mcp-server@latest",
        "awslabs.agentcore-browser-mcp-server.exe"
      ],
      "env": {
        "AWS_REGION": "us-east-1",
        "AWS_PROFILE": "your-profile"
