...
class IAMHelper:
    ...
    def __init__(self, region_name=None):
        ...
        try:
            ...
            self.kafka_client = (
                ...
                if aws_access_key and aws_secret_key
                else boto3.client('kafka', region_name=region_name)
            )
    ...
    def add_msk_permissions(self, function_name: str, kafka_type: str = 'MSK') -> Dict:
        ...
        try:
            ...
            kafka_policy = {'Version': '2012-10-17', 'Statement': statements}

            policy_name = f'{kafka_type}-Lambda-Access'

            self.iam_client.put_role_policy(
                RoleName=role_name, PolicyName=policy_name, PolicyDocument=json.dumps(kafka_policy)
            )

            self.logger.info(f'Successfully added {kafka_type} permissions to role {role_name}')
            return {
                'status': 'success',
                'message': f'{kafka_type} permissions added to role {role_name}',
            }
    ...
    def get_secret_from_arn(
        self, secret_arn: str, prompt_if_failed: bool = True
    ) -> Optional[Dict]:
