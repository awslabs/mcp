...
class UpdateEventSourceMapping:
        ...
        try:
            ...
            if function_name and check_permissions:
                ...
                try:
                    ...
                    self.logger.info(f"Successfully verified/added MSK permissions for function: {function_name}")
                except Exception as e:
                    self.logger.warning(f"Failed to add MSK permissions: {str(e)}. Continuing with update.")
                ...
                try:
                    ...
                    if current_function:
                        self.logger.info(f"Checking IAM permissions for function: {current_function}")
                        self.iam_helper.add_msk_permissions(current_function)
                        self.logger.info(f"Successfully verified/added MSK permissions for function: {current_function}")
                except Exception as e:
                    self.logger.warning(f"Failed to add MSK permissions: {str(e)}. Continuing with update.")
    ...
    def update_esm_by_function_and_source(self,
        function_name: str,
    ...
    def _display_updated_mapping(self, uuid: str) -> Dict:
        ...
        try:
            ...
            formatted_mapping = {
                ...
                'LastModified': response.get('LastModified').strftime('%Y-%m-%d %H:%M:%S') if response.get('LastModified') else None
            }
            ...
            return {"Message": "Event Source Mapping was updated, but there was an error retrieving the updated details."}