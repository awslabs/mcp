...
class MSKOAuthTokenProvider(AbstractTokenProvider):
    """OAuth token provider for MSK authentication."""
...
class ESMTesting:
    """Event Source Mapping testing utilities."""

    def __init__(self, region=None):
        """Initialize the ESM testing tool."""
        self.region = region or os.environ.get('AWS_REGION')
            ...
            try:
                for i in range(message_count):
                    message = {
                        'id': f'test-{i + 1}',
                        'timestamp': time.time(),
                        'data': f'{test_type} test message {i + 1}',
                    }
    ...
    def run_end_to_end_test(
        self, esm_uuid, secret_arn=None, bootstrap_servers=None, kafka_topic=None
    ):
        """Run end-to-end test with 5 messages."""
        return self.run_load_test_with_config(
            esm_uuid, 5, 'End-to-End Test', secret_arn, bootstrap_servers, kafka_topic
        )

    def run_small_load_test(
        self, esm_uuid, secret_arn=None, bootstrap_servers=None, kafka_topic=None
    ):
        """Run small load test with 35 messages."""
        return self.run_load_test_with_config(
            esm_uuid, 35, 'Small Load Test', secret_arn, bootstrap_servers, kafka_topic
        )

    def run_medium_load_test(
        self, esm_uuid, secret_arn=None, bootstrap_servers=None, kafka_topic=None
    ):
        """Run medium load test with 75 messages."""
        return self.run_load_test_with_config(
            esm_uuid, 75, 'Medium Load Test', secret_arn, bootstrap_servers, kafka_topic
        )

    def run_large_load_test(
        self, esm_uuid, secret_arn=None, bootstrap_servers=None, kafka_topic=None
    ):
        """Run large load test with 150 messages."""
        return self.run_load_test_with_config(
            esm_uuid, 150, 'Large Load Test', secret_arn, bootstrap_servers, kafka_topic
        )
