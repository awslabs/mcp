...
| `AUTH_TYPE`                                                       | ❌ No                       | `"oauth"`                                                | Only used when `AWS_API_MCP_TRANSPORT` is set to `"streamable-http"`. Authentication type for the MCP server. When set to `"no-auth"`, disables authentication. When set to `"oauth"`, enables OAuth authentication and requires `AUTH_ISSUER` and `AUTH_JWKS_URI` to be configured.                                                                                                                                                                                                                                                                                                                                            |
