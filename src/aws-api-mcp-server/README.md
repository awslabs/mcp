...
| `AWS_API_MCP_DISABLE_LOCAL_FILE_ACCESS`                           | ❌ No                       | `"false"`                                                | When set to `true`, rejects AWS CLI commands with local file path arguments. This takes precedence over `AWS_API_MCP_ALLOW_UNRESTRICTED_LOCAL_FILE_ACCESS`. S3 URIs (`s3://...`) and stdout redirect (`-`) remain allowed. Custom operations that require local file access (e.g., `aws cloudformation package`, `aws ecs deploy`) will be blocked when this is enabled.                                                                      |
