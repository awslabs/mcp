...
| `AUTH_TYPE`                                                       | ❌ No                       | `"no-auth"`                                              | Authentication type for the MCP server. Defaults to `"no-auth"` (no authentication). When set to any value other than `"no-auth"`, enables JWT authentication and requires `AUTH_ISSUER` and `AUTH_JWKS_URI` to be configured. Only used when `AWS_API_MCP_TRANSPORT` is set to `"streamable-http"`.                                                                                                                                                                                                                                                                                                                    |
