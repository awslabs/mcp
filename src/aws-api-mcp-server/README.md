...
| `AUTH_TYPE`                                                       | ❌ No                       | `"oauth"`                                                | Authentication type for the MCP server. Defaults to `"oauth"` (OAuth authentication). When set to `"no-auth"`, disables authentication. When set to any value other than `"no-auth"`, enables Oauth authentication and requires `AUTH_ISSUER` and `AUTH_JWKS_URI` to be configured. Only used when `AWS_API_MCP_TRANSPORT` is set to `"streamable-http"`.                                                                                                                                                                                                                                                                                                                    |
| `AUTH_JWKS_URI`                                                   | ❌ No                       | -                                                        | JWKS (JSON Web Key Set) endpoint URL for JWT token validation. This should be a publicly accessible HTTPS URL that serves the JSON Web Key Set used to verify JWT signatures. Example: `"https://your-auth-provider.com/.well-known/jwks.json"`. Only used when `AWS_API_MCP_TRANSPORT` is set to `"streamable-http"`.                                                                                                                                                                                                                                                                                                     |
