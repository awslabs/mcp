...
| `AWS_API_MCP_REGIONAL_DATA_S3_URI`                                | ❌ No                       | -                                                        | S3 URI (`s3://bucket/prefix`) or S3 access-point ARN pointing to the root of a [Capability Insights for AWS](https://github.com/aws/capability-insights-for-aws/) data set. When set, the `get_regional_availability` and `list_available_services` tools are registered and can query regional availability data — including preview Regions — downloaded via the [data-layer onboarding](https://github.com/aws/capability-insights-for-aws/#data-layer-onboarding) process. The configured AWS credentials must have `s3:GetObject` (and optionally `s3:ListBucket`) access to this location. |
| `AWS_API_MCP_REGIONAL_DATA_CACHE_TTL`                             | ❌ No                       | `"300"`                                                  | Number of seconds to cache the regional availability data in memory before re-reading from S3. Only used when `AWS_API_MCP_REGIONAL_DATA_S3_URI` is set.                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
- `get_regional_availability`: Retrieves regional availability information for AWS services and features from a [Capability Insights for AWS](https://github.com/aws/capability-insights-for-aws/) data set stored in S3. This tool is only available when the `AWS_API_MCP_REGIONAL_DATA_S3_URI` environment variable is configured. Supports querying by service, feature, and region list — including preview Regions not in publicly available data.
- `list_available_services`: Lists all AWS services and features available in the configured Capability Insights data set. Use this to discover what services can be queried with `get_regional_availability`. Only available when `AWS_API_MCP_REGIONAL_DATA_S3_URI` is configured.
