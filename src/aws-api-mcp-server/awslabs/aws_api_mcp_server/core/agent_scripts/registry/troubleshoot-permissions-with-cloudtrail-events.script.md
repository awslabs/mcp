...
- **error_timeframe** (required): The time range when the permission error occurred (e.g., "2023-06-15T14:00:00Z,2023-06-15T16:00:00Z")
...
- You MUST verify AWS CLI is properly configured with these commands:
  ```
...
- You MUST use these AWS CLI commands:
```
# Option 1: Lookup events directly from CloudTrail (last 90 days)
...
# Option 2: Query CloudWatch Logs if CloudTrail logs are delivered there
aws logs filter-log-events \
  --log-group-name <cloudtrail_log_group> \
  --filter-pattern "{$.userIdentity.userName = <aws_username_or_role> && ($.errorCode = \"*AccessDenied*\" || $.errorCode = \"*UnauthorizedOperation*\")}" \
  --start-time <start_time_epoch_milliseconds> \
  --end-time <end_time_epoch_milliseconds> \
  --region <aws_region>
```
