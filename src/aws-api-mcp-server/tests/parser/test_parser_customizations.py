...
def test_s3_cp_stdin_as_source_blocked():
    """Test that 'aws s3 cp - s3://bucket/key' (stdin as source) is blocked."""
    with pytest.raises(FileParameterError, match='streaming to stdout'):
        parse('aws s3 cp - s3://my-bucket/file.txt')


def test_s3_cp_stdout_as_destination_allowed():
    """Test that 'aws s3 cp s3://bucket/key -' (stdout as destination) is allowed."""
