...
def mock_aws_clients():
    ...
    try:
        yield {'rum_client': mock_rum, 'logs_client': mock_logs,
               ...
               'applicationsignals_client': mock_appsignals, 'sts_client': mock_sts,
               'time': mock_time}
    finally:
        for p in patches:
            p.stop()
...
async def test_session_detail_limit_capped(mock_aws_clients):
    _setup_logs_mocks(mock_aws_clients)
    result = json.loads(await rum(action='session_detail', app_monitor_name='test',
                                  session_id='abc', start_time=START, end_time=END, limit=100000))
...
async def test_session_detail_limit_invalid(mock_aws_clients):
    _setup_logs_mocks(mock_aws_clients)
    result = json.loads(await rum(action='session_detail', app_monitor_name='test',
                                  session_id='abc', start_time=START, end_time=END, limit='abc'))
...
async def test_timeseries_bad_bucket_shape(mock_aws_clients):
    _setup_logs_mocks(mock_aws_clients)
    result = json.loads(await rum(action='timeseries', app_monitor_name='test',
                                  start_time=START, end_time=END, metric='errors', bucket='1year'))
    assert result.get('error_type') == 'bad_request'
...
async def test_metrics_malformed_response(mock_aws_clients):
    ...
    mock_aws_clients['cloudwatch_client'].get_metric_data.return_value = {
        'MetricDataResults': [{'Id': 'BAD', 'Timestamps': [], 'Values': [], 'StatusCode': 'Complete'}]}
    result = json.loads(await rum(action='metrics', app_monitor_name='test',
                                  metric_names='["JsErrorCount"]', start_time=START, end_time=END))
...
async def test_metrics_names_validation_rejects_non_list(mock_aws_clients, bad_names):
    result = json.loads(await rum(action='metrics', app_monitor_name='test',
                                  metric_names=bad_names, start_time=START, end_time=END))
    assert result.get('error_type') == 'bad_request'


@pytest.mark.asyncio
async def test_crashes_unknown_platform_takes_precedence_over_mismatch(mock_aws_clients):
    """When monitor platform is unknown, caller sees the unknown-platform hint
    (not a confusing mismatch message)."""
...
async def test_correlate_dedupes_and_oversamples(mock_aws_clients):
    """correlate should dedupe trace IDs and oversample to compensate for
    duplicate events sharing the same trace_id."""
    mock_aws_clients['rum_client'].get_app_monitor.return_value = _app_monitor_response()
    mock_aws_clients['logs_client'].start_query.return_value = {'queryId': 'qid'}
    ...
    result = json.loads(await rum(action='correlate', app_monitor_name='test',
                                  page_url='/home', start_time=START, end_time=END, max_traces=5))
...
async def test_web_schema_tools_guard_unknown_platform(mock_aws_clients, action, extra):
    """Web-schema tools must return the unknown-platform hint instead of silently
    running a web query against a mobile/OTel log group."""
    _setup_unknown_platform_mocks(mock_aws_clients)
    result = json.loads(await rum(action=action, app_monitor_name='test',
...
async def test_analyze_paginates_list_anomalies(mock_aws_clients):
    ...
    mock_aws_clients['logs_client'].list_log_anomaly_detectors.return_value = {
        'anomalyDetectors': [{'anomalyDetectorArn': 'arn:det1', 'detectorName': 'd1', 'anomalyDetectorStatus': 'ACTIVE'}],
    }
...
async def test_analyze_surfaces_anomaly_truncation(mock_aws_clients):
    ...
    mock_aws_clients['logs_client'].list_anomalies.return_value = {
        'anomalies': [{'firstSeen': 0}], 'nextToken': 'more',
    }
    result = json.loads(await rum(action='analyze', app_monitor_name='test',
                                  start_time=START, end_time=END))
...
async def test_correlate_all_xray_batches_fail(mock_aws_clients):
    ...
    """
    from awslabs.cloudwatch_applicationsignals_mcp_server.rum_tools import _CORRELATE_MAX_TRACES_CAP
    _setup_logs_mocks(mock_aws_clients)
    mock_aws_clients['logs_client'].get_query_results.return_value = _logs_result(
    ...
    result = json.loads(await rum(action='correlate', app_monitor_name='test',
                                  page_url='/home', start_time=START, end_time=END))
...
async def test_crashes_rejects_invalid_platform(mock_aws_clients, bad_platform):
    ...
    _setup_mobile_logs_mocks(mock_aws_clients)
    result = json.loads(await rum(action='crashes', app_monitor_name='test',
                                  start_time=START, end_time=END, platform=bad_platform))
    assert result.get('error_type') == 'bad_request'
    assert mock_aws_clients['logs_client'].start_query.call_count == 0
