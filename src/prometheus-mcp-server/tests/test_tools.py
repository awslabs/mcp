...
class TestTools:
    ...
    async def test_get_available_workspaces_with_configured_workspace_id(self, mock_context):
        ...
        with patch("awslabs.prometheus_mcp_server.server.get_prometheus_client", return_value=mock_client), \
            ...
            assert result["count"] == 2
            assert result["region"] == "us-east-1"
            assert result["requires_user_selection"] is False  # Should be False because we have a configured workspace
    ...
    @pytest.mark.asyncio
    async def test_get_available_workspaces_with_configured_workspace_id_not_in_list(self, mock_context):
        ...
        with patch("awslabs.prometheus_mcp_server.server.get_prometheus_client", return_value=mock_client), \
            ...
            assert result["count"] == 2  # Should include both the configured workspace and the one from the list
            assert result["region"] == "us-east-1"
            assert result["requires_user_selection"] is False  # Should be False because we have a configured workspace
            assert result["configured_workspace_id"] == "ws-12345"
            assert len(result["workspaces"]) == 2
            ...
            assert result["workspaces"][0]["workspace_id"] == "ws-12345"
            assert result["workspaces"][0]["is_configured"] is True
            assert "note" in result["workspaces"][0]  # Should have a note about being detected from URL
            assert result["workspaces"][1]["workspace_id"] == "ws-67890"
            assert result["workspaces"][1]["is_configured"] is False
    ...
    def test_extract_workspace_id_from_url(self):
        ...
        valid_urls = [
            ...
            ("https://aps-workspaces.region.amazonaws.com/workspaces/ws-12345/api/v1/query", "ws-12345"),
            # Additional test cases
            ("https://aps-workspaces.us-east-1.amazonaws.com/workspaces/ws-12345/api/v1", "ws-12345"),
