...
class PrometheusConfig(BaseModel):
    ...
    aws_profile: Optional[str] = Field(None, description="AWS profile name to use for authentication (optional)")
    service_name: str = Field(default='aps', description="AWS service name for SigV4 authentication")
    retry_delay: int = Field(default=1, description="Delay between retry attempts in seconds", ge=1)
    max_retries: int = Field(default=3, description="Maximum number of retry attempts", ge=1, le=10)
...
class MetricsList(BaseModel):
    ...
    metrics: List[str] = Field(description="List of metric names available in the Prometheus server")
