...
3. **Phase 3 — security / invariant suites** (per cluster):
   ...
   - `tls_enforcement` — validates TLS on the psycopg (PG Wire) path by toggling
     ...
     reconnecting. Asserts: `verify-ca` (default, bundled RDS CA) connects and
     the session is actually encrypted (`pg_stat_ssl.ssl` is true); `require`
     connects and is encrypted; and `verify-ca` against an **unrelated CA** is
     rejected (proving certificate verification, not just encryption). It also
     runs `verify-full` against the same endpoint as an **informational** probe
     that never fails the suite — against an Aurora cluster endpoint the cert
     SAN often does not match the hostname, so `verify-full` is expected to be
     rejected there, which is exactly why `verify-ca` is the default. Skipped on
     ...
     and the `verify-ca` positive case needs the bundled RDS CA present — run
