...
from awslabs.mcp_lambda_handler.session import NoOpSessionStore, DynamoDBSessionStore
from awslabs.mcp_lambda_handler.types import (
    JSONRPCError, JSONRPCResponse, ServerInfo, Capabilities, InitializeResult, JSONRPCRequest, TextContent, ErrorContent, ImageContent
)
...
def test_create_error_and_success_response():
    handler = MCPLambdaHandler("test")
    err = handler._create_error_response(123, "msg", request_id="abc", error_content=[{"foo": "bar"}], session_id="sid", status_code=400)
...
def test_dynamodb_session_store_methods():
    ...
    with patch("boto3.resource") as mock_resource:
        ...
        assert isinstance(sid, str)
        # get_session (found)
        mock_table.get_item.return_value = {"Item": {"expires_at": time.time() + 1000, "data": {"a": 1}}}
...
def make_lambda_event(jsonrpc_payload):
    ...
    return {
        ...
        "body": json.dumps(jsonrpc_payload) if isinstance(jsonrpc_payload, dict) else jsonrpc_payload,
...
def test_handle_request_valid():
    ...
    def echo(x: int) -> int:
        """Echo tool.\n\nArgs:\n    x: an integer\n        """
        return x
    # Use the 'tools/call' pattern for tool invocation
    req = {
        "jsonrpc": "2.0",
        "id": 4,
        "method": "tools/call",
    ...
    if isinstance(resp, dict) and "body" in resp:
        ...
        assert "content" in body["result"]
        assert isinstance(body["result"]["content"], list)
        # The echo tool should return 42 as text
        assert str(body["result"]["content"][0]["text"]) == "42"
        assert body["id"] == 4
        assert body["jsonrpc"] == "2.0"
    else:
        pytest.fail(f"Unexpected response: {resp}")
