...
class TestListInstances:
    ...
    async def test_list_instances_client_error(self, mock_aws_client):
        ...
        mock_client.describe_instances.side_effect = ClientError(
            error_response={
                'Error': {'Code': 'InvalidInstanceId', 'Message': 'Invalid instance ID'}
            },
...
class TestGetInstanceDetails:
    ...
    @pytest.mark.asyncio
    async def test_get_instance_details_success(
        self, mock_validate, mock_aws_client, mock_instance_data
    ):
...
class TestLaunchInstance:
    ...
    @pytest.mark.asyncio
    async def test_launch_instance_success(
        self, mock_find_subnet, mock_validate, mock_aws_client, mock_instance_data
    ):
        ...
        mock_client = MagicMock()
        mock_aws_client.get_client.return_value = mock_client
        mock_client.run_instances.return_value = {'Instances': [mock_instance_data]}
    ...
    @patch('awslabs.ec2_mcp_server.modules.vpc_management.find_suitable_subnet')
    @pytest.mark.asyncio
    async def test_launch_instance_minimal_params(
        self, mock_find_subnet, mock_validate, mock_aws_client, mock_instance_data
    ):
    ...
    @patch('awslabs.ec2_mcp_server.modules.instances.validate_ami_id')
    @pytest.mark.asyncio
    async def test_launch_instance_with_public_ip(
        self, mock_validate, mock_aws_client, mock_instance_data
    ):
        ...
        assert (
            'SecurityGroupIds' not in call_args
        )  # Should be removed when using NetworkInterfaces

    @patch('awslabs.ec2_mcp_server.modules.instances.aws_client')
    @patch('awslabs.ec2_mcp_server.modules.instances.validate_ami_id')
    @patch('awslabs.ec2_mcp_server.modules.vpc_management.find_suitable_subnet')
    @pytest.mark.asyncio
    async def test_launch_instance_with_user_data(
        self, mock_find_subnet, mock_validate, mock_aws_client, mock_instance_data
    ):
        ...
        result = await launch_instance(ami_id='ami-12345678', user_data=user_script)
    ...
    @patch('awslabs.ec2_mcp_server.modules.vpc_management.find_suitable_subnet')
    @pytest.mark.asyncio
    async def test_launch_instance_with_monitoring_and_termination_protection(
        self, mock_find_subnet, mock_validate, mock_aws_client, mock_instance_data
    ):
        ...
        result = await launch_instance(
            ami_id='ami-12345678', monitoring_enabled=True, disable_api_termination=True
        )
...
class TestGetSubnetInfo:
    ...
    async def test_get_subnet_info_public_subnet(self, mock_aws_client):
        ...
        mock_client.describe_route_tables.return_value = {
            'RouteTables': [
                {'Routes': [{'GatewayId': 'igw-12345678', 'DestinationCidrBlock': '0.0.0.0/0'}]}
            ]
        }
...
class TestStopInstance:
    ...
    async def test_stop_instance_success(self, mock_validate, mock_aws_client):
        ...
        mock_client.stop_instances.assert_called_once_with(
            InstanceIds=['i-1234567890abcdef0'], Force=True
        )
...
class TestAdditionalInstanceCases:
    ...
    @patch('awslabs.ec2_mcp_server.modules.vpc_management.find_suitable_subnet')
    @pytest.mark.asyncio
    async def test_launch_instance_subnet_find_error(
        self, mock_find_subnet, mock_validate, mock_aws_client
    ):
        """Test launch instance when subnet finding fails."""
        mock_validate.return_value = True
        mock_find_subnet.return_value = {'status': 'error', 'message': 'No suitable subnet found'}
    ...
    @patch('awslabs.ec2_mcp_server.modules.vpc_management.find_suitable_subnet')
    @pytest.mark.asyncio
    async def test_launch_instance_run_instances_error(
        self, mock_find_subnet, mock_validate, mock_aws_client
    ):
