...
class TestListKeyPairs:
    ...
    async def test_list_key_pairs_success(self, mock_aws_client, mock_key_pair_data):
        ...
        mock_client.describe_key_pairs.return_value = {'KeyPairs': [mock_key_pair_data]}
...
class TestCreateKeyPair:
    ...
    @patch('awslabs.ec2_mcp_server.modules.key_pairs.aws_client')
    @pytest.mark.asyncio
    async def test_create_key_pair_success_with_storage(
        self, mock_aws_client, mock_validate, mock_key_storage, mock_create_key_pair_response
    ):
        ...
        mock_key_storage.store_private_key = AsyncMock(
            return_value={
                'status': 'success',
        ...
        result = await create_key_pair(
            key_name='test-key', storage_method='secrets_manager', tags={'Environment': 'test'}
        )
    ...
    @patch('awslabs.ec2_mcp_server.modules.key_pairs.aws_client')
    @pytest.mark.asyncio
    async def test_create_key_pair_storage_failure_rollback(
        self, mock_aws_client, mock_validate, mock_key_storage, mock_create_key_pair_response
    ):
        ...
        mock_key_storage.store_private_key = AsyncMock(
            return_value={'status': 'error', 'message': 'Storage service unavailable'}
        )
...
class TestDeleteKeyPair:
    ...
    @patch('awslabs.ec2_mcp_server.modules.key_pairs.aws_client')
    @pytest.mark.asyncio
    async def test_delete_key_pair_success_without_storage_info(
        self, mock_aws_client, mock_validate
    ):
    ...
    @patch('awslabs.ec2_mcp_server.modules.key_pairs.aws_client')
    @pytest.mark.asyncio
    async def test_delete_key_pair_with_stored_key_success(
        self, mock_aws_client, mock_validate, mock_key_storage
    ):
    ...
    @patch('awslabs.ec2_mcp_server.modules.key_pairs.aws_client')
    @pytest.mark.asyncio
    async def test_delete_key_pair_without_deleting_stored_key(
        self, mock_aws_client, mock_validate
    ):
        ...
        result = await delete_key_pair(key_name='test-key', delete_stored_key=False)
...
class TestRegisterModule:
    ...
    def test_register_module(self):
        ...
        expected_tools = ['list_key_pairs', 'create_key_pair', 'delete_key_pair']
