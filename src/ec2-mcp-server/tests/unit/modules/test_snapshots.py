...
class TestListSnapshots:
    ...
    async def test_list_snapshots_success(self, mock_aws_client, mock_snapshot_data):
        ...
        mock_client.describe_snapshots.return_value = {'Snapshots': [mock_snapshot_data]}
    ...
    async def test_list_snapshots_with_all_params(self, mock_aws_client, mock_snapshot_data):
        ...
        result = await list_snapshots(
            snapshot_ids=snapshot_ids, owner_ids=owner_ids, filters=filters
        )

        assert result['status'] == 'success'
        mock_client.describe_snapshots.assert_called_once_with(
            SnapshotIds=snapshot_ids, OwnerIds=owner_ids, Filters=filters
        )
    ...
    async def test_list_snapshots_with_missing_start_time(self, mock_aws_client):
        ...
        mock_client = MagicMock()
        mock_aws_client.get_client.return_value = mock_client
        mock_client.describe_snapshots.return_value = {'Snapshots': [snapshot_data]}
    ...
    async def test_list_snapshots_client_error(self, mock_aws_client):
        ...
        mock_client.describe_snapshots.side_effect = ClientError(
            error_response={
                'Error': {'Code': 'InvalidSnapshot.NotFound', 'Message': 'Snapshot not found'}
            },
...
class TestCreateSnapshot:
    ...
    @patch('awslabs.ec2_mcp_server.modules.snapshots.aws_client')
    @pytest.mark.asyncio
    async def test_create_snapshot_success_minimal(
        self, mock_aws_client, mock_validate, mock_create_snapshot_response
    ):
    ...
    async def test_create_snapshot_with_description(
        ...
        result = await create_snapshot(
            volume_id='vol-1234567890abcdef0', description='My test snapshot'
        )
    ...
    async def test_create_snapshot_with_tags(
        ...
        result = await create_snapshot(volume_id='vol-1234567890abcdef0', tags=tags)
    ...
    @patch('awslabs.ec2_mcp_server.modules.snapshots.aws_client')
    @pytest.mark.asyncio
    async def test_create_snapshot_with_all_params(
        self, mock_aws_client, mock_validate, mock_create_snapshot_response
    ):
...
class TestRegisterModule:
    ...
    def test_register_module(self):
        ...
        expected_tools = ['list_snapshots', 'create_snapshot']
