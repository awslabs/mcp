...
class TestConfig(unittest.TestCase):
    ...
    @patch('os.environ')
    def test_get_config_missing_values(self, mock_environ):
        """Test that missing environment variables use defaults."""
        # Set up mock environment with missing values
        mock_environ.get.side_effect = (
            lambda key, default=None: {
                # Only return defaults, no actual environment variables
            }.get(key, default)
        )
        ...
        self.assertIsNone(config['aws_profile'])
