...
class TestResponseSanitizer:
    ...
    def test_sanitize_dict(self):
        ...
        response = {
            ...
            'secret_key': 'wJalrXUtnFEMI/K7MDENG/bPxRfiCYEXAMPLEKEY',  # 40 chars  # pragma: allowlist secret
            'password': 'password=mysecret123',  # pragma: allowlist secret
            'ip_address': '192.168.1.100',
            'email': 'user@example.com',
            'nested': {
                'aws_access_key': 'AKIAIOSFODNN7EXAMPLE',  # 20 chars  # pragma: allowlist secret
    ...
    def test_sanitize_string(self):
        ...
        sensitive_string = (
            'Access key: AKIAIOSFODNN7EXAMPLE, IP: 192.168.1.100'  # pragma: allowlist secret
        )
        sanitized = ResponseSanitizer.sanitize(sensitive_string)

        assert '[REDACTED' in sanitized
        assert 'AKIAIOSFODNN7EXAMPLE' not in sanitized  # pragma: allowlist secret
        assert '192.168.1.100' not in sanitized

    def test_add_security_warning(self):
        """Test adding security warnings for public resources."""
        response = {'status': 'success', 'public_ip': '54.123.45.67'}
    ...
    def test_add_security_warning_no_public_ip(self):
        ...
        response = {'status': 'success', 'private_ip': '10.0.1.100'}

        response_with_warning = ResponseSanitizer.add_security_warning(response)

        assert (
            'warnings' not in response_with_warning
            or len(response_with_warning.get('warnings', [])) == 0
        )
...
class TestAdditionalSecurityCases:
    ...
    def test_response_sanitizer_mixed_data_types(self):
        ...
        response = [
            ...
            123,
            {'secret': 'AKIAIOSFODNN7EXAMPLE'},  # Use a known pattern  # pragma: allowlist secret
            True,
            None,
            [],
        ]
    ...
    def test_response_sanitizer_edge_case_patterns(self):
        ...
        response = {
            ...
            'almost_secret_key': 'abcdefghijklmnopq123456789012345678901',  # 41 chars, should not match  # pragma: allowlist secret
            'partial_ip': '192.168.1',  # Incomplete IP
            'not_email': 'user@',  # Incomplete email
            'password_in_text': 'password=secret123',  # Password keyword with pattern  # pragma: allowlist secret
        }
        ...
        assert (
            sanitized['almost_secret_key']
            == 'abcdefghijklmnopq123456789012345678901'  # pragma: allowlist secret
        )
