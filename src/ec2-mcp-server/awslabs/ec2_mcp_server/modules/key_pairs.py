            ...
            result.update(
                {
                    ...
                    'storage_details': {
                        k: v for k, v in storage_result.items() if k not in ['status', 'message']
                    },
                }
            )
            result['message'] += (
                f' and private key securely stored using {key_storage.STORAGE_METHODS[storage_method]}'
            )
        else:
        ...
        if delete_stored_key:
            if storage_method and storage_location:
                ...
                if storage_result.get('status') == 'success':
                    ...
                    result['warning'] = (
                        f'Key pair deleted but stored key deletion failed: {storage_result.get("message")}'
                    )
            else:
                result['warning'] = (
                    'Key pair deleted but stored key deletion was requested without storage_method and storage_location. Private key may still exist in AWS storage.'
                )
