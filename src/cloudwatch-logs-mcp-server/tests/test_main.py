...
from awslabs.cloudwatch_logs_mcp_server.server import main
...
class TestMain:
    ...
    @patch('awslabs.cloudwatch_logs_mcp_server.server.mcp.run')
    @patch('sys.argv', ['awslabs.cloudwatch-logs-mcp-server'])
    ...
    @patch('awslabs.cloudwatch_logs_mcp_server.server.mcp.run')
    @patch('sys.argv', ['awslabs.cloudwatch-logs-mcp-server', '--sse', '--port', '9999'])
    def test_main_sse(self, mock_run):
        ...
        from awslabs.cloudwatch_logs_mcp_server.server import mcp
    ...
    def test_module_execution(self):
        ...
        import inspect
        from awslabs.cloudwatch_logs_mcp_server import server
