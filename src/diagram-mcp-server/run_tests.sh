...
# Check if diagrams_mcp_server module can be imported
...
if [ $? -ne 0 ]; then
    ...
    # Check if pip is available
        ...
        ln -sf $(pwd)/awslabs/diagrams_mcp_server $(pwd)/diagrams_mcp_server 2>/dev/null
    fi
...
# $PYTEST_CMD --cov=diagrams_mcp_server --cov-report=term-missing tests/

# If you want to run with coverage and generate an HTML report, uncomment the following line
# $PYTEST_CMD --cov=diagrams_mcp_server --cov-report=html tests/
