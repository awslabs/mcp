...
import re
from awslabs.mcp_cost_analysis_expert.helpers import CostAnalysisHelper
from awslabs.mcp_cost_analysis_expert.static import COST_REPORT_TEMPLATE
...
def _create_unit_pricing_table(services_info: Dict) -> Tuple[str, float, float, float]:
    ...
    cost_calculation_table, total_min, total_max, base_cost = _create_cost_calculation_table(
        services_info
    )
...
def _calculate_base_cost(
    custom_cost_data: Dict, services_info: Dict, total_min: float = 0, total_max: float = 0
) -> Optional[float]:
...
async def _generate_custom_data_report(
    ...
    cost_calculation_table, total_min, total_max, initial_base_cost = (
...
async def _generate_pricing_data_report(
    ...
    if len(pricing_structure['recommendations']['best_practices']) >= 3:
        report = report.replace(
            '{best_practice_1}', pricing_structure['recommendations']['best_practices'][0]
        )
        report = report.replace(
            '{best_practice_2}', pricing_structure['recommendations']['best_practices'][1]
        )
        report = report.replace(
            '{best_practice_3}', pricing_structure['recommendations']['best_practices'][2]
        )
