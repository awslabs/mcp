        ...
        try:
            ...
            try:
                ...
                if not anti_affinity_result['compliant']:
                    all_compliant = False
                logger.info(f"Pod anti-affinity check completed: {anti_affinity_result['compliant']}")
            except Exception as e:
            ...
            try:
                ...
                if not metrics_server_result['compliant']:
                    all_compliant = False
                logger.info(f"Metrics server check completed: {metrics_server_result['compliant']}")
            except Exception as e:
            ...
            try:
                ...
                if not custom_metrics_result['compliant']:
                    all_compliant = False
                logger.info(f"Custom metrics check completed: {custom_metrics_result['compliant']}")
            except Exception as e:
                ...
                for hpa in hpas_response.items:
                    ...
                    for metric in metrics:
                        ...
                        if metric_type in ['Object', 'Pods', 'External']:
                            logger.info(f"Found HPA with custom/external metrics: {hpa_namespace}/{name}")
  ...
  preStop:
                ...
                try:
                    ...
                    for deployment in deployments_response.items:
                        ...
                        if 'istio' in name:
                            ...
                            istio_found = True
                            service_mesh_components.append(f"Deployment: {deploy_namespace}/{name}")
                            break
                except Exception:
                    pass
                ...
                for pod in pods_response.items:
                    ...
                    if len(containers) > 1:
                        for container in containers:
                            ...
                            if (container_name in ['istio-proxy', 'linkerd-proxy', 'consul-proxy', 'envoy'] or
                                'proxy' in container_name or 'sidecar' in container_name):
                                logger.info(f"Found sidecar container: {container_name} in {pod_namespace}/{name}")
                                has_sidecars = True
        ...
        try:
            ...
            try:
                ...
                if access_config.get('authenticationMode') == 'API_AND_CONFIG_MAP' or access_config.get('authenticationMode') == 'API':
                    ...
                    try:
                        response = eks_client.list_access_entries(clusterName=cluster_name, maxResults=1)
                        if response and 'accessEntries' in response and len(response['accessEntries']) > 0:
                            access_entries_enabled = True
                    except Exception:
                        pass
            except Exception as e:
    ...
    def _check_c3(self, k8s_api, cluster_name: str, namespace: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            service_count = 0
            try:
                services = k8s_api.list_resources(
                    kind="Service",
                    api_version="v1"
                )
                ...
                if kube_proxy_cm and hasattr(kube_proxy_cm, 'items') and len(kube_proxy_cm.items) > 0:
                    ...
                    if kube_proxy_cm_dict.get('data') and 'config.conf' in kube_proxy_cm_dict['data']:
                        ...
                        if 'mode: "ipvs"' in config_data:
                            ...
                            best_practices.append("kube-proxy is running in IPVS mode, which is recommended for large clusters")
                        else:
                            kube_proxy_mode = "iptables"
                            issues.append("kube-proxy is running in iptables mode, which may cause network latency in clusters with >1000 services")
            except Exception as e:
                    ...
                    for container in containers:
                        if container.get('name') == 'aws-node':
                            for env in container.get('env', []):
                                if env.get('name') == 'WARM_IP_TARGET' and env.get('value') and int(env.get('value', '0')) > 0:
                                    cni_ip_caching = True
            ...
            if namespace:
                ...
                try:
                    ...
                    return {
                        ...
                        "remediation": self._get_resource_quota_remediation() if not has_quota else ""
                    }
                except Exception as e:
            ...
            if namespace:
                ...
                try:
                    ...
                    return {
                        ...
                        "remediation": self._get_limit_range_remediation() if not has_limit_range else ""
                    }
                except Exception as e:
                ...
                try:
                    ...
                    for addon in addons.get('addons', []):
                        if addon.lower() == 'coredns':
                            ...
                            logger.info(f"Cluster {cluster_name} is using EKS managed addon for CoreDNS")
                            break
                except Exception as e:
