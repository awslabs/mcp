...
class TestHybridNodesHandler:
    ...
    async def test_get_eks_vpc_config_vpc_not_found(self, mock_context, mock_mcp):
        ...
        mock_eks_client.describe_cluster.return_value = {
            'cluster': {
                'resourcesVpcConfig': {'vpcId': 'vpc-nonexistent'}
            }
        }
        ...
        mock_ec2_client.describe_route_tables.return_value = {
            'RouteTables': [{
                ...
                'Routes': [
                    ...
                    {'DestinationCidrBlock': '192.168.0.0/16', 'TransitGatewayId': 'tgw-remote'},
                ]
            }]
        }
    ...
    async def test_get_eks_vpc_config_with_no_pod_networks(self, mock_context, mock_mcp):
        ...
        mock_ec2_client.describe_route_tables.return_value = {
            'RouteTables': [{
                ...
                'Routes': [
                    ...
                    {'DestinationCidrBlock': '192.168.0.0/16', 'TransitGatewayId': 'tgw-nopod'},
                    {'DestinationCidrBlock': '172.16.0.0/16', 'VpcPeeringConnectionId': 'pcx-nopod'}
                ]
            }]
        }
        ...
        assert any(r['destination_cidr_block'] == '192.168.0.0/16' and 
                   r['target_type'] == 'transitgateway' for r in result.routes)
        assert any(r['destination_cidr_block'] == '172.16.0.0/16' and 
                   r['target_type'] == 'vpcpeeringconnection' for r in result.routes)
    ...
    async def test_get_eks_vpc_config_impl(self, mock_context, mock_mcp):
        ...
        # Mock subnets response for the direct implementation test
        mock_ec2_client.describe_subnets.return_value = {
            'Subnets': [
                {
    ...
    def _create_mock_insight_item(insight_id="test-insight-id", category="CONFIGURATION"):
        ...
        from awslabs.eks_mcp_server.models import EksInsightStatus
    ...
    async def test_get_eks_insights_list_mode(self, mock_context, mock_mcp):
        ...
        mock_eks_client.list_insights.return_value = {
            'insights': [
                ...
                self._create_mock_insight_item(insight_id="insight-2", category="UPGRADE_READINESS")
            ]
        }
    ...
    async def test_get_eks_insights_detail_mode(self, mock_context, mock_mcp):
        ...
        insight_data = self._create_mock_insight_item(insight_id="detail-insight", category="CONFIGURATION")
        ...
        # Verify success message in content
        assert "Successfully retrieved details for insight detail-insight" in result.content[0].text
    ...
    async def test_get_eks_insights_with_category_filter(self, mock_context, mock_mcp):
        ...
        mock_eks_client.list_insights.return_value = {
            'insights': [
                self._create_mock_insight_item(insight_id="config-insight-1", category="CONFIGURATION"),
                self._create_mock_insight_item(insight_id="config-insight-2", category="CONFIGURATION")
                # UPGRADE_READINESS insights would be filtered out by the AWS API
            ]
        }
    ...
    async def test_get_eks_insights_with_region(self, mock_context, mock_mcp):
        ...
        mock_eks_client.list_insights.return_value = {
            'insights': [
                self._create_mock_insight_item(insight_id="region-insight", category="CONFIGURATION"),
            ]
        }
        ...
        with patch('awslabs.eks_mcp_server.hybrid_nodes_handler.AwsHelper') as mock_aws_helper:
            ...
            mock_aws_helper.create_boto3_client.assert_called_with(
    ...
    async def test_get_eks_insights_insight_not_found(self, mock_context, mock_mcp):
        ...
        mock_eks_client.describe_insight.side_effect = mock_eks_client.exceptions.ResourceNotFoundException(
            error_response, 'DescribeInsight'
        )
