...
class TestVpcConfigHandler:
    ...
    @pytest.mark.asyncio
    async def test_get_eks_vpc_config_with_explicit_vpc_id(
        ...
        mock_eks_client.describe_cluster.return_value = {
            "cluster": {
                "resourcesVpcConfig": {
                    "vpcId": "vpc-explicit"
                },  # Include VPC ID in response
                # No remote network config in this test
            }
        }

        # Initialize the handler and mock client getters
        handler = VpcConfigHandler(mock_mcp)
        with patch.object(handler, "_get_ec2_client", return_value=mock_ec2_client):
            with patch.object(handler, "_get_eks_client", return_value=mock_eks_client):
                ...
                mock_eks_client.describe_cluster.assert_called_once_with(
                    name="test-cluster"
                )
    ...
    async def test_get_eks_vpc_config_vpc_not_found(self, mock_context, mock_mcp):
        ...
        error_response = {
            "Error": {
                ...
                "Message": "VPC vpc-nonexistent not found",
            }
        }
        mock_ec2_client.describe_vpcs.side_effect = (
            mock_ec2_client.exceptions.ClientError(error_response, "DescribeVpcs")
        )
    ...
    async def test_get_eks_vpc_config_no_vpc_id(self, mock_context, mock_mcp):
        ...
        mock_eks_client.describe_cluster.return_value = {
            "cluster": {"resourcesVpcConfig": {}}  # No VPC ID in response
        }
        ...
        with patch.object(handler, "_get_ec2_client", return_value=mock_ec2_client):
            with patch.object(handler, "_get_eks_client", return_value=mock_eks_client):
                ...
                assert (
                    "Could not determine VPC ID for cluster" in result.content[0].text
                )
    ...
    async def test_get_eks_vpc_config_with_remote_network(self, mock_context, mock_mcp):
        ...
        mock_eks_client.describe_cluster.return_value = {
            "cluster": {
                ...
                "remoteNetworkConfig": {
                    "remoteNodeNetworks": [
                        {"cidrs": ["192.168.0.0/16", "192.168.1.0/24"]}
                    ],
                    "remotePodNetworks": [
                        {"cidrs": ["172.16.0.0/16", "172.17.0.0/16"]}
                    ],
                },
            }
        }
    ...
    @pytest.mark.asyncio
    async def test_get_eks_vpc_config_with_no_pod_networks(
        self, mock_context, mock_mcp
    ):
        ...
        mock_eks_client.describe_cluster.return_value = {
            "cluster": {
                ...
                "remoteNetworkConfig": {
                    "remoteNodeNetworks": [
                        {"cidrs": ["192.168.0.0/16", "172.16.0.0/16"]}
                    ]
    ...
    async def test_get_eks_vpc_config_impl(self, mock_context, mock_mcp):
        ...
        with patch.object(handler, "_get_ec2_client", return_value=mock_ec2_client):
            with patch.object(handler, "_get_eks_client", return_value=mock_eks_client):
                ...
                assert any(
                    route["destination_cidr_block"] == "192.168.0.0/16"
                    for route in data["routes"]
                )
                ...
                # Check first subnet
                subnet1 = next(
                    (s for s in data["subnets"] if s["subnet_id"] == "subnet-12345"),
                    None,
                )
                ...
                # Check second subnet
                subnet2 = next(
                    (s for s in data["subnets"] if s["subnet_id"] == "subnet-67890"),
                    None,
                )
    ...
    async def test_get_vpc_id_for_cluster_success(self, mock_context, mock_mcp):
        ...
        # Call the helper method with the mock client
        vpc_id, cluster_response = await handler._get_vpc_id_for_cluster(
            mock_context, "test-cluster", mock_eks_client
        )
    ...
    async def test_get_vpc_id_for_cluster_no_vpc_id(self, mock_context, mock_mcp):
        ...
        mock_eks_client.describe_cluster.return_value = {
            "cluster": {"resourcesVpcConfig": {}}  # No VPC ID
        }
        ...
        with pytest.raises(Exception) as excinfo:
            await handler._get_vpc_id_for_cluster(
                mock_context, "test-cluster", mock_eks_client
            )
    ...
    async def test_get_vpc_details_success(self, mock_context, mock_mcp):
        ...
        cidr_block, additional_cidr_blocks = await handler._get_vpc_details(
            mock_context, "vpc-12345", mock_ec2_client
        )
    ...
    async def test_get_vpc_details_vpc_not_found(self, mock_context, mock_mcp):
        ...
        # Call the helper method and expect an exception
        with pytest.raises(Exception) as excinfo:
            await handler._get_vpc_details(
                mock_context, "vpc-nonexistent", mock_ec2_client
            )
    ...
    async def test_get_vpc_details_api_error(self, mock_context, mock_mcp):
        ...
        # Call the helper method and expect an exception
        with pytest.raises(Exception) as excinfo:
            await handler._get_vpc_details(mock_context, "vpc-12345", mock_ec2_client)
    ...
    async def test_get_subnet_information(self, mock_context, mock_mcp):
        ...
        # Call the helper method with the mock client
        subnets = await handler._get_subnet_information(
            mock_context, "vpc-12345", mock_ec2_client
        )
        ...
        subnet3 = next(
            (s for s in subnets if s["subnet_id"] == "subnet-disallowed"), None
        )
    ...
    async def test_get_route_table_information(self, mock_context, mock_mcp):
        ...
        # Call the helper method with the mock client
        routes = await handler._get_route_table_information(
            mock_context, "vpc-12345", mock_ec2_client
        )
        ...
        assert any(
            r["destination_cidr_block"] == "10.2.0.0/16"
            and r["target_type"] == "networkinterface"
            for r in routes
        )
        assert any(
            r["destination_cidr_block"] == "10.3.0.0/16"
            and r["target_type"] == "unknown"
            for r in routes
        )
    ...
    @pytest.mark.asyncio
    async def test_get_remote_cidr_blocks_with_cluster_response(
        self, mock_context, mock_mcp
    ):
        ...
        remote_node_cidr_blocks, remote_pod_cidr_blocks = (
            await handler._get_remote_cidr_blocks(
    ...
    @pytest.mark.asyncio
    async def test_get_remote_cidr_blocks_without_cluster_response(
        self, mock_context, mock_mcp
    ):
        ...
        # Initialize the handler and mock client getter
        handler = VpcConfigHandler(mock_mcp)
        with patch.object(handler, "_get_eks_client", return_value=mock_eks_client):
            # Call the helper method with no cluster response
            remote_node_cidr_blocks, remote_pod_cidr_blocks = (
                await handler._get_remote_cidr_blocks(mock_context, "test-cluster")
            )
    ...
    @pytest.mark.asyncio
    async def test_get_eks_vpc_config_impl_inner_exception(
        self, mock_context, mock_mcp
    ):
