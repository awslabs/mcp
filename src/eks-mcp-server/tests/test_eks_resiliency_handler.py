...
class TestEKSResiliencyHandlerInit:
    ...
    @pytest.mark.asyncio
    async def test_check_eks_resiliency_connection_error(self, mock_mcp, mock_client_cache, mock_context):
    ...
    async def test_check_eks_resiliency_success(self, mock_mcp, mock_client_cache, mock_context):
        ...
        check_methods = [
            '_check_singleton_pods', '_check_multiple_replicas', '_check_pod_anti_affinity',
        ...
        for method_name in check_methods:
            patches.append(patch.object(handler, method_name, return_value={
...
class TestEKSResiliencyHandlerChecksA:
    ...
    def test_check_custom_metrics(self, mock_mcp, mock_client_cache, mock_k8s_api):
        ...
        def mock_api_call(resource_path, method, **kwargs):
            if 'custom.metrics.k8s.io' in resource_path or 'external.metrics.k8s.io' in resource_path:
...
class TestEKSResiliencyHandlerChecksC:
    ...
    def test_check_c1(self, mock_mcp, mock_client_cache, mock_k8s_api):
        ...
        with patch('awslabs.eks_mcp_server.eks_resiliency_handler.AwsHelper.create_boto3_client') as mock_create_client:
            # Mock the EKS client
...
class TestEKSResiliencyHandlerChecksD:
    ...
    def test_check_d4(self, mock_mcp, mock_client_cache, mock_k8s_api):
        ...
        mock_namespace.to_dict.return_value = {
            'metadata': {
                'name': 'test-namespace'
            }
        }
