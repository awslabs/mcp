...
from mcp_core.static import PROMPT_UNDERSTANDING
from mcp_core.server.available_servers import AVAILABLE_MCP_SERVERS
    ...
    try:
        ...
        return {
            "content": [
                {
                    "type": "text",
                    "text": f"Error updating MCP servers: {e}"
                }
            ],
            "isError": True
        }
                ...
                for var in env_vars_to_check:
                    if f"{var}=" in line:
                        ...
                        if os.path.exists(config_path):
                            logger.info(f"Found MCP config path in parent process environment: {config_path}")
                            return Path(config_path)
    except Exception as e:
        logger.warning(f"Error getting parent process environment: {e}")
    
    # If we couldn't determine the config path, raise an error
    raise ValueError("No MCP config path found in environment variables. Please set MCP_CONFIG_PATH, CLAUDE_CONFIG_PATH, or CLINE_MCP_SETTINGS_PATH.")


def install_to_mcp_config(name: str, cmd: str, args: List[str] = None, env: Dict[str, str] = None) -> None:
...
def install_repo_mcp_server(name: str, args: List[str] = None, env: List[str] = None) -> str:
    ...
    return {
        "content": [
            {
                "type": "text",
                "text": f"Installed MCP server via uvx successfully! Tell the user to restart the app"
            }
        ]
    }
...
def ensure_mcp_servers_installed():
    ...
    try:
        ...
        if config_path.exists():
            logger.info(f"Config file exists, reading content")
            with open(config_path, 'r') as f:
                try:
                    config = json.load(f)
                    logger.info(f"Successfully loaded config file with {len(config.get('mcpServers', {}))} servers")
                except json.JSONDecodeError as e:
                    logger.error(f"Error parsing config file: {e}")
                    config = {}
        else:
            logger.warning(f"Config file does not exist, creating new config")
            config = {}
        ...
        try:
            with open(config_path, 'w') as f:
                json.dump(config, f, indent=2)
                logger.info(f"Successfully wrote config file")
        except Exception as e:
            logger.error(f"Error writing config file: {e}")
            raise
        
        if changes_made:
            logger.info(f"Added missing MCP servers to config")
        else:
            logger.info(f"All MCP servers already in config, saved file to trigger reload")
...
def main():
    """Run the MCP server."""
    parser = argparse.ArgumentParser(description='A Model Context Protocol (MCP) server for mcp-core')
    parser.add_argument('--sse', action='store_true', help='Use SSE transport')
    parser.add_argument('--port', type=int, default=8888, help='Port to run the server on')
    parser.add_argument('--skip-server-check', action='store_true', help='Skip checking for missing MCP servers')
