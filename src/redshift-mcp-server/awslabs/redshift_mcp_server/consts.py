...
SELECT
    ...
    pg_stat_user_tables.n_live_tup AS tbl_rows,
    (c.relpages * 8192 / 1024 / 1024)::bigint AS size,
    CASE
        WHEN c.relpages > 0 THEN (pg_stat_user_tables.n_live_tup::float / c.relpages * 100)::numeric(5,2)
        ELSE 0
    END AS pct_used,
    CASE
        WHEN pg_stat_user_tables.last_analyze IS NULL THEN 100
        WHEN pg_stat_user_tables.n_mod_since_analyze = 0 THEN 0
        ELSE LEAST(100, (pg_stat_user_tables.n_mod_since_analyze::float / NULLIF(pg_stat_user_tables.n_live_tup, 0) * 100)::numeric(5,2))
    END AS stats_off,
    1.0 AS skew_rows
FROM pg_catalog.pg_class c
JOIN pg_catalog.pg_namespace n ON c.relnamespace = n.oid
LEFT JOIN pg_catalog.pg_stat_user_tables ON c.oid = pg_stat_user_tables.relid
