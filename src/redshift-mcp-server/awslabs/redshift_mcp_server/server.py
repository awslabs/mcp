...
async def list_tables_tool(
    ...
    Redshift-specific properties (from pg_catalog, may be None on serverless):
    ...
    - stats_live_row_count, stats_dead_row_count: Row estimates.
    - stats_last_analyze_time, stats_last_autoanalyze_time: Analyze timestamps.
    - stats_analyze_count, stats_autoanalyze_count: Analyze counts.
    - stats_rows_modified_since_analyze: Rows modified since last analyze.
    ...
    6. Review table statistics to identify tables that may need ANALYZE or have unusual activity patterns.
    ...
    7. Monitor table statistics and run ANALYZE when needed for accurate query planning.
    """
    try:
        logger.info(
...
async def describe_execution_plan_tool(
    ...
    9. Review table activity statistics to assess whether tables need ANALYZE or have unusual access patterns.
