...
class TestDiscoverFunctions:
    ...
    async def test_discover_tables(self, mocker):
            ...
            (
                {
                    'Records': [
                        [
                            ...
                            {'longValue': 990},
                            {'longValue': 10},
                            {'stringValue': '2024-01-15 10:00:00'},
                            {'stringValue': '2024-01-16 12:00:00'},
                            {'longValue': 3},
                            {'longValue': 7},
                            {'longValue': 25},
                        ]
        ...
        assert result[0]['stats_rows_modified_since_analyze'] == 25
...
class TestDescribeExecutionPlan:
    ...
    async def test_describe_execution_plan_with_table_designs(self, mocker):
        ...
        assert id_col['redshift_sortkey_position'] == 1
    ...
    async def test_describe_execution_plan_with_column_stats(self, mocker):
        ...
        # Third call: COLUMN_STATS_SQL response (after discover_tables and discover_columns)
        ...
        # Set side_effect: first call is EXPLAIN, second is column stats
...
class TestGeneratePerformanceSuggestions:
    ...
    def test_suggestions_for_stale_statistics(self):
        """Test suggestions when rows_modified_since_analyze is high relative to row count."""
        from awslabs.redshift_mcp_server.redshift import _generate_performance_suggestions
        ...
        table_designs = [
            {
                ...
                'redshift_diststyle': 'KEY',
                'redshift_estimated_row_count': 10000,
                'stats_rows_modified_since_analyze': 5000,
                'columns': [
                    {
                        'column_name': 'id',
                        'redshift_encoding': 'lzo',
        ...
        assert any('ANALYZE' in s and '5,000 rows modified' in s for s in suggestions)

    def test_suggestions_for_dead_rows(self):
        """Test suggestions when dead/live row ratio is high."""
        from awslabs.redshift_mcp_server.redshift import _generate_performance_suggestions
        ...
        table_designs = [
            {
                ...
                'table_name': 'sessions',
                'redshift_diststyle': 'KEY',
                'stats_dead_row_count': 5000,
                'stats_live_row_count': 10000,
                'columns': [
                    {
                        'column_name': 'id',
                        'redshift_encoding': 'lzo',
        ...
        assert any('VACUUM' in s and '5,000 dead rows' in s for s in suggestions)
