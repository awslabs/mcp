...
>
> In read-write mode `execute_query` runs statements with autocommit and no transaction wrapper, so any modifying statement the model generates takes effect immediately. Transactions are not supported yet, so for now there is no rollback and nothing to undo once a statement runs. The server asks you to confirm each statement that could change something before it executes, which is the only point at which you can stop it. There is also an unsafe mode that skips those prompts, leaving nothing between a generated statement and your data, so do not turn it on unless you genuinely have to.
>
...
- `sql`: SQL statement to execute (a single statement)
...
The mode is read once at startup, so it is fixed for the life of the server process and cannot be changed per call. Only the exact value `read-write` selects it; anything else, including a typo, falls back to `read-only` and logs a warning naming the supported modes.
...
| Statement-type guard | Denies `UNLOAD`, `GRANT`, `REVOKE`, `TRUNCATE`, `VACUUM`, `ANALYZE`, `CALL`, `COMMENT`, `CANCEL`, and transaction control | Not applied |
...
| Undo | Nothing to undo | None for now; transactions are not supported yet, and no snapshot is taken |
