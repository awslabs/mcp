...
def main():
    ...
    try:
        ...
        for i, line in enumerate(lines):
            stripped = line.strip()
            if (stripped.startswith('import ') or stripped.startswith('from ')) and any(
                pkg in stripped for pkg in problematic_packages
            ):
        ...
        try:
            ...
            if not hasattr(module, APP_VAR_NAME):
                ...
                print(
                    'If your BedrockAgentResolver instance has a different name, update APP_VAR_NAME.'
                )
                return False
            ...
            print(
                'Next steps: Use this schema in your CDK code with bedrock.ApiSchema.fromLocalAsset()'
            )
            return True
            ...
            if 'No module named' in str(simplified_error):
                missing_dep = (
                    str(simplified_error).split("'")[-2]
                    if "'" in str(simplified_error)
                    else str(simplified_error).split('No module named ')[-1].strip()
                )
        ...
        try:
            ...
            print(
                'Remember that the goal is to extract the API definition, not to run the business logic.'
            )
            return False
    finally:
