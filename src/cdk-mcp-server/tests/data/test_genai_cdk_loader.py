...
def test_get_construct_map():
    ...
    assert 'Amazon Bedrock constructs for agents, knowledge bases, and more' in construct_map['bedrock']
    assert 'Amazon OpenSearch Serverless constructs for vector search' in construct_map['opensearchserverless']
...
def test_get_genai_cdk_overview_success(mock_file, mock_join, mock_dirname):
    ...
    mock_file.assert_called_with('/mock/path/static/genai_cdk/bedrock/overview.md', 'r', encoding='utf-8')
...
def test_list_available_sections_success(mock_walk, mock_exists, mock_join, mock_dirname):
    ...
    mock_walk.return_value = [
        ('/mock/path/static/genai_cdk/bedrock/agent', [], ['overview.md', 'usage.md', 'configuration.md']),
        ('/mock/path/static/genai_cdk/bedrock/agent/actiongroups', [], ['overview.md', 'usage.md']),
    ]
...
def test_get_genai_cdk_construct_section_success(mock_file, mock_join, mock_dirname):
    ...
    mock_file.assert_called_with('/mock/path/static/genai_cdk/bedrock/agent/usage.md', 'r', encoding='utf-8')
...
@patch('os.listdir')
@patch('os.walk')
def test_list_available_constructs_success(mock_walk, mock_listdir, mock_exists, mock_join, mock_dirname):
    ...
    agent_subdir_constructs = [c for c in constructs if c['type'] == 'bedrock' and 'agent' in c['name'].lower()]
    knowledgebase_subdir_constructs = [c for c in constructs if c['type'] == 'bedrock' and 'knowledgebase' in c['name'].lower()]
    ...
    assert len(knowledgebase_subdir_constructs) > 0, "No constructs found from knowledgebase subdirectory"
