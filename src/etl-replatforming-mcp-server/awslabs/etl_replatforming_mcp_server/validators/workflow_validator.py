...
class WorkflowValidator:
    ...
    def validate(self, workflow: FlexWorkflow) -> Dict[str, Any]:
        ...
        schedule_errors = self.validate_schedule_type(workflow)
        condition_errors = self.validate_dependency_conditions(workflow)
        error_handling_errors = self.validate_error_handling(workflow)
        ...
        all_errors = (missing_fields + dependency_errors + task_type_errors + 
                     schedule_errors + condition_errors + error_handling_errors + 
                     command_errors + timeout_errors + retry_errors)
        
        return {
            "is_complete": len(all_errors) == 0,
            "missing_fields": missing_fields,
            "dependency_errors": dependency_errors,
            "completion_percentage": self._calculate_completion_percentage(workflow, all_errors)
        }
    

    def _calculate_completion_percentage(self, workflow: FlexWorkflow, missing_fields: List[str]) -> float:
    ...
    def validate_task_commands(self, workflow: FlexWorkflow) -> List[str]:
        ...
        for task in workflow.tasks:
            if task.command and task.command.strip() == '?':
                ...
                sql_keywords = ['SELECT', 'INSERT', 'UPDATE', 'DELETE', 'CREATE', 'DROP', 'ALTER', 'CALL']
    ...
    def validate_schedule_type(self, workflow: FlexWorkflow) -> List[str]:
        ...
        if workflow.schedule:
            ...
            elif workflow.schedule.type == "rate":
        ...
        for task in workflow.tasks:
            for dep in task.depends_on:
                if dep.condition not in valid_conditions:
                    errors.append(f"dependency_invalid_condition_{dep.condition}")
