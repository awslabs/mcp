...
This MCP server provides expert guidance on cell-based architecture patterns
based on the AWS Well-Architected whitepaper "Reducing the Scope of Impact
with Cell-Based Architecture".

## Available Tools

### query-cell-concepts
Query cell-based architecture concepts with progressive complexity levels
...
async def query_cell_concepts(
    concept: str = Field(
        ...,
        description=(
            'The cell-based architecture concept to query '
            '(e.g., "cell isolation", "fault tolerance", "blast radius")'
        ),
    ),
    ...
    Use this tool when a user asks to explain or define a concept related to
    cell-based architecture. Returns a markdown-formatted explanation tailored
    to the requested experience level for progressive learning.
    """
    try:
        logger.info(
            f'Querying concept: {concept}, level: {detail_level}, section: {whitepaper_section}'
        )
        knowledge = CellBasedArchitectureKnowledge()
        return knowledge.get_concept_explanation(concept, detail_level, whitepaper_section)
...
async def get_implementation_guidance(
    stage: Literal['planning', 'design', 'implementation', 'monitoring'] = Field(
        ...,
        description=(
            'Implementation stage: planning (initial planning), design (architecture design), '
            'implementation (coding/deployment), monitoring (observability)'
        ),
    ),
    aws_services: List[str] = Field(
        default_factory=list,
        description=(
            'AWS services to consider for implementation '
            '(e.g., ["lambda", "dynamodb", "api-gateway"])'
        ),
    ),
    ...
    Use this tool when a user needs actionable guidance for a specific stage
    of implementing cell-based architecture. Returns markdown-formatted
    guidance with content appropriate for the user's experience level.
    """
    try:
        logger.info(
            f'Getting guidance for stage: {stage}, level: {experience_level}, '
            f'services: {aws_services}'
        )
        knowledge = CellBasedArchitectureKnowledge()
...
async def analyze_cell_design(
    ...
    focus_areas: List[str] = Field(
        default_factory=list,
        description=(
            'Specific areas to focus the analysis on '
            '(e.g., ["isolation", "scalability", "fault_tolerance"])'
        ),
    ),
) -> str:
    """Analyze cell-based architecture designs and provide comprehensive recommendations.

    Use this tool to evaluate an architecture description against cell-based
    architecture principles. Returns identified strengths, recommendations
    for improvement, and a compliance score.
    """
    try:
        logger.info(f'Analyzing cell design, focus areas: {focus_areas}')

        response = '# Cell-Based Architecture Design Analysis\n\n'
...
async def validate_architecture(
    ...
    Use this tool to check a design document for key cell-based architecture
    criteria such as isolation and fault tolerance. Returns pass/fail per
    criterion plus an overall compliance score.
    """
    try:
        logger.info('Validating architecture')

        response = '# Cell-Based Architecture Validation\n\n'

        design_lower = design_document.lower()
        validation_results = {
            'isolation': 'PASS' if 'isolation' in design_lower else 'FAIL',
            'fault_tolerance': 'PASS' if 'fault' in design_lower else 'FAIL',
        }
...
@mcp.resource(
    uri='resource://cell-architecture-guide',
    name='CellArchitectureGuide',
...
async def cell_architecture_guide() -> str:
    ...
    try:
        guide = {
            ...
            'description': (
                'Comprehensive guide organized by AWS Well-Architected whitepaper sections'
            ),
            'sections': {},
        }
...
@mcp.resource(
    uri='resource://implementation-patterns',
    name='ImplementationPatterns',
    mime_type='application/json',
)
async def implementation_patterns() -> str:
    """Return common implementation patterns and AWS service integration examples."""
    try:
        patterns = {
...
def main() -> None:
    ...
    Environment Variables:
        AWS_REGION: AWS region for service calls (default: us-east-1).
            Accepted for forward compatibility; not currently consumed by any tool.
        AWS_PROFILE: AWS profile to use for authentication. Accepted for forward
            compatibility; not currently consumed by any tool.
    ...
    try:
        ...
        logger.info('Starting Cell-Based Architecture MCP Server')
