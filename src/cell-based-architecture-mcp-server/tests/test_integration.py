...
class TestMCPProtocolCompliance:
    ...
    def test_server_resources_registration(self):
        ...
        expected_resources = [cell_architecture_guide, implementation_patterns, best_practices]
...
class TestEndToEndWorkflows:
    ...
    async def test_implementation_guidance_workflow(self):
        ...
        result = await get_implementation_guidance(
            stage='design', aws_services=['lambda', 'dynamodb'], experience_level='intermediate'
        )
...
class TestTimeoutHandling:
    ...
    async def test_tool_timeout_handling(self):
        ...
        result = await query_cell_concepts(concept='test concept', detail_level='beginner')
    ...
    async def test_resource_error_recovery(self):
        ...
        with patch(
            'awslabs.cell_based_architecture_mcp_server.knowledge.CellBasedArchitectureKnowledge.WHITEPAPER_SECTIONS',
            {},
        ):
            result = await cell_architecture_guide()
...
class TestAWSServiceMocking:
    ...
    async def test_aws_region_configuration(self):
        ...
        with patch.dict(os.environ, {'AWS_REGION': 'us-west-2'}):
            result = await query_cell_concepts(
                concept='aws integration', detail_level='intermediate'
            )
    ...
    async def test_aws_profile_handling(self):
        ...
        with patch.dict(os.environ, {'AWS_PROFILE': 'test-profile'}):
            result = await query_cell_concepts(concept='aws services', detail_level='expert')
    ...
    async def test_no_aws_credentials_handling(self):
        ...
        result = await query_cell_concepts(
            concept='cell design principles', detail_level='beginner'
        )
...
class TestConcurrentRequests:
    ...
    async def test_concurrent_tool_calls(self):
        ...
        tasks = []
        for i in range(5):
            task = query_cell_concepts(concept=f'concept {i}', detail_level='intermediate')
            tasks.append(task)
