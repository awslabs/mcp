...
@pytest.mark.asyncio
async def test_import_csv_to_table_non_csv_file():
    """Test import_csv_to_table returns error when non-CSV file is provided."""
    from awslabs.s3_tables_mcp_server.file_processor import csv as csv_mod
    ...
    namespace = 'testns'
    table_name = 'testtable'
    s3_url = 's3://bucket/file.txt'  # Non-CSV file
    uri = 'https://s3tables.us-west-2.amazonaws.com/iceberg'
    catalog_name = 's3tablescatalog'
    rest_signing_name = 's3tables'
    rest_sigv4_enabled = 'true'
    ...
    assert 'Only .csv files are supported' in result['error']


@pytest.mark.asyncio
async def test_import_csv_to_table_missing_columns():
    """Test import_csv_to_table returns error when CSV file is missing required columns."""
    from awslabs.s3_tables_mcp_server.file_processor import csv as csv_mod
    ...
    mock_table = MagicMock()
    mock_iceberg_schema = MagicMock()
    # Iceberg table has 3 columns: col1, col2, col3
    mock_iceberg_schema.fields = [
        MagicMock(name='col1'),
        MagicMock(name='col2'),
        MagicMock(name='col3'),
    ]
    mock_iceberg_schema.fields[0].name = 'col1'
    mock_iceberg_schema.fields[1].name = 'col2'
    mock_iceberg_schema.fields[2].name = 'col3'
    mock_table.schema.return_value = mock_iceberg_schema
    mock_catalog.load_table.return_value = mock_table
    ...
    # CSV file only has 2 columns: col1, col2 (missing col3)
    mock_csv_table = MagicMock()
    mock_csv_table.schema.names = ['col1', 'col2']
    mock_csv_table.num_rows = 42
    ...
    with (
        ...
        assert result['status'] == 'error'
        assert 'CSV file is missing required columns' in result['error']
        assert 'col3' in result['error']
        mock_load_catalog.assert_called_once()
        mock_get_s3_client.assert_called_once()
        mock_read_csv.assert_called_once()
        # Table.append should not be called since validation fails
