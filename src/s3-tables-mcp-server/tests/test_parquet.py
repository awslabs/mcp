...
@pytest.mark.asyncio
async def test_import_parquet_to_table_missing_columns():
    """Test import_parquet_to_table returns error when Parquet file is missing required columns."""
    from awslabs.s3_tables_mcp_server.file_processor import parquet as parquet_mod
    ...
    # Parquet file only has 2 columns: col1, col2 (missing col3)
    mock_parquet_table = MagicMock()
    mock_parquet_table.schema.names = ['col1', 'col2']
    mock_parquet_table.num_rows = 42
    ...
    with (
        ...
        assert result['status'] == 'error'
        assert 'Parquet file is missing required columns' in result['error']
