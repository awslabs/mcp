...
async def test_parse_checkov_json_output_invalid():
    ...
    valid_but_empty = json.dumps({"results": {"failed_checks": []}, "summary": {"passed": 0, "failed": 0}})
...
async def test_run_checkov_scan_with_absolute_path(temp_terraform_dir):
    ...
    with patch('subprocess.run', return_value=mock_result):
        with patch('awslabs.terraform_mcp_server.impl.tools.run_checkov_scan._ensure_checkov_installed', return_value=True):
            # Call the function
