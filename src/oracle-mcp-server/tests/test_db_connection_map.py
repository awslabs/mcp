...
def test_get_keys_omits_secret_arn(conn_map):
    """get_keys must not expose secret_arn (operator-only; not surfaced to the model)."""
    conn = MagicMock()
    conn.service_name = 'ORCL'
    conn.sid = None
    conn.secret_arn = 'arn:aws:secretsmanager:us-east-1:123:secret:x'  # pragma: allowlist secret
    conn_map.set(ConnectionMethod.ORACLE_PASSWORD, 'i1', 'e1', 'd1', conn, 1521)
    keys = conn_map.get_keys()
    assert 'secret_arn' not in keys[0]
...
def test_endpoint_only_fallback_matches_regardless_of_stored_target_name(conn_map, mock_conn):
    """Endpoint-only query finds a connection stored under a non-empty target_name.

    Regression guard: a connection is always stored with a target_name (service_name/
    sid), so the instance-omitted fallback must not require target_name=='' to match.
    """
    conn_map.set(
        ConnectionMethod.ORACLE_PASSWORD,
        'my-instance',
    ...
    # Caller supplies only the endpoint (instance_identifier defaults to it) and omits
    # target_name — the connection was stored under 'my-instance'/'ORCL'.
    got = conn_map.get(
        ConnectionMethod.ORACLE_PASSWORD,
        'my-instance.rds.amazonaws.com',
        'my-instance.rds.amazonaws.com',
...
def test_endpoint_only_fallback_ambiguous_returns_none(conn_map):
    """Endpoint-only query with multiple same-endpoint tenants returns None, not an arbitrary one.

    Two PDBs on one CDB share (method, db_endpoint, database, port) but differ by
    target_name. An endpoint-only lookup that omits target_name must not silently pick
    one — it could hit the wrong tenant.
    """
    conn_map.set(
        ConnectionMethod.ORACLE_PASSWORD,
        'cdb',
