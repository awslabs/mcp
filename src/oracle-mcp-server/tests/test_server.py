...
def test_parse_instance_identifier_odb_arn_partition():
    """A partitioned ODB ARN (aws-us-gov) is recognized via the service field."""
    arn = 'arn:aws-us-gov:odb:us-gov-west-1:123456789012:autonomous-database/adb_gov'
    service_type, resolved_id = _parse_instance_identifier(arn)
    assert service_type == 'odb'
    assert resolved_id == arn


def test_parse_instance_identifier_odb_non_autonomous_arn_raises():
    """A non-Autonomous ODB ARN (e.g. cloud-vm-cluster) raises a clear error."""
    arn = 'arn:aws:odb:us-east-1:123456789012:cloud-vm-cluster/cvc_xxxxxxxx'
    with pytest.raises(ValueError, match='expected an Autonomous Database ARN'):
        _parse_instance_identifier(arn)
...
def test_internal_create_connection_odb_autonomous_db(mocker):
    ...
    assert isinstance(conn, OracledbPoolConnection)
    # The private endpoint DNS name is used (not the IP) so TLS 'require' can validate
    # the server certificate.
    assert conn.host == 'adb-private.us-east-1.example.com'
    assert (
        conn.secret_arn
        == 'arn:aws:secretsmanager:us-east-1:123:secret:adb-admin'  # pragma: allowlist secret
    )
    assert response['db_endpoint'] == 'adb-private.us-east-1.example.com'
...
def test_internal_create_connection_odb_arn(mocker):
    ...
    mock_odb.get_autonomous_database.return_value = {
        'autonomousDatabase': {
            'privateEndpoint': 'adb-arn.us-east-1.example.com',
    ...
    assert isinstance(conn, OracledbPoolConnection)
    assert response['db_endpoint'] == 'adb-arn.us-east-1.example.com'
...
def test_internal_create_connection_odb_resolves_endpoint_before_cache_lookup(mocker):
    ...
    mock_odb = MagicMock()
    mock_odb.get_autonomous_database.return_value = {
        'autonomousDatabase': {'privateEndpoint': 'adb-x.us-east-1.example.com'}
    }
    mocker.patch('boto3.client', return_value=mock_odb)
    ...
    # The cache lookup must use the resolved endpoint, not '' (db_endpoint arg is 3rd positional).
    assert get_spy.call_args.args[2] == 'adb-x.us-east-1.example.com'
...
def test_internal_create_connection_odb_with_explicit_secret(mocker):
    ...
    mock_odb.get_autonomous_database.return_value = {
        'autonomousDatabase': {
            'privateEndpoint': 'adb-secrettest.us-east-1.example.com',
        }
    }
    mocker.patch('boto3.client', return_value=mock_odb)
    server_config.configured_default_secret_arn = (
    ...
    assert conn.host == 'adb-secrettest.us-east-1.example.com'
    assert (
        conn.secret_arn
        == 'arn:aws:secretsmanager:us-east-1:123:secret:my-custom-secret'  # pragma: allowlist secret
    )
...
def test_resolve_odb_private_endpoint_uses_dns_name(mocker):
    """The private endpoint DNS name (privateEndpoint) is used, not the IP."""
...
def test_resolve_odb_private_endpoint_missing_dns_name_raises(mocker):
    """When privateEndpoint (the DNS name) is absent, resolution raises — no IP fallback."""
    from awslabs.oracle_mcp_server.server import _resolve_odb_private_endpoint

    mock_odb = MagicMock()
    mock_odb.get_autonomous_database.return_value = {
        'autonomousDatabase': {'privateEndpointIp': '10.0.1.50'}
    }
    mocker.patch('boto3.client', return_value=mock_odb)

    with pytest.raises(ValueError, match='no private endpoint'):
        _resolve_odb_private_endpoint('us-east-1', 'adb_x')
...
def test_main_rejects_both_service_name_and_sid(mocker):
    """Supplying both --service_name and --sid exits at startup (sole enforcement point)."""
    from awslabs.oracle_mcp_server import server as server_module

    mocker.patch(
        'sys.argv',
        ['prog', '--db_endpoint', 'ep', '--service_name', 'ORCL', '--sid', 'ORCL'],
    )
    mocker.patch.object(server_module, 'mcp')
