...
### Features
- Support for Oracle Database@AWS (ODB): Autonomous (ADB-S / ADB-D) and Exadata Dedicated databases. For Autonomous Databases the private endpoint **DNS name** is auto-resolved via `odb:GetAutonomousDatabase`, so the default `--ssl_encryption require` validates the server certificate
