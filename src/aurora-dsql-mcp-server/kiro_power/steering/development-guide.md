...
- **PREFER UUIDs for primary keys** - Use `gen_random_uuid()` for distributed writes; UUIDs avoid coordination and are recommended as the default identifier type
  ...
  - **SERIAL is not supported** - Use `GENERATED { ALWAYS | BY DEFAULT } AS IDENTITY` instead
...
- **SERIAL is not supported** — use `GENERATED { ALWAYS | BY DEFAULT } AS IDENTITY` for auto-incrementing columns

#### Choosing a CACHE Size

CACHE must be specified explicitly when creating sequences or identity columns. Supported values are 1 or >= 65536.
