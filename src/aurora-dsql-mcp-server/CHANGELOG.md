...
- Parse SQL with `pglast` before applying function and read-only policy checks, so PostgreSQL quoted and Unicode-escaped identifiers are evaluated as the database interprets them. The parser guard also preserves psycopg placeholders and supports Aurora DSQL asynchronous DDL syntax during validation.
