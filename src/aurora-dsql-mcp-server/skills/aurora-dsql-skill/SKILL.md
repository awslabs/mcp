...
- MUST serialize arrays into a single-column representation; PREFER `JSONB` (operators work directly), with `TEXT` as a MAY for opaque columns; ASK the user. For `JSONB` arrays, expand at query time with `jsonb_array_elements_text(data)`
