...
- WARNING: Fix applied but may require application code changes (e.g., removed foreign key)
- ERROR (unfixable): Cannot be auto-fixed, requires manual intervention

## Example
Input: "CREATE TABLE t (id SERIAL PRIMARY KEY, data JSON);"
Output with fix=True returns fixed_sql: "CREATE TABLE t (id BIGINT PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY (CACHE 1), data TEXT);"
""",
)
async def dsql_lint(
