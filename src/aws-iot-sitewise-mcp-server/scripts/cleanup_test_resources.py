...
have been left behind from failed tests.
...
def main():
    ...
    logger.info(
        "Starting cleanup for region: {}, prefix: {}".format(args.region, args.prefix)
    )
    ...
    try:
        ...
        if not args.force:
            response = input(
                "Delete {} orphaned test resources? (y/N): ".format(total_found)
            )
