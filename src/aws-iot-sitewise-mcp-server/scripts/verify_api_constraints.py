...
try:
    from site_wise_mcp_poc.validation import (
        SiteWiseQuotas, validate_asset_name, validate_asset_model_id,
        validate_max_results, validate_property_alias, ValidationError
    )
...
class APIConstraintVerifier:
            ...
            try:
                ...
                if 'expected_error_code' in test:
                    if result.get('error_code') != test['expected_error_code']:
                        print(f"FAIL: {test['name']}: Wrong error code: {result.get('error_code')}")
                        failed += 1
                        continue
    ...
    def generate_report(self):
        ...
        if total_failed > 0:
            ...
            print("   • Run tests with real AWS credentials")
