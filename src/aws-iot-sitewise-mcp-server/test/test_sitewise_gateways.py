...
class TestSiteWiseGateways:
    ...
    def test_describe_gateway_success(self, mock_boto_client):
        ...
        mock_response = {
            ...
            "gatewayArn": "arn:aws:iotsitewise:us-east-1:123456789012:gateway/gateway-123",
            "gatewayPlatform": {
                "greengrassV2": {"coreDeviceThingName": "test-core-device"}
            },
    ...
    def test_list_gateways_with_next_token(self, mock_boto_client):
        ...
        result = list_gateways(
            region="us-west-2", next_token="prev-token", max_results=100
        )

        assert result["success"] is True
        mock_client.list_gateways.assert_called_once_with(
            maxResults=100, nextToken="prev-token"
        )
    ...
    def test_describe_time_series_with_all_params(self, mock_boto_client):
        ...
        mock_response[
            "timeSeriesLastUpdateDate"
        ].isoformat.return_value = "2023-01-02T00:00:00Z"
        mock_client.describe_time_series.return_value = mock_response

        # Test with alias only
        result = describe_time_series(
            alias="/company/plant/temperature", region="us-west-2"
        )
    ...
    def test_delete_time_series_with_all_params(self, mock_boto_client):
        ...
        mock_client.reset_mock()
        result = delete_time_series(
            alias="/company/plant/temperature", region="us-east-1"
        )
    ...
    def test_all_functions_client_error_handling(self, mock_boto_client):
        ...
        mock_client.describe_gateway.side_effect = ClientError(
            error_response, "DescribeGateway"
        )
        ...
        mock_client.update_gateway.side_effect = ClientError(
            error_response, "UpdateGateway"
        )
        result = update_gateway(
            gateway_id="gateway-123", gateway_name="Updated Gateway"
        )
        assert result["success"] is False
        assert result["error_code"] == "InternalFailureException"
        ...
        mock_client.disassociate_time_series_from_asset_property.side_effect = (
            ClientError(error_response, "DisassociateTimeSeriesFromAssetProperty")
        )
        result = disassociate_time_series_from_asset_property(
            alias="/company/plant/temperature",
            asset_id="asset-123",
