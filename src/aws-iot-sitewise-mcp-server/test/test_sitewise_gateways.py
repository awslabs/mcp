...
class TestSiteWiseGateways:
    ...
    @patch("awslabs.aws_iot_sitewise_mcp_server.tools.sitewise_gateways.boto3.client")
    def test_create_gateway_success(self, mock_boto_client):
        ...
        mock_response = {
            ...
            "gatewayArn": "arn:aws:iotsitewise:us-east-1:<account-id>:gateway/gateway-123",
        }
    ...
    def test_describe_time_series_success(self, mock_boto_client):
        ...
        mock_response = {
            ...
            "timeSeriesArn": "arn:aws:iotsitewise:us-east-1:<account-id>:time-series/ts-789",
        }
    ...
    def test_create_gateway_with_tags(self, mock_boto_client):
        ...
        gateway_platform = {
            "greengrass": {"groupArn": "arn:aws:greengrass:us-east-1:<account-id>:group/test-group"}
        }
        ...
        result = create_gateway(
            gateway_name="Test Gateway", gateway_platform=gateway_platform, region="us-east-1"
        )
    ...
    def test_describe_time_series_with_all_params(self, mock_boto_client):
        ...
        assert result["success"] is True
        mock_client.describe_time_series.assert_called_once_with(alias="/company/plant/temperature")
        ...
        assert result["success"] is True
        mock_client.describe_time_series.assert_called_once_with(
            alias="/company/plant/temperature", assetId="asset-123", propertyId="prop-456"
        )
    ...
    def test_all_functions_client_error_handling(self, mock_boto_client):
        ...
        mock_client.create_gateway.side_effect = ClientError(error_response, "CreateGateway")
        result = create_gateway(
            gateway_name="Test Gateway", gateway_platform={"greengrass": {"groupArn": "arn:test"}}
        )
        assert result["success"] is False
        ...
        result = describe_gateway_capability_configuration(
            gateway_id="gateway-123", capability_namespace="iotsitewise:opcuacollector:1"
        )
        assert result["success"] is False
        assert result["error_code"] == "InternalFailureException"
        ...
        result = associate_time_series_to_asset_property(
            alias="/company/plant/temperature", asset_id="asset-123", property_id="prop-456"
        )
        assert result["success"] is False
        assert result["error_code"] == "InternalFailureException"
        ...
        mock_client.delete_time_series.side_effect = ClientError(error_response, "DeleteTimeSeries")
