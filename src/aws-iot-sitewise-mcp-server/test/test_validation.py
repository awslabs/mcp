...
class TestValidation:
    ...
    def test_validate_asset_id_invalid(self):
        ...
        with pytest.raises(
            ValidationError, match="Asset ID cannot exceed 36 characters"
        ):
            validate_asset_id("a" * 37)

        with pytest.raises(
            ValidationError, match="Asset ID contains invalid characters"
        ):
    ...
    def test_validate_asset_model_id_invalid(self):
        ...
        with pytest.raises(
            ValidationError, match="Asset model ID cannot exceed 36 characters"
        ):
    ...
    def test_validate_asset_name_invalid(self):
        ...
        with pytest.raises(
            ValidationError, match="Asset name cannot exceed 256 characters"
        ):
            validate_asset_name("a" * 257)

        with pytest.raises(
            ValidationError, match="Asset name contains invalid characters"
        ):
    ...
    def test_validate_property_alias_invalid(self):
        ...
        with pytest.raises(
            ValidationError, match="Property alias cannot exceed 2048 characters"
        ):
    ...
    def test_validate_asset_model_properties_invalid(self):
        ...
        # Too many properties
        with pytest.raises(
            ValidationError, match="Cannot have more than 200 properties"
        ):
            validate_asset_model_properties(
                [
                    {
        ...
        with pytest.raises(ValidationError, match="Property must have a name"):
            validate_asset_model_properties(
                [{"dataType": "DOUBLE", "type": {"measurement": {}}}]
            )
    ...
    def test_validate_batch_entries_invalid(self):
        ...
        with pytest.raises(
            ValidationError, match="Entry .* missing required 'entryId'"
        ):
    ...
    def test_validate_gateway_platform_valid(self):
        ...
        validate_gateway_platform(
            {
                "greengrass": {
                    "groupArn": "arn:aws:greengrass:us-east-1:123456789012:group/test"
                }
            }
        )
        validate_gateway_platform(
            {"greengrassV2": {"coreDeviceThingName": "test-device"}}
        )
    ...
    def test_validate_gateway_platform_invalid(self):
        ...
        with pytest.raises(
            ValidationError, match="Gateway platform must specify either"
        ):
            validate_gateway_platform({"invalid": "config"})

        with pytest.raises(
            ValidationError, match="Greengrass configuration must include"
        ):
            validate_gateway_platform({"greengrass": {}})

        with pytest.raises(
            ValidationError, match="Greengrass V2 configuration must include"
        ):
