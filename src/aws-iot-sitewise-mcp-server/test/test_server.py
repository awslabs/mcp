...
from awslabs.aws_iot_sitewise_mcp_server.server import main, run_server, signal_handler
...
class TestServer:
    ...
    async def test_run_server_setup(self, mock_get_version, mock_fastmcp, mock_task_group):
        ...
        # Asset tools: 8, Asset model tools: 7, Data tools: 9, Gateway tools: 12, Access tools: 11
        # Total expected: 47 tools
        assert mock_mcp_instance.add_tool.call_count == 42
    ...
    async def test_run_server_tool_categories(
        ...
        tool_names = [call[0][1] for call in tool_calls]  # Second argument is tool name
    ...
    @patch("awslabs.aws_iot_sitewise_mcp_server.server.get_package_version")
    @pytest.mark.asyncio
    async def test_run_server_error_handling(self, mock_get_version, mock_fastmcp, mock_task_group):
