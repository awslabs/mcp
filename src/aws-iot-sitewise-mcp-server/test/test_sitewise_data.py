...
class TestSiteWiseData:
    ...
    def test_batch_put_asset_property_value_success(self, mock_boto_client):
        ...
        mock_client.batch_put_asset_property_value.assert_called_once_with(
            entries=entries
        )
    ...
    def test_batch_get_asset_property_value_success(self, mock_boto_client):
        ...
        entries = [
            {"entryId": "entry1", "assetId": "asset-123", "propertyId": "prop-456"}
        ]
    ...
    def test_batch_get_asset_property_value_history_success(self, mock_boto_client):
        ...
        result = batch_get_asset_property_value_history(
            entries=entries, region="us-east-1"
        )
    ...
    def test_client_error_handling(self, mock_boto_client):
        ...
        error_response = {
            "Error": {"Code": "InvalidRequestException", "Message": "Invalid query"}
        }
        mock_client.execute_query.side_effect = ClientError(
            error_response, "ExecuteQuery"
        )

        result = execute_query(
            query_statement="SELECT * FROM invalid_table", region="us-east-1"
        )
    ...
    @patch("awslabs.aws_iot_sitewise_mcp_server.tools.sitewise_data.boto3.client")
    def test_get_interpolated_asset_property_values_with_all_params(
    ...
    @patch("awslabs.aws_iot_sitewise_mcp_server.tools.sitewise_data.boto3.client")
    def test_batch_get_asset_property_value_history_with_next_token(
        self, mock_boto_client
    ):
    ...
    @patch("awslabs.aws_iot_sitewise_mcp_server.tools.sitewise_data.boto3.client")
    def test_batch_get_asset_property_aggregates_with_next_token(
        self, mock_boto_client
    ):
    ...
    def test_execute_query_without_optional_params(self, mock_boto_client):
        ...
        mock_client.execute_query.assert_called_once_with(
            queryStatement=query, maxResults=100
        )
