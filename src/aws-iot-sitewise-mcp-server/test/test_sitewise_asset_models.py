...
class TestSiteWiseAssetModels:
    ...
    def test_create_asset_model_success(self, mock_boto_client):
        ...
        mock_response = {
            ...
            "assetModelArn": "arn:aws:iotsitewise:us-east-1:<account-id>:asset-model/test-model-123",
        ...
        assert (
            ...
            == "arn:aws:iotsitewise:us-east-1:<account-id>:asset-model/test-model-123"
        )
    ...
    def test_update_asset_model_success(self, mock_boto_client):
        ...
        result = update_asset_model(
            asset_model_id="test-model-123", asset_model_name="Updated Model", region="us-east-1"
        )
    ...
    def test_asset_model_validation_errors(self, mock_boto_client):
        ...
        # Test various validation failures that happen during parameter validation
    ...
    def test_client_error_handling(self, mock_boto_client):
        ...
        error_response = {"Error": {"Code": "ConflictException", "Message": "Model already exists"}}
        mock_client.create_asset_model.side_effect = ClientError(error_response, "CreateAssetModel")
    ...
    def test_create_asset_model_validation_errors(self):
        ...
        # Test too many tags
        too_many_tags = {f"key{i}": f"value{i}" for i in range(51)}  # Exceeds 50 tag limit
        result = create_asset_model(
            asset_model_name="Test Model",
            tags=too_many_tags,
        )
    ...
    def test_describe_asset_model_with_all_params(self, mock_boto_client):
        ...
        mock_client.describe_asset_model.assert_called_once_with(
            assetModelId="test-model-123", assetModelVersion="ACTIVE", excludeProperties=True
        )

    def test_list_asset_models_validation_errors(self):
        """Test list asset models validation error cases."""
        # Test next token too long
        result = list_asset_models(next_token="x" * 4097)  # Exceeds 4096 character limit
        assert result["success"] is False
    ...
    def test_delete_asset_model_with_client_token(self, mock_boto_client):
        ...
        result = delete_asset_model(
            asset_model_id="test-model-123", region="us-west-2", client_token="delete-token"
        )
    ...
    def test_list_asset_model_properties_validation_errors(self):
        ...
        result = list_asset_model_properties(
            asset_model_id="test-model-123", next_token="x" * 4097  # Exceeds 4096 character limit
        )
        assert result["success"] is False
        assert "Next token too long" in result["error"]
    ...
    def test_all_functions_client_error_handling(self, mock_boto_client):
        ...
        error_response = {
            "Error": {"Code": "InternalFailureException", "Message": "Internal server error"}
        }
        ...
        mock_client.update_asset_model.side_effect = ClientError(error_response, "UpdateAssetModel")
        result = update_asset_model(asset_model_id="test-123", asset_model_name="Updated")
        assert result["success"] is False
        assert result["error_code"] == "InternalFailureException"

        # Test delete_asset_model error handling
        mock_client.delete_asset_model.side_effect = ClientError(error_response, "DeleteAssetModel")
