...
def execute_training_action(
    ...
    try:
        ...
        if label_bucket_name and label_s3_prefix:
            ...
            raise CustomValidationError("Both label_bucket_name and label_s3_prefix must be provided together for supervised learning")
        
        # Create model evaluation configuration if all evaluation parameters are provided
        model_evaluation_config = None
        evaluation_params = [evaluation_start_time, evaluation_end_time, evaluation_bucket_name, evaluation_s3_prefix]
        ...
        if all(evaluation_params_provided):
            ...
            raise CustomValidationError("All four evaluation parameters (evaluation_start_time, evaluation_end_time, evaluation_bucket_name, evaluation_s3_prefix) must be provided together for pointwise diagnostics")
        ...
        if metrics_bucket_name and metrics_s3_prefix:
            ...
            # If only one is provided, raise an error
            raise CustomValidationError("Both metrics_bucket_name and metrics_s3_prefix must be provided together for model metrics")
        ...
        if training_mode == 'START_RETRAINING_SCHEDULER':
            retraining_config = RetrainingConfiguration(
                ...
                promotion=promotion or "SERVICE_MANAGED",  # Default to SERVICE_MANAGED if not provided
