    ...
    try:
        ...
        if asset_model_description and len(asset_model_description) > 2048:
            raise ValidationError(
                "Asset model description cannot exceed 2048 characters"
            )
        ...
        if asset_model_hierarchies and len(asset_model_hierarchies) > 10:
            raise ValidationError(
                "Cannot have more than 10 hierarchies per asset model"
            )
...
def list_asset_models(
    ...
    Args:
        ...
        max_results: The maximum number of results to return (
            1-250,
            default: 50)        asset_model_types: The type of asset model (
            ASSET_MODEL,
            COMPONENT_MODEL)
    Returns:
...
def create_asset_model_composite_model(
    ...
    try:
        ...
        return {
            ...
            "asset_model_composite_model_path": response[
...
describe_asset_model_tool = Tool.from_function(
    ...
    description=(
        "Retrieve detailed information about an AWS IoT SiteWise " "asset model."
    ),
)
...
list_asset_models_tool = Tool.from_function(
    ...
    description=(
        "Retrieve a paginated list of asset model summaries from AWS " "IoT SiteWise."
    ),
)
...
create_asset_model_composite_model_tool = Tool.from_function(
    ...
    description=(
        "Create a composite model for an existing asset model in AWS " "IoT SiteWise."
    ),
)
