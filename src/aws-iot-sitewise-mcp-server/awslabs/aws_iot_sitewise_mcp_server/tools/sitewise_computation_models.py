...
def _determine_computation_model_configuration_type(
    ...
    try:
        ...
        if configuration_type is not None:
            if configuration_type.lower() in ['asset_model_level', 'asset model level configuration']:
                return {
                    "success": True,
    ...
    **Smart Optimization**: If you know the configuration type, provide it via the `configuration_type`
    ...
    try:
        ...
        if not is_asset_model_level and (resolve_to_resource_id or resolve_to_resource_type):
            result["info"] = "Resolve parameters ignored for Asset Level Configuration (already tied to specific assets)"
        elif is_asset_model_level and not (resolve_to_resource_id or resolve_to_resource_type):
            result["info"] = "Asset Model Level Configuration - consider providing resolve parameters for specific asset context"
...
def list_computation_model_resolve_to_resources(
    ...
    try:
        ...
        return {
            "success": True,
            "computationModelResolveToResourceSummaries": response.get("computationModelResolveToResourceSummaries", []),
            "nextToken": response.get("nextToken"),
        }
