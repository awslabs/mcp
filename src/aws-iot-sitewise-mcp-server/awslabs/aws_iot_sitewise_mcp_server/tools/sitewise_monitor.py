...
def list_access_policies(
    identity_type: Optional[str] = Field(
        None, description='The type of identity (USER, GROUP, IAM)'
    ),
    identity_id: Optional[str] = Field(None, description='The ARN of the identity'),
    resource_type: Optional[str] = Field(
        None, description='The type of resource (PORTAL, PROJECT)'
    ),
...
def _convert_position_and_size(x: int, y: int, width: int, height: int) -> Dict[str, int]:
    ...
    """
    return {'x': x * 4, 'y': y * 4, 'w': width * 4, 'h': height * 4}


def _create_grafana_target(
    metric: Dict[str, Any],
    ref_id: str,
    datasource_uid: str,
    region: str,
    query_type: str = 'PropertyValueHistory',
) -> Dict[str, Any]:
    """Create a Grafana target from a classic monitor metric."""
    return {
        'assetIds': [metric['assetId']],
        'clientCache': True,
        'datasource': {'type': 'grafana-iot-sitewise-datasource', 'uid': datasource_uid},
        'flattenL4e': True,
...
def _convert_widget_to_panel(
    widget: Dict[str, Any], datasource_uid: str, region: str
) -> Dict[str, Any]:
    ...
    if widget_type == 'sc-line-chart':
        ...
        panel['type'] = 'timeseries'
        panel['fieldConfig'] = {'defaults': {'custom': {'drawStyle': 'points'}}}
        ...
        panel['type'] = 'table'
        panel['transformations'] = [{'id': 'reduce', 'options': {'reducers': ['last']}}]
...
def convert_monitor_dashboard_to_grafana(
    dashboard_definition: str = Field(
        ..., description='JSON string of the classic monitor dashboard definition'
    ),
    dashboard_name: Optional[str] = Field(
        None, description='Name for the Grafana dashboard (defaults to extracted from definition)'
    ),
    ...
    dashboard_uid: Optional[str] = Field(
        None, description='Custom UID for the Grafana dashboard (auto-generated if not provided)'
    ),
) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            'conversion_summary': {
                ...
                'widget_types_converted': list(
                    {widget['type'] for widget in dashboard_def.get('widgets', [])}
                ),
                'dashboard_name': dashboard_name,
                'dashboard_uid': dashboard_uid,
                'total_metrics': sum(
                    len(widget.get('metrics', [])) for widget in dashboard_def.get('widgets', [])
                ),
            },
        }
...
def convert_dashboard_by_id(
    dashboard_id: str = Field(
        ..., description='The ID of the classic monitor dashboard to convert'
    ),
    datasource_uid: str = Field(..., description='UID of the Grafana IoT SiteWise datasource'),
    dashboard_name: Optional[str] = Field(
        None, description='Name for the Grafana dashboard (defaults to original dashboard name)'
    ),
    dashboard_uid: Optional[str] = Field(
        None, description='Custom UID for the Grafana dashboard (auto-generated if not provided)'
    ),
    ...
    try:
        ...
        if conversion_result['success']:
            ...
            conversion_result['original_dashboard'] = {
                ...
                'creation_date': response.get('dashboardCreationDate').isoformat()
                if response.get('dashboardCreationDate')
                else None,
                'last_update_date': response.get('dashboardLastUpdateDate').isoformat()
                if response.get('dashboardLastUpdateDate')
                else None,
            }
...
def validate_dashboard_definition(
    dashboard_definition: str = Field(
        ..., description='JSON string of the dashboard definition to validate'
    ),
) -> Dict[str, Any]:
    ...
    try:
        ...
        for i, widget in enumerate(widgets):
            ...
            if widget_type not in supported_types:
                ...
                validation_results['warnings'].append(
                    f'Widget {i}: Unsupported type "{widget_type}"'
                )
            ...
            for field in required_fields:
                if field not in widget:
                    validation_results['errors'].append(
                        f'Widget {i}: Missing required field "{field}"'
                    )
                    validation_results['valid'] = False
            ...
            for j, metric in enumerate(metrics):
                ...
                if 'propertyId' not in metric:
                    validation_results['errors'].append(
                        f'Widget {i}, Metric {j}: Missing propertyId'
                    )
                    validation_results['valid'] = False

        validation_results['summary']['widget_types'] = list(set(widget_types))
        validation_results['summary']['supported_types'] = [
            t for t in widget_types if t in supported_types
        ]
        ...
        validation_results['summary']['conversion_ready'] = (
            validation_results['valid'] and len(unsupported_types) == 0
        )
    ...
    return {
        ...
        'conversion_rules': {
            ...
            'widget_mappings': {
                ...
                'sc-status-grid': {'grafana_type': 'stat', 'query_type': 'PropertyValue'},
                'sc-status-timeline': {
