...
class ComputationModelDataBindingListItem(BaseModel):
    ...
    def validate_list_item(cls, values):
        ...
        defined = [
            v for v in [values.assetModelProperty, values.assetProperty]
            if v is not None
        ]
        if len(defined) != 1:
            raise ValueError(
...
class ComputationModelDataBindingValue(BaseModel):
    ...
    def validate_binding_value(cls, values):
        """Validate that exactly one binding type is specified."""
        defined = [
            v for v in [values.assetModelProperty, values.assetProperty, values.list]
            if v is not None
        ]
        if len(defined) != 1:
            raise ValueError(
...
class CreateComputationModelRequest(BaseModel):
    ...
    def validate_name(cls, v):
        ...
        validate_string_length(v, 1, 256, 'computationModelName')
        validate_regex_pattern(v, re.compile(r'^[a-zA-Z0-9 _\-#$*!@]+$'), 'computationModelName', '^[a-zA-Z0-9 _\\-#$*!@]+$')
        return v
    ...
    def validate_description(cls, v):
        ...
        if v:
            validate_string_length(v, 1, 2048, 'computationModelDescription')
            validate_regex_pattern(v, re.compile(r'^[a-zA-Z0-9 _\-#$*!@]+$'), 'computationModelDescription', '^[a-zA-Z0-9 _\\-#$*!@]+$')
        return v
    ...
    def validate_data_binding_keys(cls, v):
        ...
        for key in v.keys():
            ...
            validate_regex_pattern(key, re.compile(r'^[a-z][a-z0-9_]*$'), f'Data binding key "{key}"', '^[a-z][a-z0-9_]*$')
        return v
    ...
    def validate_computation_model_version(cls, v):
        ...
        if v:
            ...
            if v not in valid_versions and not (v.isdigit() and 1 <= int(v) <= 9999999999):
                raise ValueError('computationModelVersion must be LATEST, ACTIVE, or a positive integer between 1 and 9999999999')
        return v
...
class TrainingPayload(BaseModel):
    ...
    def validate_training_mode(cls, v):
        ...
        return validate_enum_value(v, ['TRAIN_MODEL', 'START_RETRAINING_SCHEDULER', 'STOP_RETRAINING_SCHEDULER'], 'trainingMode')
    ...
    def validate_training_mode_constraints(cls, values):
        ...
        if training_mode == 'TRAIN_MODEL':
            if values.exportDataStartTime is None or values.exportDataEndTime is None:
                raise ValueError('exportDataStartTime and exportDataEndTime are required for TRAIN_MODEL mode')
        ...
        # For START_RETRAINING_SCHEDULER, retraining configuration is required
            ...
            if values.retrainingConfiguration is None:
                raise ValueError('retrainingConfiguration is required for START_RETRAINING_SCHEDULER mode')
        
        # For STOP_RETRAINING_SCHEDULER, no additional parameters should be provided
        elif training_mode == 'STOP_RETRAINING_SCHEDULER':
            additional_params = [
                values.exportDataStartTime, values.exportDataEndTime, values.targetSamplingRate,
                values.labelInputConfiguration, values.modelEvaluationConfiguration, 
                values.modelMetricsDestination, values.retrainingConfiguration
            ]
            if any(param is not None for param in additional_params):
                raise ValueError('STOP_RETRAINING_SCHEDULER mode does not accept any additional parameters')
...
class InferencePayload(BaseModel):
            ...
            for day, time_ranges in v.items():
                ...
                for time_range in time_ranges:
                    ...
                    if not time_range_pattern.match(time_range):
                        raise ValueError(f'Time range "{time_range}" for "{day}" must be in 24-hour format "HH:MM-HH:MM"')
                    ...
                    if start_minutes >= end_minutes:
                        raise ValueError(f'Start time must be before end time in range "{time_range}" for "{day}"')
    ...
    def validate_inference_mode_constraints(cls, values):
        ...
        # For STOP mode, START-only parameters should not be provided
            ...
            start_only_params = [
                values.dataDelayOffsetInMinutes,
                values.targetModelVersion
            ]
            if any(param is not None for param in start_only_params):
                raise ValueError('dataDelayOffsetInMinutes and targetModelVersion can only be used with START inference mode')
...
class TargetResource(BaseModel):
    ...
    def validate_target_resource(cls, values):
        ...
        defined = [
            v for v in [values.assetId, values.computationModelId]
            if v is not None
        ]
        if len(defined) != 1:
            raise ValueError(
                'TargetResource must define exactly one of: '
                'assetId or computationModelId.'
            )
        return values
...
class DataBindingValueFilter(BaseModel):
    ...
    def validate_asset_model_property(cls, v):
        ...
        if v:
            if 'assetModelId' not in v or 'propertyId' not in v:
                raise ValueError('assetModelProperty filter must contain both assetModelId and propertyId')
            validate_uuid_format(v['assetModelId'], 'assetModelId')
            validate_uuid_format(v['propertyId'], 'propertyId')
        return v
    ...
    def validate_filter(cls, values):
        ...
        defined = [
            v for v in [values.asset, values.assetModel, values.assetProperty, values.assetModelProperty]
            if v is not None
        ]
        if len(defined) != 1:
            raise ValueError(
