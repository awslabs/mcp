...
--          parent_asset_id, asset_external_id, asset_external_model_id, hierarchy_id
...
WHERE pa.property_id = ap.property_id
  AND pa.quality = 'GOOD'
  AND pa.resolution = '1h'
  AND pa.event_timestamp BETWEEN TIMESTAMP '2023-01-01 00:00:00' AND TIMESTAMP '2023-01-02 00:00:00'
  AND ap.property_name LIKE '%{exploration_goal.lower()}%'
...
SELECT 
    ...
    ROUND(COUNT(rts.double_value) * 100.0 / COUNT(rts.asset_id), 2) as completeness_percent,
    ROUND(SUM(CASE WHEN rts.quality = 'GOOD' THEN 1 ELSE 0 END) * 100.0 / COUNT(rts.asset_id), 2) as quality_percent,
