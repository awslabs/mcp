...

{
        ...
        if partition == 'aws'
        else f"❌ **NOT SUPPORTED**: Migration is not supported in the '{partition}' partition. This tool only works in the AWS commercial partition ('aws'). Please use this migration guide in an AWS commercial region."
    }

{
        ''
        if partition == 'aws'
        else '**Migration process will be skipped due to unsupported partition.**'
    }
...
ROLE_ARN=$(aws iam get-role --role-name "$ROLE_NAME" --query 'Role.Arn' --output text --region {
        region
    })
...
WORKSPACE_ID=$(aws grafana create-workspace \\
  ...
  --workspace-description "Migrated from SiteWise Portal: $PORTAL_NAME ({
        portal_id
    }) - $PORTAL_DESCRIPTION" \\
