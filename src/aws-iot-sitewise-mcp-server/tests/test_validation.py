...
class TestValidation:
    ...
    def test_validate_asset_model_id_invalid(self):
        ...
        with pytest.raises(ValidationError, match='assetModelId must be between 13 and 139 characters'):
            validate_asset_model_id('short')  # Too short (5 characters)

        with pytest.raises(ValidationError, match='Invalid assetModelId format'):
            validate_asset_model_id('test-model-123')  # Wrong format (15 characters but invalid pattern)
