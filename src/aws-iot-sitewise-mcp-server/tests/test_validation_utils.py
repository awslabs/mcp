...
class TestValidationUtils:
    ...
    def test_validate_regex_pattern_invalid(self):
        ...
        with pytest.raises(ValueError, match='String must match pattern: lowercase letters only'):
            validate_regex_pattern('Test123', pattern, pattern_description='lowercase letters only')
    ...
    def test_validate_s3_bucket_name_invalid(self):
        ...
        # Invalid format
        with pytest.raises(ValueError, match='bucketName must match pattern: S3 naming conventions'):
    ...
    def test_validate_positive_timestamp_invalid(self):
        ...
        with pytest.raises(ValueError, match='timestamp must be a positive Unix epoch timestamp'):
            validate_positive_timestamp('1640995200')
    ...
    def test_validate_iso8601_duration_invalid(self):
        ...
        # Custom field name
        with pytest.raises(ValueError, match='retentionPeriod must be in ISO 8601 duration format'):
    ...
    def test_validate_lookback_window_invalid(self):
        ...
        with pytest.raises(ValueError, match='lookbackWindow must be one of: P180D, P360D, P540D, P720D'):
    ...
    def test_validate_retraining_frequency_invalid(self):
        ...
        # Invalid format
        with pytest.raises(ValueError, match='retrainingFrequency must be in ISO 8601 duration format'):
    ...
    def test_validate_iana_timezone_invalid(self):
        ...
        with pytest.raises(ValueError, match='tz "invalid" must be a valid IANA timezone identifier'):
        ...
        validation_module.IANA_TIMEZONE_PATTERN = re.compile(r'^[A-Z][a-zA-Z_0-9]*(/[A-Z][a-zA-Z_0-9]*)*$')
        try:
            with pytest.raises(ValueError, match='Invalid timezone component'):
                validate_iana_timezone('America/Chi1cago')  # Contains number, should fail component validation
        finally:
    ...
    def test_validate_time_range_invalid(self):
        ...
        # Custom field name - the error message includes quotes around the value
        with pytest.raises(ValueError, match='businessHours "25:00-26:00" must be in 24-hour format'):
    ...
    def test_validate_client_token_valid(self):
        ...
        valid_tokens = [
            ...
            'a' * 36,  # Minimum length
            'a' * 64,  # Maximum length
            'AbCdEf123456789012345678901234567890',  # Mixed case
        ]
    ...
    def test_time_range_pattern_edge_cases(self):
        ...
        # Invalid time ranges
        invalid_ranges = [
            '9:00-17:00',  # Missing leading zero (actually valid in our pattern)
