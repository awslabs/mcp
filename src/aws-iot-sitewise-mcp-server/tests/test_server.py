...
class TestServer:
    ...
    async def test_run_server_setup(self, mock_fastmcp, mock_task_group):
        ...
        # Asset tools: 8, Asset model tools: 7, Data tools: 9, Gateway tools: 12, Access tools: 6, Monitor tools: 12, plus get_sitewise_server_mode
        # Total expected: 55 tools (34 read-only + 21 write)
        assert mock_mcp_instance.add_tool.call_count == 55

        # Verify prompts were added (4 prompts: 3 read-only + 1 write-only)
    ...
    async def test_run_server_prompts(self, mock_fastmcp, mock_task_group):
        ...
        # Verify all 4 prompts were added (3 read-only + 1 write-only)
        assert mock_mcp_instance.add_prompt.call_count == 4
    ...
    async def test_run_server_error_handling(self, mock_fastmcp, mock_task_group):
        ...
        # Verify tools were added even with the error
        assert mock_mcp_instance.add_tool.call_count == 55
        assert mock_mcp_instance.add_prompt.call_count == 4
