...
class TestSiteWiseMonitor:
    ...
    def test_client_error_handling(self, mock_boto_client):
        ...
        mock_client.describe_access_policy.side_effect = ClientError(
            error_response, 'DescribeAccessPolicy'
        )
    ...
    @patch('awslabs.aws_iot_sitewise_mcp_server.tools.sitewise_monitor.create_sitewise_client')
    def test_describe_access_policy_client_error(self, mock_boto_client):
        """Test describe access policy with ClientError."""
        mock_client = Mock()
        mock_boto_client.return_value = mock_client
    ...
    @patch('awslabs.aws_iot_sitewise_mcp_server.tools.sitewise_monitor.create_sitewise_client')
    def test_describe_dashboard_client_error(self, mock_boto_client):
        """Test describe dashboard with ClientError."""
        mock_client = Mock()
        mock_boto_client.return_value = mock_client
        ...
        mock_client.describe_dashboard.side_effect = ClientError(
            error_response, 'DescribeDashboard'
        )
    ...
    @patch('awslabs.aws_iot_sitewise_mcp_server.tools.sitewise_monitor.create_sitewise_client')
    def test_describe_portal_client_error(self, mock_boto_client):
        """Test describe portal with ClientError."""
        mock_client = Mock()
        mock_boto_client.return_value = mock_client
        ...
        mock_client.describe_portal.side_effect = ClientError(error_response, 'DescribePortal')

        result = describe_portal(portal_id='portal-123', region='us-east-1')

        assert result['success'] is False
        assert result['error_code'] == 'InvalidRequestException'
        assert 'Invalid portal ID' in result['error']

    @patch('awslabs.aws_iot_sitewise_mcp_server.tools.sitewise_monitor.create_sitewise_client')
    def test_describe_project_client_error(self, mock_boto_client):
        """Test describe project with ClientError."""
        mock_client = Mock()
        mock_boto_client.return_value = mock_client
        ...
        mock_client.describe_project.side_effect = ClientError(error_response, 'DescribeProject')
    ...
    @patch('awslabs.aws_iot_sitewise_mcp_server.tools.sitewise_monitor.create_sitewise_client')
    def test_list_access_policies_client_error(self, mock_boto_client):
        """Test list access policies with ClientError."""
        mock_client = Mock()
        mock_boto_client.return_value = mock_client
        ...
        mock_client.list_access_policies.side_effect = ClientError(
            error_response, 'ListAccessPolicies'
        )
    ...
    @patch('awslabs.aws_iot_sitewise_mcp_server.tools.sitewise_monitor.create_sitewise_client')
    def test_list_dashboards_client_error(self, mock_boto_client):
        """Test list dashboards with ClientError."""
        mock_client = Mock()
        mock_boto_client.return_value = mock_client
        ...
        mock_client.list_dashboards.side_effect = ClientError(error_response, 'ListDashboards')

        result = list_dashboards(project_id='invalid-project', region='us-east-1')
    ...
    @patch('awslabs.aws_iot_sitewise_mcp_server.tools.sitewise_monitor.create_sitewise_client')
    def test_list_portals_client_error(self, mock_boto_client):
        """Test list portals with ClientError."""
        mock_client = Mock()
        mock_boto_client.return_value = mock_client
        ...
        mock_client.list_portals.side_effect = ClientError(error_response, 'ListPortals')
    ...
    @patch('awslabs.aws_iot_sitewise_mcp_server.tools.sitewise_monitor.create_sitewise_client')
    def test_list_projects_client_error(self, mock_boto_client):
        """Test list projects with ClientError."""
        mock_client = Mock()
        mock_boto_client.return_value = mock_client
        ...
        mock_client.list_projects.side_effect = ClientError(error_response, 'ListProjects')
    ...
    @patch('awslabs.aws_iot_sitewise_mcp_server.tools.sitewise_monitor.create_sitewise_client')
    def test_list_access_policies_with_fieldinfo_parameters(self, mock_boto_client):
        """Test list access policies with FieldInfo parameters to cover FieldInfo checks."""
        mock_client = Mock()
        mock_boto_client.return_value = mock_client
        ...
        # Create FieldInfo instances to test the isinstance checks
        field_info = FieldInfo()

        result = list_access_policies(
            identity_type=field_info,
            identity_id=field_info,
            resource_type=field_info,
            resource_id=field_info,
            next_token=field_info,
            max_results=field_info,
            region='us-east-1',
        )

        assert result['success'] is True
        # Should call with no parameters since all were FieldInfo instances
        mock_client.list_access_policies.assert_called_once_with()

    @patch('awslabs.aws_iot_sitewise_mcp_server.tools.sitewise_monitor.create_sitewise_client')
    def test_list_dashboards_with_fieldinfo_parameters(self, mock_boto_client):
        """Test list dashboards with FieldInfo parameters to cover FieldInfo checks."""
        mock_client = Mock()
        mock_boto_client.return_value = mock_client

        mock_response = {'dashboardSummaries': [], 'nextToken': None}
        mock_client.list_dashboards.return_value = mock_response
        ...
        assert result['success'] is True
        # Should call with only projectId since others were FieldInfo instances
        mock_client.list_dashboards.assert_called_once_with(projectId='project-123')

    @patch('awslabs.aws_iot_sitewise_mcp_server.tools.sitewise_monitor.create_sitewise_client')
    def test_list_portals_with_fieldinfo_parameters(self, mock_boto_client):
        """Test list portals with FieldInfo parameters to cover FieldInfo checks."""
        mock_client = Mock()
        mock_boto_client.return_value = mock_client

        mock_response = {'portalSummaries': [], 'nextToken': None}
        mock_client.list_portals.return_value = mock_response
    ...
    @patch('awslabs.aws_iot_sitewise_mcp_server.tools.sitewise_monitor.create_sitewise_client')
    def test_list_projects_with_fieldinfo_parameters(self, mock_boto_client):
        """Test list projects with FieldInfo parameters to cover FieldInfo checks."""
        mock_client = Mock()
        mock_boto_client.return_value = mock_client

        mock_response = {'projectSummaries': [], 'nextToken': None}
        mock_client.list_projects.return_value = mock_response
        ...
        assert result['success'] is True
        # Should call with only portalId since others were FieldInfo instances
        mock_client.list_projects.assert_called_once_with(portalId='portal-123')
...
class TestDashboardConversion:
    ...
    def test_create_grafana_target(self):
        ...
        metric = {'assetId': 'test-asset-id', 'propertyId': 'test-property-id'}
    ...
    def test_convert_widget_to_panel_line_chart(self):
        ...
        widget = {
            ...
            'width': 6,
            'height': 6,
            'metrics': [{'assetId': 'test-asset-id', 'propertyId': 'test-property-id'}],
        }
    ...
    def test_convert_dashboard_by_id_success(self, mock_boto_client):
        ...
        result = convert_dashboard_by_id(
            dashboard_id='dashboard-123', datasource_uid='test-datasource-uid', region='us-east-1'
        )
            ...
            'widgets': [
                {
                    ...
                    'width': 6,
                    'height': 6,
                    'metrics': [{'assetId': 'test-asset', 'propertyId': 'test-property'}],
                }
            ]
        }
    ...
    def test_convert_monitor_dashboard_with_fieldinfo_parameters(self):
        """Test dashboard conversion with FieldInfo parameters to cover FieldInfo checks."""
        from pydantic.fields import FieldInfo
        ...
        result = convert_monitor_dashboard_to_grafana(
            dashboard_definition=dashboard_json,
            dashboard_name=field_info,
            dashboard_uid=field_info,
            datasource_uid='test-datasource-uid',
            region='us-east-1',
        )
        ...
        assert result['success'] is True
        grafana_dashboard = result['grafana_dashboard']
        assert grafana_dashboard['title'] == 'Converted Dashboard'  # Default name
        assert len(grafana_dashboard['uid']) == 16  # Auto-generated UID

    @patch('awslabs.aws_iot_sitewise_mcp_server.tools.sitewise_monitor.create_sitewise_client')
    def test_convert_dashboard_by_id_with_fieldinfo_parameters(self, mock_boto_client):
        """Test dashboard conversion by ID with FieldInfo parameters."""
        from pydantic.fields import FieldInfo
        ...
        assert result['success'] is True
        assert result['grafana_dashboard']['title'] == 'Original Dashboard'

    @patch('awslabs.aws_iot_sitewise_mcp_server.tools.sitewise_monitor.create_sitewise_client')
    def test_convert_dashboard_by_id_client_error(self, mock_boto_client):
        """Test dashboard conversion by ID with ClientError."""
        mock_client = Mock()
        mock_boto_client.return_value = mock_client
    ...
    def test_convert_monitor_dashboard_conversion_error(self):
        """Test dashboard conversion with general conversion error."""
        # Test with a dashboard that will cause an exception during conversion
        classic_dashboard = {
            'widgets': [
                {
                    'type': 'sc-line-chart',
        ...
        # Mock uuid.uuid4 to raise an exception to trigger the general exception handler
        with patch(
            'awslabs.aws_iot_sitewise_mcp_server.tools.sitewise_monitor.uuid.uuid4'
        ) as mock_uuid:
            ...
            assert result['success'] is False
            assert result['error_code'] == 'ConversionError'
            assert 'Conversion error' in result['error']

    @patch('awslabs.aws_iot_sitewise_mcp_server.tools.sitewise_monitor.create_sitewise_client')
    def test_convert_dashboard_by_id_conversion_error(self, mock_boto_client):
        """Test dashboard conversion by ID with general conversion error."""
        mock_client = Mock()
        mock_boto_client.return_value = mock_client
        ...
        with patch(
            'awslabs.aws_iot_sitewise_mcp_server.tools.sitewise_monitor.convert_monitor_dashboard_to_grafana'
        ) as mock_convert:
    ...
    def test_validate_dashboard_definition_missing_widgets(self):
        """Test validation with missing widgets field."""
        invalid_dashboard = {}  # Missing widgets field

        result = validate_dashboard_definition(json.dumps(invalid_dashboard))

        assert result['success'] is True
        assert result['valid'] is False
        assert 'Missing required field: widgets' in result['errors']

    def test_validate_dashboard_definition_validation_error(self):
        """Test validation with general validation error."""
        # Mock json.loads to raise an exception to trigger the general exception handler
        with patch(
            'awslabs.aws_iot_sitewise_mcp_server.tools.sitewise_monitor.json.loads'
        ) as mock_loads:
            mock_loads.side_effect = Exception('Validation failed')

            result = validate_dashboard_definition('{"widgets": []}')
    ...
    def test_convert_widget_to_panel_no_title(self):
        """Test converting widget without title to cover default title case."""
        widget = {
            'type': 'sc-line-chart',
            # No title field
            'x': 0,
        ...
        assert panel['title'] == 'Widget Title'  # Default title

    def test_convert_widget_to_panel_no_metrics(self):
        """Test converting widget without metrics."""
        widget = {
            'type': 'sc-line-chart',
            'title': 'Empty Widget',
            'x': 0,
        ...
        assert panel['type'] == 'timeseries'
        assert panel['title'] == 'Empty Widget'
        assert len(panel['targets']) == 0  # No targets created
