...
class TestBulkImportSchema:
    ...
    def test_bulk_import_schema_asset_model_external_id_validation(self):
        ...
        schema2 = BulkImportSchema(
            assetModels=[asset_model_no_ext_id],
            assets=[asset_no_model_ext_ref]
        )
        ...
        with pytest.raises(ValidationError, match="Asset 'test-asset-with-both' references unknown AssetModelExternalId 'unknown-model-external-id'"):
            BulkImportSchema(
                assetModels=[asset_model_with_ext_id],
                assets=[asset_with_both_ids]
            )
        ...
        with pytest.raises(ValidationError, match="Asset 'TestAssetNoExtId' references unknown AssetModelExternalId 'unknown-model-external-id-2'"):
            BulkImportSchema(
                assetModels=[asset_model_with_ext_id],
                assets=[asset_no_ext_id]
            )
        ...
        schema_valid = BulkImportSchema(
            assetModels=[asset_model_with_ext_id_2],
            assets=[asset_with_valid_ref]
        )
