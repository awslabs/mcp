...
from .aws_clients import AWS_REGION, appsignals_client, iam_client, synthetics_client, s3_client, sts_client
...
async def analyze_canary_failures(canary_name: str, region: str = AWS_REGION) -> str:
    ...
    try:
        ...
        if not consecutive_failures:
            result += f"✅ Canary is healthy - no failures since last success\n"
            if last_success_run:
        ...
        if last_success_run:
            ...
            result += f"No recent success run found in history\n\n"
        ...
        if not unique_reasons:
            result += f"✅ No consecutive failures to analyze\n"
            result += f"💡 Canary appears to be recovering or healthy\n"
            return result
        ...
        if artifact_location:
            ...
            if artifact_location.startswith('s3://'):
                ...
                if failure_time:
                    ...
                    failure_run_path = f"{base_path}/{date_path}/" if base_path else f"{date_path}/"
                else:
                    # Fallback to today
                ...
                try:
                    artifacts_response = s3_client.list_objects_v2(Bucket=bucket_name, Prefix=failure_run_path, MaxKeys=50)
                    ...
                    if failure_artifacts:
                        ...
                        har_files = [a for a in failure_artifacts if a['Key'].lower().endswith(('.har', '.har.gz', '.har.html'))]
                        screenshots = [a for a in failure_artifacts if any(ext in a['Key'].lower() for ext in ['.png', '.jpg', '.jpeg'])]
                        logs = [a for a in failure_artifacts if any(ext in a['Key'].lower() for ext in ['.log', '.txt']) or 'log' in a['Key'].lower()]

                        if last_success_run:
                            result += f"🔄 HAR COMPARISON: Failure vs Success\n"
                            ...
                            if success_time:
                                ...
                                success_run_path = f"{base_path}/{success_date_path}/" if base_path else f"{success_date_path}/"
                            else:
                                success_run_path = failure_run_path  # Use same path as fallback
                            try:
                                success_artifacts_response = s3_client.list_objects_v2(Bucket=bucket_name, Prefix=success_run_path, MaxKeys=50)
                                success_artifacts = success_artifacts_response.get('Contents', [])
                                success_har_files = [a for a in success_artifacts if a['Key'].lower().endswith(('.har', '.har.gz', '.har.html'))]
                                
                                if har_files and success_har_files:
                                    failure_har = await analyze_har_file(s3_client, bucket_name, har_files, is_failed_run=True)
                                    success_har = await analyze_har_file(s3_client, bucket_name, success_har_files, is_failed_run=False)
                                    ...
                                    if failure_har.get('request_details'):
                                        result += f"🚨 FAILED REQUESTS:\n"
                                        for req in failure_har['request_details'][:3]:
                                            result += f"• {req.get('url', 'Unknown')}: {req.get('status', 'Unknown')} ({req.get('time', 0):.1f}ms)\n"
                            except Exception:
                                pass
                        else:
                            result += f"🔍 FAILURE ANALYSIS (no success run available for comparison):\n"
                            result += f"Analyzing failure artifacts for: {selected_failure.get('Id', '')[:8]}...\n\n"
                            
                            if har_files:
                                failure_har = await analyze_har_file(s3_client, bucket_name, har_files, is_failed_run=True)
                                result += f"🌐 HAR ANALYSIS:\n"
                                result += f"• Failed requests: {failure_har.get('failed_requests', 0)}\n"
                                result += f"• Total requests: {failure_har.get('total_requests', 0)}\n\n"
                        
                        # Screenshot analysis
                        if screenshots:
                            screenshot_analysis = await analyze_screenshots(s3_client, bucket_name, screenshots, is_failed_run=True)
                            if screenshot_analysis.get('insights'):
                                result += f"📸 SCREENSHOT ANALYSIS:\n"
                        ...
                        # Log analysis
                        if logs:
                            log_analysis = await analyze_log_files(s3_client, bucket_name, logs, is_failed_run=True)
        ...
        if not artifacts_available:
            ...
            result += f"⚠️ Artifacts not available - Checking CloudWatch Logs for root cause\n"
            ...
            if failure_time:
                ...
                if log_analysis.get('status') == 'success':
                    result += f"📋 CLOUDWATCH LOGS ANALYSIS (±5 min around failure):\n"
                    ...
                    if error_logs:
                        result += f"📋 ERROR LOGS AROUND FAILURE:\n"
                        for error in error_logs:
                            result += f"• {error['timestamp'].strftime('%H:%M:%S')}: {error['message']}\n"
                else:
                    result += f"📋 {log_analysis.get('insights', ['Log analysis failed'])[0]}\n"
            else:
                result += f"📋 No failure timestamp available for targeted log analysis\n"

        # Add critical IAM checking guidance for systematic issues
        if "no test result" in str(selected_reason).lower() or "permission" in str(selected_reason).lower() or "access denied" in str(selected_reason).lower():
            try:
                ...
                if arn_check.get('correct'):
                    result += f"✅ Resource ARNs: Correct\n"
                else:
                ...
                if iam_analysis.get('issues_found'):
                    all_iam_issues.extend([f"IAM Policy: {issue}" for issue in iam_analysis['issues_found']])
                if not arn_check.get('correct') and arn_check.get('issues'):
                    all_iam_issues.extend([f"Resource ARN: {issue}" for issue in arn_check['issues']])
                ...
                if iam_analysis.get('recommendations'):
                    all_iam_recommendations.extend([f"Policy Fix: {rec}" for rec in iam_analysis['recommendations']])
        ...
        # 3. Navigation timed out / Page.captureScreenshot timed out
            ...
            if har_files and bucket_name:
                try:
                    har_timeout_analysis = await analyze_har_file(s3_client, bucket_name, har_files, is_failed_run=True)
                    
                    result += f"\n🔍 HAR FILE ANALYSIS FOR NAVIGATION TIMEOUT:\n"
                    if har_timeout_analysis.get('failed_requests', 0) > 0:
                        result += f"• Failed HTTP requests: {har_timeout_analysis['failed_requests']}\n"
                    ...
                    result += f"• Analysis status: {har_timeout_analysis.get('status', 'unknown')}\n"
                    result += f"\n"
                except Exception as har_error:
                    result += f"\n⚠️ HAR analysis failed: {str(har_error)[:100]}\n"
            else:
                result += f"\n🔍 NAVIGATION TIMEOUT DETECTED:\n"
                result += f"• No HAR files available for detailed analysis\n"
                result += f"• Timeout suggests page loading issues or UI changes\n"
