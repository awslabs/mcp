...
class TestWorkflowLinter:
    ...
    async def test_lint_cwl_success(self, mock_cwltool):
...
class TestLintingTools:
    ...
    async def test_check_linting_dependencies(self):
        ...
        with patch('awslabs.aws_healthomics_mcp_server.tools.workflow_linting.WDL') as mock_wdl, \
             patch('awslabs.aws_healthomics_mcp_server.tools.workflow_linting.cwltool') as mock_cwltool:
