...
def get_region(region: Optional[str] = None) -> str:
    """Get the AWS region from parameter, environment variable, or default.

    Args:
        region: Optional region override. If provided, this takes precedence.
...
def get_aws_session(region: Optional[str] = None) -> boto3.Session:
    ...
    Args:
        region: Optional region override. If provided, this takes precedence
                over the AWS_REGION environment variable.
    ...
    return boto3.Session(region_name=get_region(region), botocore_session=botocore_session)
...
def create_aws_client(service_name: str, region: Optional[str] = None) -> Any:
    ...
    try:
        ...
        logger.error(
            f'Failed to create {service_name} client in region {get_region(region)}: {str(e)}'
        )
        raise
...
def get_logs_client(region: Optional[str] = None) -> Any:
    ...
    Raises:
        Exception: If client creation fails
    """
    return create_aws_client('logs', region)
...
def get_ssm_client(region: Optional[str] = None) -> Any:
    ...
    Raises:
        Exception: If client creation fails
    """
    return create_aws_client('ssm', region)
