...
async def test_wdl_bundle_linting():
    ...
    # Main workflow
    main_wdl = Path('multi_file_workflows/wdl_example/main.wdl')
    if main_wdl.exists():
        wdl_files['main.wdl'] = main_wdl.read_text()

    # Task files
    alignment_wdl = Path('multi_file_workflows/wdl_example/tasks/alignment.wdl')
    if alignment_wdl.exists():
        wdl_files['tasks/alignment.wdl'] = alignment_wdl.read_text()

    variant_calling_wdl = Path(
        'multi_file_workflows/wdl_example/tasks/variant_calling.wdl'
    )
    ...
    result = await lint_workflow_bundle(
        ctx=ctx, workflow_files=wdl_files, workflow_format='wdl', main_workflow_file='main.wdl'
    )
    ...
    if result.get('findings'):
        ...
        for finding in result['findings']:
            print(
                f'  - {finding["type"]} in {finding.get("file", "unknown")}: {finding["message"]}'
            )
    ...
    if result.get('warnings'):
        ...
        for warning in result['warnings']:
            print(
                f'  - {warning["type"]} in {warning.get("file", "unknown")}: {warning["message"]}'
            )
...
async def test_cwl_bundle_linting():
    ...
    # Main workflow
    main_cwl = Path('multi_file_workflows/cwl_example/main.cwl')
    if main_cwl.exists():
        cwl_files['main.cwl'] = main_cwl.read_text()

    # Tool files
    alignment_cwl = Path('multi_file_workflows/cwl_example/tools/alignment.cwl')
    if alignment_cwl.exists():
        cwl_files['tools/alignment.cwl'] = alignment_cwl.read_text()

    variant_calling_cwl = Path(
        'multi_file_workflows/cwl_example/tools/variant_calling.cwl'
    )
    ...
    ctx = Context()
    result = await lint_workflow_bundle(
        ctx=ctx, workflow_files=cwl_files, workflow_format='cwl', main_workflow_file='main.cwl'
    )
