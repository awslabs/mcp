...
task SayHello {
    ...
    wdl_result = await lint_workflow_definition(
        ctx=ctx, workflow_content=wdl_content, workflow_format='wdl', filename='hello_world.wdl'
    )
...
steps:
  say_hello:
    ...
    cwl_result = await lint_workflow_definition(
        ctx=ctx, workflow_content=cwl_content, workflow_format='cwl', filename='hello_world.cwl'
    )
