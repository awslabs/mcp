...
"""Unit tests for the disassociate_scram_secret_tool in mutate_cluster register_module."""
...
class TestMutateClusterDisassociateScramSecretTool(unittest.TestCase):
    """Tests for the disassociate_scram_secret_tool in mutate_cluster register_module."""
    ...
    @patch('boto3.client')
    def test_disassociate_scram_secret_tool_success(self, mock_boto3_client):
        ...
        # Set up the mock return value for batch_disassociate_scram_secret
        expected_result = {
            'ClusterArn': 'arn:aws:kafka:us-west-2:123456789012:cluster/test-cluster',
            'UnprocessedScramSecrets': [],
        }
        mock_client.batch_disassociate_scram_secret.return_value = expected_result

        # Register the module
        register_module(mock_mcp)

        # Extract the disassociate_scram_secret_tool function
        disassociate_scram_secret_index = self._find_tool_index(
            mock_mcp, 'disassociate_scram_secret'
        )
        self.assertIsNotNone(
            disassociate_scram_secret_index,
            'Could not find disassociate_scram_secret tool registration',
        )

        disassociate_scram_secret_tool = self._extract_tool_function(
            mock_tool_decorator, disassociate_scram_secret_index
        )
        self.assertIsNotNone(
            disassociate_scram_secret_tool,
            'Could not find disassociate_scram_secret_tool function',
        )
        ...
        result = disassociate_scram_secret_tool(
        ...
        # Verify batch_disassociate_scram_secret was called
    ...
    @patch('boto3.client')
    def test_disassociate_scram_secret_tool_tag_check_failure(self, mock_boto3_client):
        """Test the disassociate_scram_secret_tool function with failed tag check."""
        # Setup
        mock_mcp = Mock()
        mock_tool_decorator = Mock()
        mock_mcp.tool.return_value = mock_tool_decorator
        ...
        # Verify batch_disassociate_scram_secret was NOT called
        mock_client.batch_disassociate_scram_secret.assert_not_called()
