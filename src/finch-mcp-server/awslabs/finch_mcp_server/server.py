...
def get_default_log_path():
    ...
    if os.name == 'nt':  # Windows
        ...
        # Use ~/.local/finch-mcp-server/ for persistent logs
        if 'HOME' in os.environ:
            log_dir = os.path.join(Path.home(), '.local', 'finch-mcp-server')
        else:
            return None  # No suitable location found
    ...
    try:
        ...
        # Return None if we can't create or write to the directory
        return None
...
def main(enable_aws_resource_write: bool = False):
    ...
    if log_file:
        ...
        logger.info('Logging to stderr only')
    else:
...
if __name__ == '__main__':
    ...
    # Set log file from command line if provided
    if args.log_file:
