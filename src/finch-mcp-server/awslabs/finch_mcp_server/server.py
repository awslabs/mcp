...
from  awslabs.finch_mcp_server.utils.vm import check_finch_installation, stop_vm, get_vm_status, is_vm_running, is_vm_nonexistent, is_vm_stopped, initialize_vm, start_stopped_vm, configure_ecr
...
from awslabs.finch_mcp_server.consts import SERVER_NAME, LOG_FILE
...
async def finch_push_image(request: PushImageRequest) -> Result:
    ...
    This tool first ensures that the Finch VM is running, starting it if necessary.
    Then it pushes a container image to a repository. If the image URL is an ECR repository,
    it verifies that ECR login is properly configured before proceeding with the push.
    
    The tool expects the image to be already built and available locally. It uses
    the 'finch image push' command to perform the actual push operation.
    
    Parameters:
        image (str): The full image name to push, including the repository URL and tag.
