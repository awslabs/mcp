    ...
    def test_unix_default_path(self):
        ...
        with (
            ...
            expected_path = os.path.join(
                str(mock_home), '.local', 'finch-mcp-server', 'finch_mcp_server.log'
            )
            assert path == expected_path
            expected_dir = os.path.join(str(mock_home), '.local', 'finch-mcp-server')
...
class TestIntegrationLogging:
    ...
    def test_get_default_log_path_write_test_fails(self):
        """Test get_default_log_path when write test fails."""
        with (
