...
async def test_generate_prompts_invalid_http_method():
    ...
    with patch('awslabs.openapi_mcp_server.prompts.prompt_manager.create_operation_prompt') as mock_create:
        mock_create.return_value = True
        
        manager = MCPPromptManager()
        result = await manager.generate_prompts(mock_server, api_name, openapi_spec)
...
def test_register_api_resource_handler_exception_handling():
    ...
    with patch('awslabs.openapi_mcp_server.prompts.prompt_manager.logger') as mock_logger:
        ...
        mock_logger.warning.assert_called_with('Failed to register resource handler: Registration failed')
