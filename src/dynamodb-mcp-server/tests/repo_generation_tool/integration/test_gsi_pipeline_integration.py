...
class TestGSIPipelineIntegration:
    ...
    def test_gsi_entity_structure_generation(self, generation_output_dir, code_generator):
        ...
        expected_gsi_instance_methods = [
            'build_gsi_pk_statusindex',
            'build_gsi_sk_statusindex',
            'build_gsi_pk_locationindex',
            'build_gsi_sk_locationindex',
            'build_gsi_pk_engagementindex',
            'build_gsi_sk_engagementindex',
            'build_gsi_pk_agegroupindex',
            'build_gsi_sk_agegroupindex',
        ]
        ...
        expected_lookup_methods = [
            'build_gsi_pk_for_lookup_statusindex',
            'build_gsi_sk_for_lookup_statusindex',
            'build_gsi_pk_for_lookup_locationindex',
            'build_gsi_sk_for_lookup_locationindex',
        ]
    ...
    def test_gsi_repository_method_generation(self, generation_output_dir, code_generator):
        ...
        assert 'since_date: str' in repos_content
        ...
        # Check for the method name and parameters separately to be more flexible
        assert 'def get_highly_engaged_users_by_session_range' in repos_content
        assert 'engagement_level: str' in repos_content
        assert 'min_sessions: int' in repos_content
        assert 'max_sessions: int' in repos_content
    ...
    def test_generated_gsi_code_imports_successfully(self, generation_output_dir, code_generator):
        ...
        try:
            ...
            # Test GSI key builders (instance methods)
            assert hasattr(user, 'build_gsi_pk_statusindex')
            assert hasattr(user, 'build_gsi_sk_statusindex')
            assert hasattr(user, 'build_gsi_pk_locationindex')
            assert hasattr(user, 'build_gsi_sk_locationindex')

            # Test key building functionality
            status_pk = user.build_gsi_pk_statusindex()
            assert status_pk == 'STATUS#active'

            location_pk = user.build_gsi_pk_locationindex()
            assert location_pk == 'COUNTRY#US'

            location_sk = user.build_gsi_sk_locationindex()
...
class TestGSIComprehensiveIntegration:
    ...
    def test_gsi_linting_integration(
        self, generation_output_dir, code_generator, repo_generation_tool_path
    ):
        """Test that generated GSI code passes linting."""
        fixtures_path = Path(__file__).parent.parent / 'fixtures'
        user_analytics_schema = (
            fixtures_path / 'valid_schemas' / 'user_analytics' / 'user_analytics_schema.json'
        )
        ...
        result = code_generator(user_analytics_schema, generation_output_dir)
        assert result.returncode == 0, f'GSI generation with linting failed: {result.stderr}'

        # Verify linting was successful (no linting errors in output)
        assert 'ruff' in result.stdout.lower() or result.returncode == 0

        # Double-check by running ruff manually
        import subprocess

        ruff_result = subprocess.run(
            ['uv', 'run', 'ruff', 'check', str(generation_output_dir)],
            cwd=repo_generation_tool_path,
            capture_output=True,
            text=True,
        )

        assert ruff_result.returncode == 0, (
            f'Generated GSI code failed manual ruff check: {ruff_result.stdout}'
        )
    ...
    def test_end_to_end_gsi_pipeline_with_sample_usage(
        ...
        # Entities should have GSI key builders
        assert 'build_gsi_pk_statusindex' in entities_content
        assert 'build_gsi_sk_locationindex' in entities_content
