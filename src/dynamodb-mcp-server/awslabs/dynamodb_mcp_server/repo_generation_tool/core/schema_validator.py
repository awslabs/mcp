...
class SchemaValidator:
    ...
    def validate_schema_file(self, schema_path: str) -> ValidationResult:
        ...
        try:
            # Load JSON file
            # Note: schema_path is intentionally user-provided without path restrictions.
            # This is a developer tool where users need to validate their own schema files
            # from any location on their filesystem.
            ...
            self.result.add_error('json', f'Invalid JSON: {e}', 'Fix JSON syntax errors')
            return self.result
        except Exception as e:
            self.result.add_error(
                'file', f'Error reading file: {e}', 'Check file permissions and format'
            )
            return self.result
    ...
    def format_validation_result(self) -> str:
        ...
        if self.result.errors:
            output.append('❌ Schema validation failed:')
