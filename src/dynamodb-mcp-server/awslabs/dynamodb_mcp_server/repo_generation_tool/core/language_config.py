...
class LanguageConfigLoader:
    ...
    def load(language: str) -> LanguageConfig:
        ...
        # Resolve to absolute path and verify it's within languages_dir (prevent path traversal)
        try:
            resolved_path = config_path.resolve()
            if not str(resolved_path).startswith(str(languages_dir.resolve())):
                raise ValueError(f'Invalid language path: {language}')
        except (OSError, ValueError) as e:
            raise ValueError(f'Invalid language: {language}') from e

        if not resolved_path.exists():
            raise FileNotFoundError(f'Language configuration not found: {resolved_path}')
