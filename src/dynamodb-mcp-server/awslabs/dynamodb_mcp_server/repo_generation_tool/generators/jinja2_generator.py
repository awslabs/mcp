...
class Jinja2Generator(BaseGenerator):
    ...
    def _preprocess_entity_config(self, entity_config: dict[str, Any]) -> dict[str, Any]:
        ...
        processed_config['pk_params'] = self.template_parser.extract_parameters(
            entity_config.get('pk_template', '')
        )
        processed_config['sk_params'] = self.template_parser.extract_parameters(
            entity_config.get('sk_template', '')
        )
