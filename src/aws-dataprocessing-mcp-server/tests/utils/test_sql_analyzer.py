...
class TestSqlAnalyzer:
    ...
    def test_get_query_type_basic_statements(self):
        ...
        for query, expected_type in test_cases:
            ...
            assert result == expected_type, f"Expected {expected_type}, got {result} for query: {query}"
    ...
    def test_security_injection_scenarios(self):
        ...
        for query in malicious_queries:
            assert SqlAnalyzer.contains_write_operations(query), f"Security bypass detected for: {repr(query)}"
            assert not SqlAnalyzer.is_read_only_query(query), f"Security bypass in is_read_only_query for: {repr(query)}"
    ...
    def test_legitimate_read_queries(self):
        ...
        for query in legitimate_queries:
            assert not SqlAnalyzer.contains_write_operations(query), f"Legitimate query blocked: {repr(query)}"
    ...
    def test_enhanced_write_operations_detection(self):
        ...
        for query in enhanced_write_queries:
            assert SqlAnalyzer.contains_write_operations(query), f"Enhanced write operation not detected: {repr(query)}"
            assert not SqlAnalyzer.is_read_only_query(query), f"Write operation incorrectly marked as read-only: {repr(query)}"
    ...
    def test_enhanced_ddl_object_type_detection(self):
        ...
        for query, expected_type in ddl_queries:
            assert SqlAnalyzer.contains_write_operations(query), f"DDL operation not detected: {repr(query)}"
            assert SqlAnalyzer.get_query_type(query) == expected_type, f"Wrong query type for: {repr(query)}"
            assert not SqlAnalyzer.is_read_only_query(query), f"DDL operation incorrectly marked as read-only: {repr(query)}"
    ...
    def test_case_insensitive_enhanced_operations(self):
        ...
        for query in case_variations:
            assert SqlAnalyzer.contains_write_operations(query), f"Case variation not detected: {repr(query)}"
