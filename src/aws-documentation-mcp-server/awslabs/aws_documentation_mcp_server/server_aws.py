...
def main():
    ...
    # Hoist response models into module globals so LazyServer's wrapper-build
    # step (inside mcp.run()) can resolve the string return-type annotations
    # on `search_documentation` and `recommend` via `typing.get_type_hints`.
    from awslabs.aws_documentation_mcp_server import models

    globals()['SearchResponse'] = models.SearchResponse
    globals()['RecommendationResult'] = models.RecommendationResult
