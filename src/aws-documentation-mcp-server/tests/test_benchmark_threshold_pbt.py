...
"""Property-based tests for the benchmark threshold assertion."""
...
def test_threshold_assertion_property(median_ms: float) -> None:
    """check_threshold passes at or below INIT_TIME_THRESHOLD_MS, fails above it."""
    result = check_threshold(median_ms)
    if median_ms > INIT_TIME_THRESHOLD_MS:
        assert result is False, (
            f'Expected check_threshold({median_ms}) to fail (False) '
            f'since {median_ms} > {INIT_TIME_THRESHOLD_MS}, but got True'
        )
    else:
        assert result is True, (
            f'Expected check_threshold({median_ms}) to pass (True) '
