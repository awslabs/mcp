...
"""Benchmark script for AWS Documentation MCP Server initialization time.
...
def compute_median(values: list[float]) -> float:
    """Compute the median of a list of values via statistics.median."""
...
def identify_threshold_violations(
    ...
    """Return violation strings for phases exceeding threshold_ms."""
    violations = []
...
def _run_timed_subprocess(code: str) -> float:
    """Run Python code in a subprocess, return printed elapsed time in ms, or -1 on failure."""
    try:
...
def measure_server_init(module: str = 'server_aws') -> float:
...
def measure_single_import(module_name: str) -> float:
    ...
    code = (
        f'import time; s=time.perf_counter(); import {module_name}; print(time.perf_counter()-s)'
    )
    return _run_timed_subprocess(code)


def measure_phase(phase_name: str, code: str) -> float:
    """Measure a specific init phase in an isolated subprocess."""
    return _run_timed_subprocess(code)
...
def run_benchmark(iterations: int, module: str = 'server_aws') -> dict:
    """Run the full benchmark suite for a given server module."""
    print(
        f'Measuring {module} init time ({iterations} iterations)...',
        file=sys.stderr,
    )
    init_times: list[float] = []
    for i in range(iterations):
        t = measure_server_init(module)
        if t >= 0:
            init_times.append(t)
    ...
    return {
        ...
        'dependency_imports_ms': {name: round(t, 1) for name, t in dep_times.items() if t >= 0},
        'phases': [{'name': name, 'time_ms': round(t, 1)} for name, t in sorted_phases],
...
def format_text_report(results: dict) -> str:
    ...
    lines = []
    lines.append('=' * 60)
    lines.append(f'AWS Documentation MCP Server ({results["module"]}) — Init Benchmark')
    lines.append('=' * 60)
...
def main():
    ...
    parser.add_argument(
        '--module',
        default='server_aws',
        choices=['server_aws', 'server_aws_cn'],
        help='Which server module to benchmark (default: server_aws)',
    )
    ...
    results = run_benchmark(args.iterations, module=args.module)
