"""Tests for schema integration features of the stepfunctions-mcp-server."""
...
with pytest.MonkeyPatch().context() as CTX:
    ...
    from awslabs.stepfunctions_mcp_server.server import (
        ...
        get_schema_arn_from_state_machine_arn,
...
class TestSchemaRegistry:
    ...
    def test_get_schema_valid_arn(self, caplog):
        ...
        with patch('awslabs.stepfunctions_mcp_server.server.schemas_client') as mock_client:
            # Set up the mock
...
class TestSchemaArnRetrieval:
    ...
    @patch('os.environ', {'STATE_MACHINE_INPUT_SCHEMA_ARN_TAG_KEY': 'schema-arn-tag'})
    @patch('awslabs.stepfunctions_mcp_server.server.STATE_MACHINE_INPUT_SCHEMA_ARN_TAG_KEY', 'schema-arn-tag')
    def test_get_schema_arn_from_tags(self):
        """Test getting schema ARN from state machine tags."""
        ...
        with patch('awslabs.stepfunctions_mcp_server.server.lambda_client') as mock_client:
            ...
            # Call the function
            result = get_schema_arn_from_state_machine_arn('test-state-machine-arn')

            # Verify the result
            assert result == schema_arn

            # Verify the client was called correctly
            mock_client.list_tags.assert_called_once_with(Resource='test-state-machine-arn')

    def test_get_schema_arn_no_tag_key_configured(self):
        """Test when tag key is not configured."""
        with patch('awslabs.stepfunctions_mcp_server.server.STATE_MACHINE_INPUT_SCHEMA_ARN_TAG_KEY', None):
            with patch('awslabs.stepfunctions_mcp_server.server.lambda_client') as mock_client:
                # Call the function
                result = get_schema_arn_from_state_machine_arn('test-state-machine-arn')
    ...
    def test_get_schema_arn_tag_not_found(self):
        ...
        with patch('os.environ', {'STATE_MACHINE_INPUT_SCHEMA_ARN_TAG_KEY': 'schema-arn-tag'}):
            with patch(
...
class TestToolCreationWithSchema:
    """Tests for Step Functions tool creation with schemas."""
    ...
    def test_create_tool_with_valid_schema(self, mock_mcp):
        ...
        with patch('awslabs.stepfunctions_mcp_server.server.get_schema_from_registry') as mock_get_schema:
