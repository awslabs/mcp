...
def internal_get_cluster_valid_endpoints(
    ...
    try:
        ...
        cluster_port = DEFAULT_MYSQL_PORT
    # Note: port=0 when Port is None is intentional — endpoints with port=0
    # are excluded by the `if host and port:` checks below, causing ValueError
    # at the end. A cluster without a valid port is treated as an error.
    ...
    if instance_ids:
        ...
        for instance_id in instance_ids:
            try:
                ...
                logger.warning(
                    "Failed to fetch endpoint for member instance '{}': {} - {}",
                    instance_id,
