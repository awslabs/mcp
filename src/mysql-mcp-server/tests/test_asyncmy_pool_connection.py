...
def test_get_credentials_from_secret():
    ...
    mock_client.get_secret_value.return_value = {
        'SecretString': '{"username": "testuser", "password": "testpass"}'  # pragma: allowlist secret
    }
    ...
    with patch(
        ...
        username, password = _get_credentials_from_secret(
            'arn:test',  # pragma: allowlist secret
            'us-east-1',
        )
...
def test_get_credentials_from_secret_missing_username_raises():
    ...
    mock_client = MagicMock()
    secret = {'password': 'p'}  # pragma: allowlist secret
    ...
    with patch(
        ...
        with pytest.raises(ValueError, match='does not contain username'):
            _get_credentials_from_secret('arn:test', 'us-east-1')  # pragma: allowlist secret
