...
class MCPTestBase:
    ...
    def __init__(self, server_path: str, command: str = "uv", args: List[str] = None, env: Dict[str, str] = None):
        ...
        try:
            ...
            if test_type == TestType.TOOL_CALL.value:
                ...
                result = await self.client.get_prompt(
                    test_config["prompt_name"], 
                    test_config.get("arguments", {})
                )
            else:
                return TestResult("custom_test", False, f"Unknown test type: {test_type}")
            
            return TestResult("custom_test", True, details={"result": result.model_dump() if hasattr(result, 'model_dump') else str(result)})
...
def create_validation_rule(rule_type: str, pattern: str, field: Optional[str] = None) -> Dict[str, Any]:
...
def assert_test_results(results: List[TestResult], expected_success_count: Optional[int] = None):
    ...
    if expected_success_count is not None:
        ...
        assert success_count == expected_success_count, f"Expected {expected_success_count} successful tests, got {success_count}"
